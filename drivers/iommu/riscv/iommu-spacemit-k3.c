...
static int k3_iommu_platform_probe(struct platform_device *pdev)
{
	enum riscv_iommu_igs_settings igs;
	struct device *dev = &pdev->dev;
	struct k3_iommu_device *k3_iommu = NULL;
	struct riscv_iommu_device *iommu = NULL;
	struct irq_domain *msi_domain;
	struct resource *res = NULL;
	int vec, ret;

	k3_iommu = devm_kzalloc(dev, sizeof(*k3_iommu), GFP_KERNEL);
	if (!k3_iommu)
		return -ENOMEM;

	iommu = &k3_iommu->iommu;
	...
	k3_iommu->clk = devm_clk_get(dev, NULL);
	if (IS_ERR(k3_iommu->clk))
		return dev_err_probe(dev, PTR_ERR(k3_iommu->clk),
				     "could not get clk\n");
	ret = clk_prepare_enable(k3_iommu->clk);
	if (ret)
...
static void k3_iommu_platform_remove(struct platform_device *pdev)
{
	struct riscv_iommu_device *iommu = dev_get_drvdata(&pdev->dev);
	struct k3_iommu_device *k3_iommu = container_of(iommu, struct k3_iommu_device, iommu);
	bool msi = !(iommu->fctl & RISCV_IOMMU_FCTL_WSI);

	clk_disable_unprepare(k3_iommu->clk);
...
static void k3_iommu_platform_shutdown(struct platform_device *pdev)
{
	riscv_iommu_disable(dev_get_drvdata(&pdev->dev));
};

static const struct of_device_id k3_iommu_of_match[] = {
	{.compatible = "spacemit,k3-iommu",},
	{},
};

static struct platform_driver k3_iommu_platform_driver = {
	.probe = k3_iommu_platform_probe,
	.remove = k3_iommu_platform_remove,
	.shutdown = k3_iommu_platform_shutdown,
	.driver = {
		.name = "spacemit,k3-iommu",
		.of_match_table = k3_iommu_of_match,
		.suppress_bind_attrs = true,
	},
};

builtin_platform_driver(k3_iommu_platform_driver);
