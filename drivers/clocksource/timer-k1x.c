...
static int spacemit_timer_probe(struct platform_device *pdev)
{
	...
	ret = spacemit_of_timer_init(np);
	if (ret)
		dev_err(&pdev->dev, "spacemit_timer: probe failed for node %s\n", np->name);
...
MODULE_DEVICE_TABLE(of, spacemit_timer_of_match);

static struct platform_driver spacemit_timer_driver = {
...
module_platform_driver(spacemit_timer_driver);
#endif
