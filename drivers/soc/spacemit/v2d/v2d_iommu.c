	...
	unsigned long iova, phys_addr_t paddr, size_t size)
{
	...
	while (size != 0) {
		val = ((paddr & mask) >> TTB_ENTRY_SHIFT) & 0x1FFFFFF;
		//V2DLOGD("table val:0x%x\n",val);
		*ttb_entry = val;
		size -= res->page_size;
		ttb_entry++;
		paddr += res->page_size;
	...
	for (i = 0; i < TBU_NUM; i++) {
		if (tbu_slot[i] != -1) {
			...
			if (tbu->ttb_size == 0) {
				...
				V2DLOGD("tbu:%d,0x%08x,0x%08x,0x%08x\n", i,
					__read_reg(res, V2D_MMU_TTBLR_BASE+i*V2D_MMU_TBUx_STEP),
					__read_reg(res, V2D_MMU_TTBHR_BASE+i*V2D_MMU_TBUx_STEP),
					__read_reg(res, V2D_MMU_TCR0_BASE+i*V2D_MMU_TBUx_STEP));
			}
		}
	}
}
...
int v2d_iommu_map_sg(unsigned long iova, struct scatterlist *sg, unsigned int nents, int prot)
{
	...
	for_each_sg(sg, s, nents, i) {
		paddr = sg_dma_address(s) + s->offset;
		size = s->length;
...
#else
	reg = __read_reg(res, V2D_MMU_LAST_RD_PA_ADDR_HI);
	last_pa = reg & 0xFF;
	reg = __read_reg(res, V2D_MMU_LAST_RD_PA_ADDR_LO);
	last_pa = (last_pa << 32) | reg;
	reg = __read_reg(res, V2D_MMU_LAST_RD_VA_ADDR_HI);
	last_va = reg & 0xFF;
	reg = __read_reg(res, V2D_MMU_LAST_RD_VA_ADDR_LO);
	last_va = (last_va << 32) | reg;
#endif
	/* Print IRQ status. */
