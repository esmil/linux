...
#include "v2d_drv.h"
#include "v2d_reg.h"
// #include <dt-bindings/power/spacemit-power.h>
...
static struct device_attribute v2d_sysfs_files[] = {
	__ATTR(clkrate, 0644, v2d_sysfs_clkrate_get, v2d_sysfs_clkrate_set),
};
...
#define V2D_TBU_VA_STEP (0x10000000)
...
static int v2d_get_dma_addr(struct v2d_info *v2dinfo, struct v2d_pending_post_task *cfg)
{
	...
	if (pInfo->valid) {
		...
		if (pLayer1->fbc_enable) {
			...
			pLayer1->phyaddr_uv_l = pLayer1->offset ? (pLayer1->phyaddr_y_l+pLayer1->offset) : 0;
			pLayer1->phyaddr_uv_h = pLayer1->offset ? pLayer1->phyaddr_y_h : 0;
		}
	}
...
static int v2d_job_submit(struct v2d_info *info, V2D_SUBMIT_TASK_S *psubmit)
{
	...
	if (v2d_fence_generate(info, &fence, &pTask->completeFencefd)) {
		pr_err(KERN_ERR "%s-%s-Failed to generate fence(%pf),fd(%d)-slot1\n", "v2d", __func__, fence, pTask->completeFencefd);
		err = -EINVAL;
		goto error;
	}
	if (copy_to_user((__user uint8_t *)psubmit+offsetof(V2D_SUBMIT_TASK_S, completeFencefd), &pTask->completeFencefd, sizeof(int32_t)) != 0) {
...
static void v2d_post_work_func(struct kthread_work *work)
{
	...
	list_for_each_entry_safe(post, next, &info->post_list, head) {
		while (down_timeout(&info->sem_lock, msecs_to_jiffies(2500))) {
			pr_err(KERN_ERR "%s hang do softreset\n", "v2d");
			do_softreset();
		}
...
static int v2d_dev_open(struct inode *inode, struct file *filp)
{
	...
	if (v2d_dev_ref == 1) {
#ifndef V2D_DISABLE_JINDIE_BAND_CAL
		update_spacemit_ddr_bw_read_req(v2dInfo->ddr_qos_cons, 384000);
#endif
	}
#endif
#endif
...
static int v2d_dev_release(struct inode *inode, struct file *filp)
{
	...
	if (v2d_dev_ref == 0) {
#ifndef V2D_DISABLE_JINDIE_BAND_CAL
		update_spacemit_ddr_bw_read_req(v2dInfo->ddr_qos_cons, PM_QOS_CPUIDLE_BLOCK_DEFAULT_VALUE);
#endif
	}
#endif
#endif
...
static int v2d_probe(struct platform_device *pdev)
{
	...
	info->v2d_reset = devm_reset_control_get_exclusive(&pdev->dev, NULL);
	...
	sched_set_fifo_low(info->post_thread);
	kthread_init_work(&info->post_work, v2d_post_work_func);
#if IS_ENABLED(CONFIG_SPACEMIT_DDR_FC) && defined(CONFIG_PM)
#ifndef V2D_DISABLE_JINDIE_BAND_CAL
	info->ddr_qos_cons = register_spacemit_ddr_bw_cons("v2d", PM_QOS_CPUIDLE_BLOCK_DEFAULT_VALUE, PM_QOS_CPUIDLE_BLOCK_DEFAULT_VALUE);
	if (info->ddr_qos_cons == NULL) {
		V2DLOGE("info->ddr_qos_cons is null\n");
		rval = -1;
		goto err;
	}
#endif
...
#if IS_ENABLED(CONFIG_SPACEMIT_DDR_FC) && defined(CONFIG_PM)
#ifndef V2D_DISABLE_JINDIE_BAND_CAL
	rm_spacemit_ddr_bw_cons(info->ddr_qos_cons);
#endif
#endif
	misc_deregister(&info->mdev);
	if (info->v2d_job_done_wq)
