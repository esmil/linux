...
void v2d_dump_irqraw_status(void)
{
	...
	pr_err(KERN_ERR "v2d dump core:0x%x, dec0:0x%x, dec1:0x%x, enc:0x%x\n",  irqerr_raw, dec0_irqraw, dec1_irqraw, encirq);
}
...
#ifdef DUMP_V2DREG
static void v2d_dumpReg(void)
{
	int i;

	V2DLOGD("dump v2d core reg\n");
	for (i = 0; i < 86; i++)
		V2DLOGD("reg%d:0x%08x\n", i, v2d_read(V2D_CORE_BASE+i*4));
	V2DLOGD("dump fbc dec0 reg\n");
	for (i = 0; i < 14; i++)
		V2DLOGD("reg%d:0x%08x\n", i, v2d_read(V2D_L0_DEC_REG0+i*4));
}
#endif
...
#if 0
static int getBytePerPixel(V2D_COLOR_FORMAT_E enFormat)
{
	int Bpp = 0;
...
static void ConfigV2dFbcDecoder(V2D_SURFACE_S *pstV2DSurface, V2D_INPUT_LAYER_E enInputLayer)
{
	...
	V2DLOGD("bbox:[%d,%d,%d,%d],w:%d,h:%d.fmt:%d,split:%d,mode:%d\n",
		pDecInfo->bboxLeft, pDecInfo->bboxTop, pDecInfo->bboxRight, pDecInfo->bboxBottom,
		pstV2DSurface->w, pstV2DSurface->h, pDecInfo->enFbcdecFmt, pDecInfo->is_split, pDecInfo->enFbcdecMode);
...
{
	int *pCscMatrix, i;
	V2D_SCALE_MODE_E enScaleMode = V2D_NO_SCALE;
	...
	if ((!pstV2DSurface->solidcolor.enable) && (pstV2DSurface->phyaddr_y_l == 0) && (!pstV2DSurface->fbc_enable)) {
		...
		V2DLOGD("config %s\n", (enInputLayer > 0 ? "layer1" : "layer0"));
		V2DLOGD("rot:%d,csc:%d, fmt:%d\n", enRotateAngle, enCSCMode, pstV2DSurface->format);
		//blendlayer
		...
		if (pstV2DSurface->solidcolor.enable) {//solid color
			...
			V2DLOGD("solid clolor r:%d,g:%d,b:%d,alpha:%d\n", chl[R], chl[G], chl[B], chl[A]);
