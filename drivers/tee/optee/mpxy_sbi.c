...
static int __mpxy_mbox_send_message(struct rpmi_mbox_message *msg)
{
	int cpu, ret;
...
static void optee_riscv_mpxy_mbox_send_resp(unsigned long a0, unsigned long a1,
					    unsigned long a2, unsigned long a3,
					    unsigned long a4, unsigned long a5,
					    unsigned long a6, unsigned long a7,
					    struct mpxy_opteed_resp *res)
{
	struct mpxy_opteed_msg tx = { .a0 = a0, .a1 = a1, .a2 = a2, .a3 = a3,
				      .a4 = a4, .a5 = a5, .a6 = a6, .a7 = a7 };
	struct mpxy_opteed_resp *rx = res;
	struct rpmi_mbox_message msg = {0};
	int ret;

	rpmi_mbox_init_send_with_response(&msg, OPTEED_MSG_COMMUNICATE,
...
static void optee_enable_shm_cache(struct optee *optee)
{
	...
	while (true) {
		...
		optee_riscv_mpxy_mbox_send_resp(OPTEE_SMC_ENABLE_SHM_CACHE,
						0, 0, 0, 0, 0, 0, 0, &res);
...
static void __optee_disable_shm_cache(struct optee *optee, bool is_mapped)
{
	...
	while (true) {
		union {
			struct mpxy_opteed_resp mpxy_mbox;
			struct optee_smc_disable_shm_cache_result result;
		} res;

		optee_riscv_mpxy_mbox_send_resp(OPTEE_SMC_DISABLE_SHM_CACHE,
						0, 0, 0, 0, 0, 0, 0,
						&res.mpxy_mbox);
 ...
 * 4. Do a normal scheduled call into secure world
 *
 * The function optee_mpxy_do_call_with_arg() performs a normal scheduled
...
/**
 * optee_mpxy_do_call_with_arg() - Do an MPXY request to OP-TEE in secure world
 * @ctx:	calling context
 * @shm:	shared memory holding the message to pass to secure world
 * @offs:	offset of the message in @shm
 * @system_thread: true if caller requests TEE system thread support
 *
 * Does and MPXY request to OP-TEE in secure world and handles eventual
 * resulting Remote Procedure Calls (RPC) from OP-TEE.
 *
 * Returns return code from secure world, 0 is OK
 */
static int optee_mpxy_do_call_with_arg(struct tee_context *ctx,
				       ...
				       bool system_thread)
{
	...
	while (true) {
		...
		optee_riscv_mpxy_mbox_send_resp(param.a0, param.a1, param.a2,
						param.a3, param.a4, param.a5,
						param.a6, param.a7, &res);
...
static u32 get_async_notif_value(bool *value_valid, bool *value_pending)
{
	struct mpxy_opteed_resp res;

	optee_riscv_mpxy_mbox_send_resp(OPTEE_SMC_GET_ASYNC_NOTIF_VALUE,
					0, 0, 0, 0, 0, 0, 0, &res);
...
static irqreturn_t irq_handler(struct optee *optee)
{
	...
	do {
		value = get_async_notif_value(&value_valid, &value_pending);
		if (!value_valid)
			break;
...
static irqreturn_t notif_pcpu_irq_handler(int irq, void *dev_id)
{
	...
	if (irq_handler(optee) == IRQ_WAKE_THREAD)
		queue_work(optee->mpxy.notif_pcpu_wq,
			   &optee->mpxy.notif_pcpu_work);
...
{
	struct optee_mpxy *optee_mpxy = container_of(work, struct optee_mpxy,
						     notif_pcpu_work);
	struct optee *optee = container_of(optee_mpxy, struct optee, mpxy);
...
static int init_pcpu_irq(struct optee *optee, u_int irq)
{
	...
	INIT_WORK(&optee->mpxy.notif_pcpu_work, notif_pcpu_irq_work_fn);
	optee->mpxy.notif_pcpu_wq = create_workqueue("optee_pcpu_notification");
	if (!optee->smc.notif_pcpu_wq) {
		rc = -EINVAL;
		goto err_free_pcpu_irq;
	}

	optee->mpxy.optee_pcpu = optee_pcpu;
	optee->mpxy.notif_irq = irq;
	...
	optee->mpxy.notif_cpuhp_state = rc;
...
static int optee_mpxy_notif_init_irq(struct optee *optee, u_int irq)
{
...
{
	cpuhp_remove_state(optee->mpxy.notif_cpuhp_state);

	destroy_workqueue(optee->mpxy.notif_pcpu_wq);

	free_percpu_irq(optee->mpxy.notif_irq, optee->mpxy.optee_pcpu);
	free_percpu(optee->mpxy.optee_pcpu);
}

static void optee_mpxy_notif_uninit_irq(struct optee *optee)
{
	if (optee->mpxy.sec_caps & OPTEE_SMC_SEC_CAP_ASYNC_NOTIF) {
		optee_stop_async_notif(optee->ctx);
		if (optee->mpxy.notif_irq) {
			if (irq_is_percpu_devid(optee->mpxy.notif_irq))
				uninit_pcpu_irq(optee);
			else
				free_irq(optee->mpxy.notif_irq, optee);

			irq_dispose_mapping(optee->mpxy.notif_irq);
		}
	}
}
			      ...
			      struct tee_ioctl_version_data *vers)
{
	...
	if (optee->mpxy.sec_caps & OPTEE_SMC_SEC_CAP_DYNAMIC_SHM)
		v.gen_caps |= TEE_GEN_CAP_REG_MEM;
	if (optee->mpxy.sec_caps & OPTEE_SMC_SEC_CAP_MEMREF_NULL)
		v.gen_caps |= TEE_GEN_CAP_MEMREF_NULL;
	*vers = v;
}

static int optee_mpxy_open(struct tee_context *ctx)
{
	struct optee *optee = tee_get_drvdata(ctx->teedev);
	u32 sec_caps = optee->mpxy.sec_caps;
...
static const struct optee_ops optee_ops = {
	.do_call_with_arg = optee_mpxy_do_call_with_arg,
	.to_msg_param = optee_to_msg_param,
	.from_msg_param = optee_from_msg_param,
};

static int enable_async_notif(void)
{
	struct mpxy_opteed_resp res;

	optee_riscv_mpxy_mbox_send_resp(OPTEE_SMC_ENABLE_ASYNC_NOTIF,
					0, 0, 0, 0, 0, 0, 0, &res);
...
static bool optee_msg_api_uid_is_optee_api(void)
{
	...
	optee_riscv_mpxy_mbox_send_resp(OPTEE_SMC_CALLS_UID,
					0, 0, 0, 0, 0, 0, 0, &res);
...
static void optee_msg_get_os_revision(void)
{
	...
	optee_riscv_mpxy_mbox_send_resp(OPTEE_SMC_CALL_GET_OS_REVISION,
					0, 0, 0, 0, 0, 0, 0, &res.mpxy_mbox);
...
static bool optee_msg_api_revision_is_compatible(void)
{
	...
	optee_riscv_mpxy_mbox_send_resp(OPTEE_SMC_CALLS_REVISION,
					0, 0, 0, 0, 0, 0, 0, &res.mpxy_mbox);
...
static bool optee_msg_exchange_capabilities(u32 *sec_caps, u32 *max_notif_value,
					    unsigned int *rpc_param_count)
{
	...
	optee_riscv_mpxy_mbox_send_resp(OPTEE_SMC_EXCHANGE_CAPABILITIES,
					a1, 0, 0, 0, 0, 0, 0, &res.mpxy_mbox);
...
static unsigned int optee_msg_get_thread_count(void)
{
	struct mpxy_opteed_resp res;

	optee_riscv_mpxy_mbox_send_resp(OPTEE_SMC_GET_THREAD_COUNT,
					0, 0, 0, 0, 0, 0, 0, &res);
	if (res.a0)
		return 0;
	return res.a1;
...
static struct tee_shm_pool *optee_config_shm_memremap(void **memremaped_shm)
{
	...
	optee_riscv_mpxy_mbox_send_resp(OPTEE_SMC_GET_SHM_CONFIG,
					0, 0, 0, 0, 0, 0, 0, &res.mpxy_mbox);
	if (res.result.status != OPTEE_SMC_RETURN_OK) {
 ...
 */
static void optee_mpxy_remove(struct platform_device *pdev)
{
	...
	optee_mpxy_notif_uninit_irq(optee);

	optee_remove_common(optee);

	if (optee->mpxy.memremaped_shm)
		memunmap(optee->mpxy.memremaped_shm);
...
static int riscv_mpxy_mbox_probe(struct platform_device *pdev)
{
...
static int optee_probe(struct platform_device *pdev)
{
	...
	thread_count = optee_msg_get_thread_count();
	...
	if (sec_caps & OPTEE_SMC_SEC_CAP_DYNAMIC_SHM) {
		 ...
		 *
		 * This means that optee_mpxy_do_call_with_arg() will use
	...
	optee->mpxy.memremaped_shm = memremaped_shm;
	optee->pool = pool;
	...
	if (sec_caps & OPTEE_SMC_SEC_CAP_ASYNC_NOTIF) {
		...
		rc = optee_mpxy_notif_init_irq(optee, irq);
		if (rc) {
