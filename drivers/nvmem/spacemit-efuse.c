...
/*
 * read efuse data to buffer for spacemit SoCs.
 */
static int spacemit_efuse_read_hw(struct spacemit_efuse_bank *efuse)
{
	int i, ret;
	u32 *buffer;
...
static const struct of_device_id spacemit_efuse_match[] = {
	{
		.compatible = "spacemit,efuse",
		.data = (void *)&spacemit_efuse_read_hw,
	},
	{ /* sentinel */},
};
