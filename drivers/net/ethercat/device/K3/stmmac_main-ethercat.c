...
#include "stmmac_ptp-ethercat.h"
#include "stmmac-ethercat.h"
#include "stmmac_xdp-ethercat.h"
#include <linux/reset.h>
#include <linux/of_mdio.h>
#include "dwmac1000-ethercat.h"
#include "dwxgmac2-ethercat.h"
#include "hwif-ethercat.h"\


static inline void ec_txq_trans_cond_update(struct netdev_queue *txq)
{
...
static void stmmac_tx_timer_arm(struct stmmac_priv *priv, u32 queue)
{
	...
	if (!tx_coal_timer || get_ecdev(priv))
		return;
...
read_again:
		...
		status = stmmac_rx_status(priv, &priv->xstats, p);
		/* check if managed by the DMA otherwise go ahead */
		netdev_dbg(priv->dev, "stmmac_rx: error: %d, status: %x\n", error, status);
		if (unlikely(status & dma_own))
		{
			break;
		}
		...
		dma_rmb();
		netdev_dbg(priv->dev, "buffer good");
		...
		if (get_ecdev(priv)) {
			...
			ecdev_receive(get_ecdev(priv), va, len);
...
{
	struct stmmac_priv *priv =
		container_of(work, struct stmmac_priv, ec_watchdog_kicker);
	stmmac_common_interrupt(priv);
}
...
static void ec_poll(struct net_device *netdev)
{
	...
	if (jiffies - priv->ec_watchdog_jiffies >= 2 * HZ) {
		priv->ec_watchdog_jiffies = jiffies;
		irq_work_queue(&priv->ec_watchdog_kicker);
	}
	...
	for (i = 0; i < maxq; i++) {
		...
		u32 chan = ch->index;
		int work_done;
		bool pending_packes = false;

		work_done = stmmac_rx(priv, budget, chan);
		work_done = stmmac_tx_clean(priv, budget, chan, &pending_packes);
	}
}
		     ...
		     struct stmmac_resources *res)
{
	...
	/*
	 * Disable SPH when using EtherCAT: with SPH enabled, the software has to
	 * actively merge packets before forwarding them to the EtherCAT master,
	 * which adds extra complexity.
	 */
	int sph_orig = priv->sph;
	priv->sph = false;
	if (!get_ecdev(priv)) {
