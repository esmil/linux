...
int get_raw_data_plane_rdma_mem_size(u32 drm_4cc_fmt, bool rot_90_or_270, u32 plane_crop_width, u32 *output_mem_size)
{
	...
	if (rot_90_or_270) {
		...
		info = drm_format_info(drm_4cc_fmt);
		if (info->cpp[0] == 0) {
			DRM_ERROR("FBC_MEM: not support format %p4cc\n", &drm_4cc_fmt);
			return -1;
		}
		if (info->num_planes == 1 && info->is_yuv == false) {
			...
			DRM_ERROR("FBC_MEM: not considered drm format %p4cc\n", &drm_4cc_fmt);
			return -1;
		}
	...
	if (output_mem_size) {
		*output_mem_size = ret_mem_size;
		DRM_DEBUG("FBC_MEM: raw layer, fmt = %p4cc, rot_90_270 = %d, crop_w = %u, cal memsize = %u\n",
		...
		u32 fbc_block_size, bool rot_90_or_270, u8 min_lines, u32 *output_mem_size)
{
	...
	uint32_t num_addr_frac = 0;
	uint32_t num_addr_line = 0;
	uint32_t align_num = (fbc_block_size == FBC_BLOCK_SIZE_16x16) ? 16 : 32;

	DRM_DEBUG("FBC_MEM: afbc layer, fmt = %p4cc, \
	...
	if (min_lines == 4) {
		num_addr_frac = (num_addr_per_line % 8 == 0) ? num_addr_per_line/8 : (num_addr_per_line/8 + 1);
		num_addr_line = num_addr_frac * 8 * 4;
		ret_mem_size = num_addr_line;
	} else if (min_lines == 8) {
		num_addr_frac = (num_addr_per_line % 16 == 0) ? num_addr_per_line/16 : (num_addr_per_line/16 + 1);
		num_addr_line = num_addr_frac * 16 * 8;
		ret_mem_size = num_addr_line;
	} else if (min_lines == 16) {
		num_addr_frac = (num_addr_per_line % 32 == 0) ? num_addr_per_line/32 : (num_addr_per_line/32 + 1);
		num_addr_line = num_addr_frac * 32 * 16;
		ret_mem_size = num_addr_line;
	}
				   ...
				   struct drm_plane_state *state)
{
	...
	bool rot_90_or_270 = is_rot_90_270(rotation);
	...
	const struct drm_format_info *info = NULL;
	u8 min_lines = pstate->dec_lines ? pstate->dec_lines : 4;

	info = drm_format_info(drm_4cc_fmt);

	DRM_DEBUG("min_lines %d ac->dec_lines %d\n", min_lines, pstate->dec_lines);
	...
	if (modifier == 0) { //raw data layer
		...
		if (modifier && info->is_yuv) { //video layer
			if (rot_90_or_270)
				min_lines = 16; //dpu hardware request 16 line at least
			else
				min_lines = 8; //dpu hardware request 8 line at least
		} else if (modifier && rot_90_or_270) {
			min_lines = 8;
		}
		...
		if (index % 2 == 0) { //dma_id is even: 0, 2, 4, 6...
			...
			if (index > 0) {
				...
				if (pre_odd_rdma_use_shared_fbc_mem) {
					DRM_INFO_ONCE("FBC_MEM: both rdma %d %d use fbc memory\n", index, index - 1);
					goto free;
				}
			}
