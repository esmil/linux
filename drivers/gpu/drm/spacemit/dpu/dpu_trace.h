...
TRACE_EVENT(dpu_mclk_scl,
	...
	TP_printk("scl_in_width = %llu, scl_in_height = %llu, scl_out_width = %llu, scl_out_height = %llu, bpp = %llu, C = %llu, width_ratio = %llu, height_ratio = %llu\n",
...
DEFINE_EVENT(dpu_func_template, spacemit_crtc_stop,
	TP_PROTO(uint32_t dev_id),
	TP_ARGS(dev_id)
);
...
TRACE_EVENT(csc_dump_state,
	TP_PROTO(int new_encoding, int new_range, int old_encoding, int old_range, int rdma_id),
	TP_ARGS(new_encoding, new_range, old_encoding, old_range, rdma_id),
	TP_STRUCT__entry(
		__field(int, new_encoding)
		__field(int, new_range)
		__field(int, old_encoding)
		__field(int, old_range)
		__field(int, rdma_id)
	),
	TP_fast_assign(
		__entry->new_encoding = new_encoding;
		__entry->new_range = new_range;
		__entry->old_encoding = old_encoding;
		__entry->old_range = old_range;
		__entry->rdma_id = rdma_id;
	),

	TP_printk("csc_dump_state new_encoding %d new_range %d old_encoding %d old_range %d rdma_id %d", \
		__entry->new_encoding,      \
		__entry->new_range,    \
		__entry->old_encoding,    \
		__entry->old_range,    \
		__entry->rdma_id)
);
