...
{
	uint32_t irq_mask = DSI_IRQ_MASK_BITS;

	//if (cpu_is_dovenr())
	//	irq_mask &= ~DSI_IRQ_VPN_BF_OVERRUN_ERR;

	if (enable)
		dsi_set_bits(base_addr, DSI_IRQ_MASK, irq_mask);
	else
		dsi_clear_bits(base_addr, DSI_IRQ_MASK, irq_mask);
}
...
static void dsi_config_video_mode(struct spacemit_dsi_device *dsi_ctx, struct spacemit_mipi_info *mipi_info)
{
	...
	if (dsi_ctx->version >= DSI_VERSION_2)
		if (mipi_info->vpn_dly_cnt && mipi_info->vpn_tx_dly_cnt)
			...
			dsi_write(base_addr, DSI_VPN_CTRL_0, (0x50 << CFG_VPN_DLY_CNT_SHIFT) | 0x508);
	else
	...
	dsi_write(base_addr, DSI_VPN_TIMING_2, ((height_val)<<16) | (v_total));
...
static void dsi_config_cmd_mode(struct spacemit_dsi_device *dsi_ctx, struct spacemit_mipi_info *mipi_info)
{
	...
	if (dsi_ctx->version >= DSI_VERSION_2)
		dsi_write(dsi_ctx->base_addr, DSI_PHY_CTRL_2, CFG_DPHY_LANE_EN | CFG_DPHY_HSTX_RX);
...
static struct dsi_core_ops dsi_core_ops = {
	...
	.dsi_enable_irq = spacemit_dsi_enable_irq,
	.dsi_update_vrr = spacemit_dsi_update_vrr,
};
