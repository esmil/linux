...
#ifndef __INNO_PARSE_EDID_H__
#define __INNO_PARSE_EDID_H__
...
#define COMBINE_HI_4LO(hi, lo)		((((unsigned int)hi) << 4) | (unsigned int)lo)
#define COMBINE_HI_8LO(hi, lo)		((((unsigned int)hi) << 8) | (unsigned int)lo)
#define PIXEL_CLOCK_LO			((unsigned int)i_pDtd[0])
#define PIXEL_CLOCK_HI			((unsigned int)i_pDtd[1])
#define PIXEL_CLOCK			(COMBINE_HI_8LO(PIXEL_CLOCK_HI, \
						  PIXEL_CLOCK_LO) * 10000)
#define H_ACTIVE_LO			((unsigned int)i_pDtd[2])
#define H_BLANKING_LO			((unsigned int)i_pDtd[3])
#define H_ACTIVE_HI			(UPPER_NIBBLE((unsigned int)i_pDtd[4]))
#define H_ACTIVE			(COMBINE_HI_8LO(H_ACTIVE_HI, H_ACTIVE_LO))
#define H_BLANKING_HI			(LOWER_NIBBLE((unsigned int)i_pDtd[4]))
#define H_BLANKING			(COMBINE_HI_8LO(H_BLANKING_HI, H_BLANKING_LO))
#define V_ACTIVE_LO			((unsigned int)i_pDtd[5])
#define V_BLANKING_LO			((unsigned int)i_pDtd[6])
#define V_ACTIVE_HI			(UPPER_NIBBLE((unsigned int)i_pDtd[7]))
#define V_ACTIVE			(COMBINE_HI_8LO(V_ACTIVE_HI, V_ACTIVE_LO))
#define V_BLANKING_HI			(LOWER_NIBBLE((unsigned int)i_pDtd[7]))
#define V_BLANKING			(COMBINE_HI_8LO(V_BLANKING_HI, V_BLANKING_LO))
#define H_SYNC_OFFSET_LO		((unsigned int)i_pDtd[8])
#define H_SYNC_WIDTH_LO			((unsigned int)i_pDtd[9])
#define V_SYNC_OFFSET_LO		(UPPER_NIBBLE((unsigned int)i_pDtd[10]))
#define V_SYNC_WIDTH_LO			(LOWER_NIBBLE((unsigned int)i_pDtd[10]))
#define V_SYNC_WIDTH_HI			((unsigned int)i_pDtd[11] & (1 | 2))
#define V_SYNC_OFFSET_HI		(((unsigned int)i_pDtd[11] & (4 | 8)) >> 2)
#define H_SYNC_WIDTH_HI			(((unsigned int)i_pDtd[11] & (16 | 32)) >> 4)
#define H_SYNC_OFFSET_HI		(((unsigned int)i_pDtd[11] & (64 | 128)) >> 6)
#define V_SYNC_WIDTH			(COMBINE_HI_4LO(V_SYNC_WIDTH_HI, \
						   V_SYNC_WIDTH_LO))
#define V_FRONT_PORCH			(COMBINE_HI_4LO(V_SYNC_OFFSET_HI, \
						   V_SYNC_OFFSET_LO))
#define H_SYNC_WIDTH			(COMBINE_HI_4LO(H_SYNC_WIDTH_HI, \
						   H_SYNC_WIDTH_LO))
#define H_FRONT_PORCH			(COMBINE_HI_4LO(H_SYNC_OFFSET_HI, \
						   H_SYNC_OFFSET_LO))
#define H_SIZE_LO			((unsigned int)i_pDtd[12])
#define V_SIZE_LO			((unsigned int)i_pDtd[13])
#define H_SIZE_HI			(UPPER_NIBBLE((unsigned int)i_pDtd[14]))
#define V_SIZE_HI			(LOWER_NIBBLE((unsigned int)i_pDtd[14]))
#define H_SIZE				(COMBINE_HI_8LO(H_SIZE_HI, H_SIZE_LO))
#define V_SIZE				(COMBINE_HI_8LO(V_SIZE_HI, V_SIZE_LO))
#define H_BORDER			((unsigned int)i_pDtd[15])
#define V_BORDER			((unsigned int)i_pDtd[16])
#define FLAGS				((unsigned int)i_pDtd[17])
