...
#define READ_FROM_CACHE_OP_QUADIO_EXT	0xec
...
static int is_read_from_cache_opcode(u8 opcode)
{
	...
	ret = ((opcode == READ_FROM_CACHE_OP) ||
		...
		(opcode == READ_FROM_CACHE_OP_QUADIO_EXT));
...
static int spacemit_qspi_adjust_op_size(struct spi_mem *mem, struct spi_mem_op *op)
{
	...
	if (op->data.dir == SPI_MEM_DATA_OUT) {
		...
		if (qspi->ahb_read_enable) {
			if (op->addr.val + op->data.nbytes > qspi->memmap_size) {
				if (op->addr.val + qspi->rx_buf_size <= qspi->memmap_size)
					qspi->force_cmd_read = 0;
				else
					qspi->force_cmd_read = 1;
...
static int spacemit_qspi_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(dev->of_node, "spacemit,qspi-sfb2ad", &qspi->sfb2ad))
	...
	if (of_property_read_u32(dev->of_node, "spacemit,qspi-rx-buf", &qspi->rx_buf_size)) {
		qspi->rx_buf_size = QSPI_RX_BUFF_MAX;
	}

	if (of_property_read_u32(dev->of_node, "spacemit,qspi-tx-buf", &qspi->tx_buf_size)) {
		qspi->tx_buf_size = QSPI_TX_BUFF_MAX;
	}

	if (of_property_read_u32(dev->of_node, "spacemit,qspi-ahb-buf", &qspi->ahb_buf_size)) {
	...
	if (of_property_read_u32(dev->of_node, "spacemit,qspi-endian-xchg", &qspi->endian_xchg)) {
		qspi->endian_xchg = 0;
	}

	if (of_property_read_u32(dev->of_node, "spacemit,qspi-cs", &qspi->cs_selected)) {
		qspi->cs_selected = QSPI_DEFAULT_CS;
	}

	if (of_property_read_u32(dev->of_node, "spacemit,qspi-tx-dma", &qspi->tx_dma_enable)) {
		qspi->tx_dma_enable = 0;
	}

	if (of_property_read_u32(dev->of_node, "spacemit,qspi-rx-dma", &qspi->rx_dma_enable)) {
		qspi->rx_dma_enable = 0;
	}
	...
	if (of_property_read_u32(dev->of_node, "spacemit,qspi-id", &qspi_bus_num))
		ctlr->bus_num = 0;
	else
