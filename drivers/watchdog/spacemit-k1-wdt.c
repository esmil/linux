...
#ifdef CONFIG_SPACEMIT_K1_WDT_TEST
#define K1_WATCHDOG_IRQ_EXPIRE_TIME	(16)
#define K1_WATCHDOG_IRQ_TEST_TIME	(64)
#define K1_WATCHDOG_IRQ_TEST_ID		0
...
static int spa_wdt_start_irq(struct watchdog_device *wdd)
{
	...
	/* set timeout = 2/256 s */
	spa_wdt_set_timeout(&info->wdt_dev, K1_WATCHDOG_IRQ_EXPIRE_TIME);
	/* enable counter and reset/interrupt */
...
static void spa_wdt_reset_test(struct spa_wdt_info *info)
{
	...
	spa_wdt_write(info, WDT_WSR, 0x0);
	spa_wdt_set_timeout(&info->wdt_dev, 1);
	spa_wdt_write(info, WDT_WMER, 0x3);
	spa_wdt_write(info, WDT_WCR, 0x1);
...
static void spa_wdt_irq_test(struct spa_wdt_info *info)
{
	...
	wdt_irq_count = 0;
	expected_irq_count = K1_WATCHDOG_IRQ_TEST_TIME / K1_WATCHDOG_IRQ_EXPIRE_TIME;
	...
	mdelay((K1_WATCHDOG_IRQ_TEST_TIME) * 1000 / 256 + 50);
			      ...
			      const char *buf, size_t size)
{
	...
	if (test_id == K1_WATCHDOG_IRQ_TEST_ID)
...
MODULE_DESCRIPTION("Spacemit k1 plat Watchdog Device Driver");
