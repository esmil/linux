...
{
	static const char msg[] = "LOG GROUPS\n"
				  "\n"
 ...
 */
static ssize_t drain_ram_read(struct mvx_log_drain_ram *drain, char __user *user_buffer, size_t count, loff_t *position,
			      size_t pos)
{
	ssize_t n = 0;
...
static ssize_t drain_ram_read_msg(struct file *file, char __user *user_buffer, size_t count, loff_t *position)
{
	...
	while (*position == drain->write_pos) {
		...
		ret = wait_event_interruptible(drain->queue, *position < drain->write_pos);
		if (ret != 0)
			return -EINTR;
	}
...
{
	int ret;
	static const struct file_operations readme_fops = {.read = readme_read};
	struct dentry *dentry;
	...
	/* Create <group>/drain. */
	dentry = debugfs_create_file("README", 0400, log->log_dir, NULL, &readme_fops);
	if (IS_ERR_OR_NULL(dentry)) {
...
static int drain_construct(struct mvx_log_drain *drain, mvx_print_fptr print, mvx_data_fptr data)
{
	drain->print = print;
	drain->data = data;
...
static void drain_destruct(struct mvx_log_drain *drain) { UNUSED(drain); }

static void drain_dmesg_print(struct mvx_log_drain *drain, enum mvx_log_severity severity, const char *tag,
...
static void drain_dmesg_data(struct mvx_log_drain *drain, enum mvx_log_severity severity, struct iovec *vec,
			     size_t count)
{
	size_t i;
	...
	for (i = 0; i < count; ++i) {
		...
		while (length > 0) {
			...
			while (j-- > 0)
				n += scnprintf(&buf[n], (buf_size)-n, " %02x", *p++);
...
void mvx_log_drain_dmesg_destruct(struct mvx_log_drain *drain) { drain_destruct(drain); }

int mvx_log_drain_add(struct mvx_log *log, const char *name, struct mvx_log_drain *drain)
{
	if (!IS_ENABLED(CONFIG_DEBUG_FS)) {
		pr_info("MVX: Debugfs is not enabled. '%s' dir is not created.\n", name);
		return 0;
...
static void drain_ram_data(struct mvx_log_drain *drain, enum mvx_log_severity severity, struct iovec *vec, size_t count)
{
...
static void drain_ram_print(struct mvx_log_drain *drain, enum mvx_log_severity severity, const char *tag,
			    const char *msg, const unsigned int n_args, ...)
{
	char buf[500];
	va_list args;
...
int mvx_log_drain_ram_construct(struct mvx_log_drain_ram *drain, size_t buffer_size)
{
	int ret;
...
int mvx_log_drain_ram_add(struct mvx_log *log, const char *name, struct mvx_log_drain_ram *drain)
{
	static const struct file_operations drain_ram_msg = {.read = drain_ram_read_msg,
	...
	/* Create dentry. */
	dentry = debugfs_create_file("msg", 0600, drain->base.dentry, NULL, &drain_ram_msg);
	if (IS_ERR_OR_NULL(dentry)) {
...
#ifdef MVX_LOG_FTRACE_ENABLE
static void drain_ftrace_print(struct mvx_log_drain *drain, enum mvx_log_severity severity, const char *tag,
			       const char *msg, const unsigned int n_args, ...)
{
	va_list args;
	char fmt[500];
...
static void drain_ftrace_data(struct mvx_log_drain *drain, enum mvx_log_severity severity, struct iovec *vec,
			      size_t count)
{
	size_t i;
...
void mvx_log_drain_ftrace_destruct(struct mvx_log_drain *drain) { drain_destruct(drain); }
...
void mvx_log_group_construct(struct mvx_log_group *group, const char *tag, const enum mvx_log_severity severity,
...
int mvx_log_group_add(struct mvx_log *log, const char *name, struct mvx_log_group *group)
{
	static const struct file_operations group_drain_fops = {.read = group_drain_read, .write = group_drain_write};
	struct dentry *dentry;
	int ret;
...
void mvx_log_group_destruct(struct mvx_log_group *group) { UNUSED(group); }
