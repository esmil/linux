 ...
 */
static int get_stride(enum mvx_format format, uint8_t *nplanes, unsigned int stride[MVX_BUFFER_NPLANES][2])
{
	switch (format) {
...
static int map_plane(struct mvx_buffer *buf, mvx_mmu_va begin, mvx_mmu_va end, unsigned int plane)
{
	while (begin < end) {
		struct mvx_buffer_plane *p = &buf->planes[plane];
		int ret;

		ret = mvx_mmu_map_pages(buf->mmu, begin, p->pages, MVX_ATTR_SHARED_RW, MVX_ACCESS_READ_WRITE);
		if (ret == 0) {
			MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO,
				      "Memory map buffer. buf=%p, plane=%u, va=0x%x, size=%zu.", buf, plane,
				      p->pages->va, mvx_buffer_size(buf, plane));
			return 0;
		}
...
void mvx_buffer_show(struct mvx_buffer *buf, struct seq_file *s)
{
	...
	for (i = 0; i < buf->nplanes; ++i) {
		...
		mvx_seq_printf(s, tag, ind, "va: 0x%08x, size: %10zu, stride: %5u, filled: %10u\n",
			       mvx_buffer_va(buf, i), mvx_buffer_size(buf, i), plane->stride, plane->filled);
	}

	ind--;
}

int mvx_buffer_construct(struct mvx_buffer *buf, struct device *dev, struct mvx_mmu *mmu, enum mvx_direction dir,
			 unsigned int nplanes, struct sg_table **sgt)
{
	int i;

	if (nplanes > MVX_BUFFER_NPLANES) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Failed to construct buffer. Too many planes. nplanes=%u.",
			      nplanes);
		return -EINVAL;
	}
...
{
	return (buf->planes[0].pages != NULL) && (buf->planes[0].pages->va != 0);
}
...
int mvx_buffer_synch(struct mvx_buffer *buf, enum dma_data_direction dir)
{
	...
	for (i = 0; i < buf->nplanes; i++) {
		...
		if (plane->filled) {
			page_off_end =
				(plane->offset + plane->pages->offset + plane->filled + PAGE_SIZE - 1) >> PAGE_SHIFT;
		}
		...
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO,
			      "plane [%d] sync pages: %p, dir: %d, page offset: %d, pages %d", i,
			      plane->pages->pages[0], buf->dir, page_off, page_count);
		if (plane->pages != NULL) {
...
int mvx_buffer_filled_set(struct mvx_buffer *buf, unsigned int plane, unsigned int filled, unsigned int offset)
{
	struct mvx_buffer_plane *p = &buf->planes[plane];
	...
	if (size < (filled + offset)) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING,
			      "Buffer plane too small. plane=%d, size=%zu, filled=%u, offset=%u.", plane, size, filled,
			      offset);
...
int mvx_buffer_frame_dim(enum mvx_format format, unsigned int width, unsigned int height, uint8_t *nplanes,
			 unsigned int *stride, unsigned int *size)
{
	...
	for (i = 0; i < *nplanes; i++) {
		...
		unsigned int tmp = DIV_ROUND_UP(width * s[i][0], SUBSAMPLE_PIXELS);
		...
		size[i] = DIV_ROUND_UP(height * s[i][1], SUBSAMPLE_PIXELS) * stride[i];
	}
	...
	if (*nplanes == 2 && (width % 2 != 0 || height % 2 != 0)) {
		...
		size[1] = DIV_ROUND_UP(height * s[1][1], SUBSAMPLE_PIXELS) * stride[1];
	}
...
int mvx_buffer_frame_set(struct mvx_buffer *buf, enum mvx_format format, unsigned int width, unsigned int height,
	...
	for (i = 0; i < buf->nplanes; i++) {
		...
		if ((plane->filled + plane->offset) > mvx_buffer_size(buf, i)) {
			MVX_LOG_PRINT(
				...
				"Buffer plane size is too small. plane=%i, size=%zu, size=%u, filled=%u, offset=%u", i,
				size[i], mvx_buffer_size(buf, i), plane->filled, plane->offset);
			return -ENOMEM;
		}
	}
...
int mvx_buffer_afbc_set(struct mvx_buffer *buf, enum mvx_format format, unsigned int width, unsigned int height,
			unsigned int afbc_width, unsigned int size, bool interlaced)
{
	int ret;
	...
	if (size > mvx_buffer_size(buf, 0)) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "AFBC buffer too small. buf_size=%zu, size=%u.", size,
			      mvx_buffer_size(buf, 0));
		return -ENOMEM;
	}
