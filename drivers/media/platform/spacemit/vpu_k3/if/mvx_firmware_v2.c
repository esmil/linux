 ...
 * permitted to do so under the terms of a subsisting license agreemen
 ...
 */
static bool is_afbc(unsigned int format) { return (format & (1 << MVE_FORMAT_BF_A)) != 0; }
 ...
 */
static void log_message(struct mvx_session *session, enum mvx_log_fwif_channel channel,
			enum mvx_log_fwif_direction direction, struct mve_msg_header *msg_header, void *data)
{
 ...
 * @rpc:		RPC message.
 */
static void log_rpc(struct mvx_session *session, enum mvx_log_fwif_direction direction,
		    struct mve_rpc_communication_area *rpc)
{
	...
	if (rpc_size > sizeof(*rpc))
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "RPC message size is too large. size=%u.", rpc->size);
...
static int get_stride90(enum mvx_format format, uint8_t *nplanes, unsigned int stride[MVX_BUFFER_NPLANES][2])
{
	switch (format) {
 ...
 */
static unsigned int read32n(volatile uint32_t *src, unsigned int offset, uint32_t *dst, size_t size)
{
 ...
 */
static int read_message(struct mvx_fw *fw, struct mve_comm_area_host *host, struct mve_comm_area_mve *mve,
			unsigned int *code, void *data, size_t *size, enum mvx_log_fwif_channel channel,
			enum mvx_fw_buffer_attr mve_buf_attr, enum mvx_fw_buffer_attr host_buf_attr)
{
	...
	if (mve_buf_attr == MVX_FW_BUF_CACHEABLE) {
		dma_sync_single_for_cpu(fw->dev, virt_to_phys(mve), MVE_PAGE_SIZE, DMA_FROM_DEVICE);
	}
	...
	/* Read the header. */
	rpos = read32n(mve->out_data, rpos, (uint32_t *)&header, sizeof(header));

	if (!((header.code >= 1001 && header.code <= 1013) || (header.code >= 2001 && header.code <= 2019) ||
	      (header.code >= 3001 && header.code <= 3004))) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING,
			      "read_message header. header.size=%d, rpos=%d, code=%d, capacity=%zu", header.size,
			      host->out_rpos, header.code, capacity);
		return 0;
	}

	/* Make sure there is enough space for both header and message. */
	capacity -= DIV_ROUND_UP(sizeof(header) + header.size, sizeof(uint32_t));
	if (capacity < 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING,
	...
	*size = header.size;
	...
	MVX_LOG_EXECUTE(&mvx_log_fwif_if, MVX_LOG_INFO,
			log_message(fw->session, channel, MVX_LOG_FWIF_DIRECTION_FIRMWARE_TO_HOST, &header, data));
 ...
 * Return: New offset in the circular buffer.
 */
static unsigned int write32n(volatile uint32_t *dst, unsigned int offset, uint32_t *src, size_t size)
{
	for (; size >= sizeof(uint32_t); size -= sizeof(uint32_t)) {
		dst[offset] = *src++;
		offset = (offset + 1) % MVE_COMM_QUEUE_SIZE_IN_WORDS;
 ...
 */
static int write_message(struct mvx_fw *fw, struct mve_comm_area_host *host, struct mve_comm_area_mve *mve,
			 unsigned int code, void *data, size_t size, enum mvx_log_fwif_channel channel,
			 enum mvx_fw_buffer_attr mve_buf_attr, enum mvx_fw_buffer_attr host_buf_attr)
{
	...
	/* Write header. */
	wpos = write32n(host->in_data, wpos, (uint32_t *)&header, sizeof(header));
	...
	if (host_buf_attr == MVX_FW_BUF_CACHEABLE) {
		wmb();
		dma_sync_single_for_device(fw->dev, virt_to_phys(host), MVE_PAGE_SIZE, DMA_TO_DEVICE);
	}
	...
	if (host_buf_attr == MVX_FW_BUF_CACHEABLE) {
		wmb();
		dma_sync_single_for_device(fw->dev, virt_to_phys(&host->in_wpos), sizeof(host->in_wpos), DMA_TO_DEVICE);
	}

	/* Log firmware message. */
	MVX_LOG_EXECUTE(&mvx_log_fwif_if, MVX_LOG_INFO,
			log_message(fw->session, channel, MVX_LOG_FWIF_DIRECTION_HOST_TO_FIRMWARE, &header, data));

	return 0;
}

static int get_region_v2(enum mvx_fw_region region, uint32_t *begin, uint32_t *end)
{
	switch (region) {
...
static void convert_buffer_general(struct mvx_fw *fw, enum mvx_direction dir, struct mvx_fw_msg *msg,
...
static void convert_buffer_frame(struct mvx_fw *fw, enum mvx_direction dir, struct mvx_fw_msg *msg,
...
static void convert_buffer_bitstream(struct mvx_fw *fw, enum mvx_direction dir, struct mvx_fw_msg *msg,
				     struct mve_buffer_bitstream *b)
{
	...
	if (dir == MVX_DIR_OUTPUT)
		mvx_buffer_filled_set(buf, 0, b->bitstream_filled_len, b->bitstream_offset);
...
static int convert_buffer_param(struct mvx_fw *fw, struct mvx_fw_msg *msg, struct mve_buffer_param *p)
{
	switch (p->type) {
	case MVE_BUFFER_PARAM_TYPE_COLOUR_DESCRIPTION: {
		struct mve_buffer_param_colour_description *c = &p->data.colour_description;
		...
		default:
			MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING,
				      "Unknown fw buffer param color desc range. range=%u.", c->range);
			return -EINVAL;
		}
		...
		if (c->flags & MVE_BUFFER_PARAM_COLOUR_FLAG_MASTERING_DISPLAY_DATA_VALID) {
			...
			d->display.luminance_min = c->min_display_mastering_luminance;
			d->display.luminance_max = c->max_display_mastering_luminance;
		}

		if (c->flags & MVE_BUFFER_PARAM_COLOUR_FLAG_CONTENT_LIGHT_DATA_VALID) {
			d->flags |= MVX_FW_COLOR_DESC_CONTENT_VALID;

			d->content.luminance_max = c->max_content_light_level;
			d->content.luminance_average = c->avg_content_light_level;
		}
	...
	default:
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO, "Default buffer param. type=%d", p->type);
		break;
	}
...
static int get_buffer(struct mvx_fw *fw, struct mve_comm_area_host *host, struct mve_comm_area_mve *mve,
		      enum mvx_direction dir, struct mvx_fw_msg *msg, enum mvx_log_fwif_channel channel,
		      enum mvx_fw_buffer_attr mve_buf_attr, enum mvx_fw_buffer_attr host_buf_attr)
{
	unsigned int code;
	union {
	...
	struct mvx_v4l2_session *vsession = container_of(session, struct mvx_v4l2_session, session);
	...
	case MVE_BUFFER_CODE_GENERAL:
		...
		break;
	default:
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Unknown fw buffer code. code=%u.", code);
		break;
	}
...
static int get_message_v2(struct mvx_fw *fw, struct mvx_fw_msg *msg)
{
	...
	case MVE_RESPONSE_CODE_SET_OPTION_FAIL: {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Firmware set option failed. index=%u, msg=%s.",
			      fw_msg.set_option_fail.index, fw_msg.set_option_fail.message);
		msg->code = MVX_FW_CODE_SET_OPTION;
		fw->msg_pending--;
		break;
	}
	...
	case MVE_RESPONSE_CODE_INPUT:
		ret = get_buffer(fw, fw->buf_in_host, fw->buf_in_mve, MVX_DIR_INPUT, msg,
				 MVX_LOG_FWIF_CHANNEL_INPUT_BUFFER, fw->buf_attr[MVX_FW_REGION_BUF_IN_MVE],
				 fw->buf_attr[MVX_FW_REGION_BUF_IN_HOST]);
		break;
	case MVE_RESPONSE_CODE_OUTPUT:
		ret = get_buffer(fw, fw->buf_out_host, fw->buf_out_mve, MVX_DIR_OUTPUT, msg,
				 MVX_LOG_FWIF_CHANNEL_OUTPUT_BUFFER, fw->buf_attr[MVX_FW_REGION_BUF_OUT_MVE],
				 fw->buf_attr[MVX_FW_REGION_BUF_OUT_HOST]);
		break;
	case MVE_BUFFER_CODE_PARAM:
	...
	case MVE_RESPONSE_CODE_ERROR: {
		...
		case MVE_ERROR_WATCHDOG:
			...
			break;
		default:
			MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Unsupported fw error code. code=%u.",
				      fw_msg.error.error_code);
			break;
		}
#if (KERNEL_VERSION(6, 7, 0) <= LINUX_VERSION_CODE)
		strncpy(msg->error.message, fw_msg.error.message,
			min(sizeof(msg->error.message), sizeof(fw_msg.error.message)));
#else
	...
	case MVE_RESPONSE_CODE_FRAME_ALLOC_PARAM:
		...
		msg->alloc_param.width = fw_msg.alloc_param.planar_alloc_frame_width;
		msg->alloc_param.height = fw_msg.alloc_param.planar_alloc_frame_height;
		msg->alloc_param.afbc_alloc_bytes = fw_msg.alloc_param.afbc_alloc_bytes;
		msg->alloc_param.afbc_width = fw_msg.alloc_param.afbc_width_in_superblocks;

		break;
	case MVE_RESPONSE_CODE_SEQUENCE_PARAMETERS:
		msg->code = MVX_FW_CODE_SEQ_PARAM;
		msg->seq_param.planar.buffers_min = fw_msg.seq_param.num_buffers_planar;
		msg->seq_param.afbc.buffers_min = fw_msg.seq_param.num_buffers_afbc;
	...
	case MVE_RESPONSE_CODE_EVENT:
		if (MVE_EVENT_ERROR_STREAM_CORRUPT == fw_msg.event.event_code) {
			...
			vfree(err_msg_sep);
			if (!is_find) {
				MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_ERROR, "EVENT code=%d %s.but not found UDT ret:%d",
					      fw_msg.event.event_code, fw_msg.event.event_data.message, ret);
			} else {
		...
		if (MVE_EVENT_ERROR_STREAM_NOT_SUPPORTED == fw_msg.event.event_code) {
			MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "event STREAM_NOT_SUPPORTED. code=%d. %s.",
		...
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO, "EVENT code=%d. %s", fw_msg.event.event_code,
			      (MVE_EVENT_ERROR_STREAM_CORRUPT == fw_msg.event.event_code ||
			       MVE_EVENT_ERROR_STREAM_NOT_SUPPORTED == fw_msg.event.event_code)
	...
	case MVE_RESPONSE_CODE_REF_FRAME_UNUSED:
		break;
	default:
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Unknown fw message code. code=%u, size=%u.", code, size);
...
static int put_buffer_general(struct mvx_fw *fw, struct mve_comm_area_host *host, struct mve_comm_area_mve *mve,
			      ...
			      enum mvx_fw_buffer_attr mve_buf_attr, enum mvx_fw_buffer_attr host_buf_attr)
{
	...
	ret = write_message(fw, host, mve, MVE_BUFFER_CODE_GENERAL, &g, sizeof(g), channel, mve_buf_attr,
			    host_buf_attr);

	return ret;
}

static int put_buffer_frame(struct mvx_fw *fw, struct mve_comm_area_host *host, struct mve_comm_area_mve *mve,
			    struct mvx_fw_msg *msg, enum mvx_log_fwif_channel channel,
			    enum mvx_fw_buffer_attr mve_buf_attr, enum mvx_fw_buffer_attr host_buf_attr)
{
	...
	case MVX_FORMAT_ABGR_8888:
		...
		break;
	default:
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Unsupported frame format. format=%u.", buf->format);
		return -EINVAL;
	}
	...
	if (is_afbc(f.format) == false) {
		...
		for (i = 0; i < buf->nplanes; i++) {
			...
			if (f.frame_flags & MVE_BUFFER_FRAME_FLAG_INTERLACE) {
				...
				planar->plane_bot[i] = planar->plane_top[i] + (round_up(stride, 2) >> stride_shift);
				if (buf->dir == MVX_DIR_OUTPUT && (rotation == 1 || rotation == 3)) {
		...
		if (f.frame_flags & MVE_BUFFER_FRAME_FLAG_INTERLACE) {
			afbc->alloc_bytes[0] = ALIGN((buf->planes[0].filled / 2), 32);
			afbc->alloc_bytes[1] = buf->planes[0].filled - afbc->alloc_bytes[0];
			afbc->plane[1] = afbc->plane[0] + afbc->alloc_bytes[0];
			afbc->afbc_width_in_superblocks[1] = afbc->afbc_width_in_superblocks[0];
		} else {
		...
		if (buf->flags & MVX_BUFFER_AFBC_32X8_SUPERBLOCK)
			afbc->afbc_params |= MVE_BUFFER_FRAME_AFBC_32X8_SUPERBLOCK;
		if (buf->flags & MVX_BUFFER_AFBC_BLOCK_SPLIT)
			afbc->afbc_params |= MVE_BUFFER_FRAME_AFBC_BLOCK_SPLIT;
	}

	ret = write_message(fw, host, mve, MVE_BUFFER_CODE_FRAME, &f, sizeof(f), channel, mve_buf_attr, host_buf_attr);

	return ret;
}

static int put_buffer_bitstream(struct mvx_fw *fw, struct mve_comm_area_host *host, struct mve_comm_area_mve *mve,
				struct mvx_fw_msg *msg, enum mvx_log_fwif_channel channel,
				enum mvx_fw_buffer_attr mve_buf_attr, enum mvx_fw_buffer_attr host_buf_attr)
{
	...
	ret = write_message(fw, host, mve, MVE_BUFFER_CODE_BITSTREAM, &b, sizeof(b), channel, mve_buf_attr,
			    host_buf_attr);
...
static int to_mve_h264_cabac(enum mvx_entropy_mode entropy_mode, unsigned int *mve_arg)
{
	switch (entropy_mode) {
...
static int to_mve_vp9_prob_update(enum mvx_vp9_prob_update prob_update, unsigned int *mve_arg)
{
	switch (prob_update) {
...
static int to_mve_rgb_to_yuv(enum mvx_rgb_to_yuv_mode mode, unsigned int *mve_arg)
{
	switch (mode) {
...
static int put_fw_opt(struct mvx_fw *fw, struct mve_request_set_option *opt, size_t size)
{
	int ret;

	ret = write_message(fw, fw->msg_host, fw->msg_mve, MVE_REQUEST_CODE_SET_OPTION, opt,
			    offsetof(typeof(*opt), data) + size, MVX_LOG_FWIF_CHANNEL_MESSAGE,
			    fw->buf_attr[MVX_FW_REGION_MSG_MVE], fw->buf_attr[MVX_FW_REGION_MSG_HOST]);
...
static int put_fw_buf_param(struct mvx_fw *fw, struct mve_buffer_param *param, size_t size)
{
	return write_message(fw, fw->buf_in_host, fw->buf_in_mve, MVE_BUFFER_CODE_PARAM, param,
			     offsetof(typeof(*param), data) + size, MVX_LOG_FWIF_CHANNEL_MESSAGE,
			     fw->buf_attr[MVX_FW_REGION_BUF_IN_MVE], fw->buf_attr[MVX_FW_REGION_BUF_IN_HOST]);
}
...
static int put_message_v2(struct mvx_fw *fw, struct mvx_fw_msg *msg)
{
	...
	case MVX_FW_CODE_STATE_CHANGE: {
		unsigned int code = msg->state == MVX_FW_STATE_STOPPED ? MVE_REQUEST_CODE_STOP : MVE_REQUEST_CODE_GO;

		ret = write_message(fw, fw->msg_host, fw->msg_mve, code, NULL, 0, MVX_LOG_FWIF_CHANNEL_MESSAGE,
				    fw->buf_attr[MVX_FW_REGION_MSG_MVE], fw->buf_attr[MVX_FW_REGION_MSG_HOST]);
		if (ret == 0)
			fw->msg_pending++;
	...
	case MVX_FW_CODE_JOB: {
		...
		ret = write_message(fw, fw->msg_host, fw->msg_mve, MVE_REQUEST_CODE_JOB, &job, sizeof(job),
				    MVX_LOG_FWIF_CHANNEL_MESSAGE, fw->buf_attr[MVX_FW_REGION_MSG_MVE],
				    fw->buf_attr[MVX_FW_REGION_MSG_HOST]);
		break;
	}
	case MVX_FW_CODE_SWITCH_OUT: {
		ret = write_message(fw, fw->msg_host, fw->msg_mve, MVE_REQUEST_CODE_SWITCH, NULL, 0,
				    MVX_LOG_FWIF_CHANNEL_MESSAGE, fw->buf_attr[MVX_FW_REGION_MSG_MVE],
				    fw->buf_attr[MVX_FW_REGION_MSG_HOST]);
		break;
	}
	case MVX_FW_CODE_PING: {
		ret = write_message(fw, fw->msg_host, fw->msg_mve, MVE_REQUEST_CODE_PING, NULL, 0,
				    MVX_LOG_FWIF_CHANNEL_MESSAGE, fw->buf_attr[MVX_FW_REGION_MSG_MVE],
				    fw->buf_attr[MVX_FW_REGION_MSG_HOST]);
		break;
	}
	case MVX_FW_CODE_SET_OPTION: {
		...
		case MVX_FW_SET_TARGET_BITRATE: {
			...
			if (msg->set_option.target_bitrate == 0) {
				param.data.rate_control.rate_control_mode = MVE_OPT_RATE_CONTROL_MODE_OFF;
				param.data.rate_control.target_bitrate = 0;
			} else {
				param.data.rate_control.rate_control_mode = MVE_OPT_RATE_CONTROL_MODE_STANDARD;
				param.data.rate_control.target_bitrate = msg->set_option.target_bitrate;
			}
		...
		case MVX_FW_SET_RATE_CONTROL: {
			...
			if (msg->set_option.rate_control.target_bitrate == 0) {
				...
				param.data.rate_control.target_bitrate = msg->set_option.rate_control.target_bitrate;
		...
		case MVX_FW_SET_COLOUR_DESC: {
			...
			if (msg->set_option.colour_desc.time_scale != 0 ||
			    msg->set_option.colour_desc.num_units_in_tick != 0) {
			...
			ret = put_fw_buf_param(fw, &param, sizeof(param.data.colour_description));
			break;
		}
		case MVX_FW_SET_SEI_USERDATA: {
			...
			ret = put_fw_buf_param(fw, &param, sizeof(param.data.user_data_unregistered));
			break;
		}
		case MVX_FW_SET_NALU_FORMAT: {
			...
			ret = to_mve_nalu_format(msg->set_option.nalu_format, &opt.data.arg);
		...
		case MVX_FW_SET_PROFILE_LEVEL: {
			...
			ret = fw->ops_priv.to_mve_profile(msg->set_option.profile_level.profile,
							  &opt.data.profile_level.profile);
			if (ret != 0)
				return ret;

			ret = fw->ops_priv.to_mve_level(msg->set_option.profile_level.level,
							&opt.data.profile_level.level);
			if (ret != 0)
				return ret;

			ret = put_fw_opt(fw, &opt, sizeof(opt.data.profile_level));
		...
		case MVX_FW_SET_IGNORE_STREAM_HEADERS: {
			...
			opt.data.arg = msg->set_option.ignore_stream_headers ? 1 : 0;
			ret = put_fw_opt(fw, &opt, sizeof(opt.data.arg));
			break;
		}
		...
		case MVX_FW_SET_GOP_TYPE: {
			...
			ret = to_mve_gop_type(msg->set_option.gop_type, &opt.data.arg);
			if (ret == 0)
				ret = put_fw_opt(fw, &opt, sizeof(opt.data.arg));
		...
		case MVX_FW_SET_QP_RANGE: {
			...
			param.type = MVE_BUFFER_PARAM_TYPE_RATE_CONTROL_QP_RANGE;
			param.data.rate_control_qp_range.qp_min = msg->set_option.qp_range.min;
			param.data.rate_control_qp_range.qp_max = msg->set_option.qp_range.max;
			ret = put_fw_buf_param(fw, &param, sizeof(param.data.rate_control_qp_range));
			break;
		}
		case MVX_FW_SET_ENTROPY_MODE: {
			...
			ret = to_mve_h264_cabac(msg->set_option.entropy_mode, &opt.data.arg);
			if (ret == 0)
				ret = put_fw_opt(fw, &opt, sizeof(opt.data.arg));
		...
		case MVX_FW_SET_VP9_PROB_UPDATE: {
			...
			ret = to_mve_vp9_prob_update(msg->set_option.vp9_prob_update, &opt.data.arg);
			if (ret == 0)
				ret = put_fw_opt(fw, &opt, sizeof(opt.data.arg));
		...
		case MVX_FW_SET_MV_SEARCH_RANGE: {
			...
			opt.data.motion_vector_search_range.mv_search_range_x = msg->set_option.mv.x;
			opt.data.motion_vector_search_range.mv_search_range_y = msg->set_option.mv.y;
			ret = put_fw_opt(fw, &opt, sizeof(opt.data.motion_vector_search_range));
			break;
		}
		case MVX_FW_SET_BITDEPTH: {
			...
			opt.data.bitdepth.luma_bitdepth = msg->set_option.bitdepth.luma;
			opt.data.bitdepth.chroma_bitdepth = msg->set_option.bitdepth.chroma;
		...
		case MVX_FW_SET_RGB_TO_YUV_MODE: {
			...
			ret = to_mve_rgb_to_yuv(msg->set_option.rgb_to_yuv_mode, &opt.data.arg);
			if (ret == 0)
				ret = put_fw_opt(fw, &opt, sizeof(opt.data.arg));
		...
		case MVX_FW_SET_QP_I: {
			...
			ret = put_fw_buf_param(fw, &param, sizeof(param.data.qp));
			break;
		}
		...
		case MVX_FW_SET_QUANT_TABLE: {
			...
			memcpy(opt.data.quant_table.matrix, msg->set_option.quant_tbl.luma,
			       sizeof(opt.data.quant_table.matrix));
			ret = put_fw_opt(fw, &opt, sizeof(opt.data.quant_table));
			if (ret != 0)
				break;

			opt.data.quant_table.type = MVE_OPT_QUANT_TABLE_CHROMA;
			memcpy(opt.data.quant_table.matrix, msg->set_option.quant_tbl.chroma,
			       sizeof(opt.data.quant_table.matrix));
			ret = put_fw_opt(fw, &opt, sizeof(opt.data.quant_table));
			break;
		}
		...
		case MVX_FW_SET_ROI_REGIONS: {
			...
			ret = put_fw_buf_param(fw, &param, sizeof(param.data.regions));
			break;
		}
	...
	case MVX_FW_CODE_FLUSH: {
		...
		case MVX_DIR_INPUT:
			ret = write_message(fw, fw->msg_host, fw->msg_mve, MVE_REQUEST_CODE_INPUT_FLUSH, NULL, 0,
					    MVX_LOG_FWIF_CHANNEL_MESSAGE, fw->buf_attr[MVX_FW_REGION_MSG_MVE],
					    fw->buf_attr[MVX_FW_REGION_MSG_HOST]);
			break;
		case MVX_DIR_OUTPUT:
			ret = write_message(fw, fw->msg_host, fw->msg_mve, MVE_REQUEST_CODE_OUTPUT_FLUSH, NULL, 0,
					    MVX_LOG_FWIF_CHANNEL_MESSAGE, fw->buf_attr[MVX_FW_REGION_MSG_MVE],
					    fw->buf_attr[MVX_FW_REGION_MSG_HOST]);
			break;
		default:
			MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Invalid flush direction. dir=%d.", msg->flush.dir);
			return -EINVAL;
		}
	...
	case MVX_FW_CODE_EOS: {
		...
		if (msg->eos_is_frame != false) {
			...
			ret = write_message(fw, host, mve, MVE_BUFFER_CODE_FRAME, &f, sizeof(f), channel,
					    fw->buf_attr[MVX_FW_REGION_BUF_IN_MVE],
					    fw->buf_attr[MVX_FW_REGION_BUF_IN_HOST]);
		} else {
			struct mve_buffer_bitstream b = {
				.host_handle        = MVX_FW_CODE_EOS,
				.bitstream_buf_addr = MVE_MEM_REGION_PROTECTED_ADDR_BEGIN,
				.bitstream_flags    = MVE_BUFFER_BITSTREAM_FLAG_EOS
			};
			ret = write_message(fw, host, mve, MVE_BUFFER_CODE_BITSTREAM, &b, sizeof(b), channel,
					    fw->buf_attr[MVX_FW_REGION_BUF_IN_MVE],
					    fw->buf_attr[MVX_FW_REGION_BUF_IN_HOST]);
		}
	...
	case MVX_FW_CODE_DUMP: {
		ret = write_message(fw, fw->msg_host, fw->msg_mve, MVE_REQUEST_CODE_DUMP, NULL, 0,
				    MVX_LOG_FWIF_CHANNEL_MESSAGE, fw->buf_attr[MVX_FW_REGION_MSG_MVE],
				    fw->buf_attr[MVX_FW_REGION_MSG_HOST]);
		fw->msg_pending++;
		break;
	...
	case MVX_FW_CODE_DEBUG: {
		ret = write_message(fw, fw->msg_host, fw->msg_mve, MVE_REQUEST_CODE_DEBUG, &msg->arg, sizeof(msg->arg),
				    MVX_LOG_FWIF_CHANNEL_MESSAGE, fw->buf_attr[MVX_FW_REGION_MSG_MVE],
				    fw->buf_attr[MVX_FW_REGION_MSG_HOST]);
		fw->msg_pending++;
		break;
	...
	if (ret != 0)
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Firmware put message failed. ret=%d.", ret);
...
static void rpc_mem_alloc(struct mvx_fw *fw, struct mve_rpc_communication_area *rpc_area)
{
	...
	if (IS_ENABLED(CONFIG_DEBUG_FS)) {
		...
		if (ret != 0) {
			MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_ERROR, "Cannot protect RPC alloc list.");
			goto out;
		}
	}
	...
	case MVE_MEM_REGION_OUTBUF:
		...
		break;
	default:
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Unsupported RPC mem alloc region. region=%u.",
			      p->mem_alloc.region);
		goto unlock_mutex;
	}
	...
	if (fw->fw_bin->securevideo != false) {
		...
		dmabuf = mvx_secure_mem_alloc(fw->fw_bin->secure.secure, p->mem_alloc.size);
		if (IS_ERR(dmabuf))
			goto unlock_mutex;
	...
	while (va < end) {
		...
		ret = mvx_mmu_map_pages(fw->mmu, va, pages, MVX_ATTR_SHARED_RW, MVX_ACCESS_READ_WRITE);
		if (ret == 0) {
	...
	if (ret != 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Failed to find memory region for RPC alloc.");
	...
	MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO,
		      "RPC alloc memory. size=%u, max_size=%u, region=%u, npages=%zu, va=0x%x.", p->mem_alloc.size,
		      p->mem_alloc.max_size, p->mem_alloc.region, npages, va);
...
static void rpc_mem_resize(struct mvx_fw *fw, struct mve_rpc_communication_area *rpc_area)
{
	union mve_rpc_params *p = &rpc_area->params;
	struct mvx_mmu_pages *pages;
	mvx_mmu_va va = 0;
	...
	if (pages != 0) {
		...
		if (fw->fw_bin->securevideo != false) {
			...
			if (size < p->mem_resize.new_size) {
				...
				dmabuf = mvx_secure_mem_alloc(fw->fw_bin->secure.secure, size);
				if (IS_ERR(dmabuf))
					goto unlock_mutex;

				ret = mvx_mmu_pages_append_dma_buf(pages, dmabuf);
				if (ret != 0) {
					dma_buf_put(dmabuf);
					goto unlock_mutex;
				}
			...
			npages = DIV_ROUND_UP(p->mem_resize.new_size, MVE_PAGE_SIZE);
			ret = mvx_mmu_resize_pages(pages, npages);
			if (ret != 0) {
				MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING,
					      "Failed to resize RPC mapped pages. ret=%d.", ret);
				goto unlock_mutex;
			}
		}
		...
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Could not find pages for RPC resize. va=0x%x.",
			      p->mem_resize.ve_pointer);
	}

	fw->client_ops->flush_mmu(fw->csession);

	MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO, "RPC resize memory. va=0x%x, new_size=%u.", p->mem_resize.ve_pointer,
		      p->mem_resize.new_size);
...
static void rpc_mem_free(struct mvx_fw *fw, struct mve_rpc_communication_area *rpc_area)
{
	union mve_rpc_params *p = &rpc_area->params;
	struct mvx_mmu_pages *pages;
	int ret;
	...
	if (pages != NULL) {
		hash_del(&pages->node);
		if (MVE_MEM_REGION_PROTECTED_ADDR_BEGIN <= p->mem_free.ve_pointer &&
		    p->mem_free.ve_pointer < MVE_MEM_REGION_PROTECTED_ADDR_END) {
			fw->latest_used_region_protected_pages =
				fw->latest_used_region_protected_pages > pages->capacity
					? fw->latest_used_region_protected_pages - pages->capacity
					: 0;
		} else if (MVE_MEM_REGION_FRAMEBUF_ADDR_BEGIN <= p->mem_free.ve_pointer &&
			   p->mem_free.ve_pointer < MVE_MEM_REGION_FRAMEBUF_ADDR_END) {
			fw->latest_used_region_outbuf_pages =
				fw->latest_used_region_outbuf_pages > pages->capacity
					? fw->latest_used_region_outbuf_pages - pages->capacity
					: 0;
		}
		mvx_mmu_free_pages(pages);
	} else {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Could not find pages for RPC free. va=0x%x.",
			      p->mem_free.ve_pointer);
	}

	fw->client_ops->flush_mmu(fw->csession);

	MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO, "RPC free memory. va=0x%x.", p->mem_free.ve_pointer);
...
static int handle_rpc_v2(struct mvx_fw *fw)
{
	...
	if (fw->buf_attr[MVX_FW_REGION_RPC] == MVX_FW_BUF_CACHEABLE) {
		dma_sync_single_for_cpu(fw->dev, virt_to_phys(rpc_area), sizeof(*rpc_area), DMA_FROM_DEVICE);
	}
	...
	if (rpc_area->state == MVE_RPC_STATE_PARAM) {
		...
		/* Log RPC request. */
		MVX_LOG_EXECUTE(&mvx_log_fwif_if, MVX_LOG_INFO,
				log_rpc(fw->session, MVX_LOG_FWIF_DIRECTION_FIRMWARE_TO_HOST, rpc_area));

		switch (rpc_area->call_id) {
		case MVE_RPC_FUNCTION_DEBUG_PRINTF: {
			MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO, "RPC_PRINT=%s",
		...
		default:
			MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO, "Unsupported RPC request. call_id=%u.",
				      rpc_area->call_id);
			ret = -EINVAL;
			break;
		}
		...
		if (fw->buf_attr[MVX_FW_REGION_RPC] == MVX_FW_BUF_CACHEABLE) {
			wmb();
			dma_sync_single_for_device(fw->dev, virt_to_phys(rpc_area), sizeof(*rpc_area), DMA_TO_DEVICE);
		}

		/* Log RPC response. */
		MVX_LOG_EXECUTE(&mvx_log_fwif_if, MVX_LOG_INFO,
				log_rpc(fw->session, MVX_LOG_FWIF_DIRECTION_HOST_TO_FIRMWARE, rpc_area));
...
static int handle_fw_ram_print_v2(struct mvx_fw *fw)
{
	...
	dma_sync_single_for_cpu(fw->dev, virt_to_phys(rpt_area), sizeof(*rpt_area), DMA_FROM_DEVICE);
	...
	if (RAM_PRINT_FLAG == rpt_area->flag && RAM_PRINT_BUF_CNT > rpt_area->index && cnt) {
		...
		wmb();
		dma_sync_single_for_device(fw->dev, virt_to_phys(&rpt_area->rd_cnt), sizeof(rpt_area->rd_cnt),
					   DMA_TO_DEVICE);
...
static int map_msq(struct mvx_fw *fw, void **data, enum mvx_fw_region region)
{
	...
	/* Memory map region. */
	ret = mvx_mmu_map_pa(fw->mmu, begin, page, MVE_PAGE_SIZE, MVX_ATTR_SHARED_RW, MVX_ACCESS_READ_WRITE);
	if (ret != 0) {
...
static int map_msq_uncache(struct mvx_fw *fw, void **data, enum mvx_fw_region region)
{
	phys_addr_t page;
	mvx_mmu_va begin;
	mvx_mmu_va end;
...
static void unmap_msq_uncache(struct mvx_fw *fw, void **data, enum mvx_fw_region region)
{
	int ret;
	mvx_mmu_va begin;
	mvx_mmu_va end;
...
#ifdef MVX_FW_DEBUG_ENABLE
static void unmap_fw_print_ram(struct mvx_fw *fw, void **data, enum mvx_fw_region region)
{
	int ret;
	mvx_mmu_va begin;
	mvx_mmu_va end;
...
static int map_fw_print_ram(struct mvx_fw *fw, void **data, enum mvx_fw_region region)
{
	phys_addr_t page;
	mvx_mmu_va begin;
	mvx_mmu_va end;
	...
	/* Memory map region. */
	ret = mvx_mmu_map_pa(fw->mmu, begin, page, MVE_FW_PRINT_RAM_SIZE, MVX_ATTR_SHARED_RW, MVX_ACCESS_READ_WRITE);
	if (ret != 0) {
...
static void print_pair(char *name_in, char *name_out, struct device *device, struct mve_comm_area_host *host,
		       struct mve_comm_area_mve *mve, int ind, struct seq_file *s, enum mvx_fw_buffer_attr mve_buf_attr)
{
	if (mve_buf_attr == MVX_FW_BUF_CACHEABLE) {
		dma_sync_single_for_cpu(device, virt_to_phys(mve), MVE_PAGE_SIZE, DMA_FROM_DEVICE);
	}
...
{
	print_pair("Msg host->mve", "Msg host<-mve", fw->dev, fw->msg_host, fw->msg_mve, ind, s,
		   fw->buf_attr[MVX_FW_REGION_MSG_MVE]);
	print_pair("Inbuf host->mve", "Inbuf host<-mve", fw->dev, fw->buf_in_host, fw->buf_in_mve, ind, s,
		   fw->buf_attr[MVX_FW_REGION_BUF_IN_MVE]);
	print_pair("Outbuf host->mve", "Outbuf host<-mve", fw->dev, fw->buf_out_host, fw->buf_out_mve, ind, s,
		   fw->buf_attr[MVX_FW_REGION_BUF_OUT_MVE]);
...
static void print_debug_v2(struct mvx_fw *fw)
{
	...
	if (fw->buf_attr[MVX_FW_REGION_MSG_MVE] == MVX_FW_BUF_CACHEABLE) {
		dma_sync_single_for_cpu(fw->dev, virt_to_phys(msg_mve), MVE_PAGE_SIZE, DMA_FROM_DEVICE);
	}
	...
	MVX_LOG_PRINT_SESSION(&mvx_log_session_if, MVX_LOG_WARNING, fw->session,
			      ...
			      msg_host->out_rpos, msg_host->in_wpos, msg_mve->out_wpos, msg_mve->in_rpos);

	MVX_LOG_PRINT_SESSION(&mvx_log_session_if, MVX_LOG_WARNING, fw->session,
			      "Dump fw rpc. state=%u, call_id=%u, size=%u, data=%u, debug_print=%s", rpc_area->state,
			      rpc_area->call_id, rpc_area->size, p->data, p->debug_print.string);
	...
	while ((capacity = get_capacity(rpos, wpos)) >= sizeof(header)) {
		...
		pos = read32n(msg_mve->out_data, rpos, (uint32_t *)&header, sizeof(header));

		MVX_LOG_PRINT_SESSION(&mvx_log_session_if, MVX_LOG_WARNING, fw->session,
				      "queue={rpos=%u, wpos=%u, capacity=%u}, msg={code=%u, size=%u}", rpos, wpos,
				      capacity, header.code, header.size);

		capacity = get_capacity(pos, wpos);
		if (header.size > capacity) {
			MVX_LOG_PRINT_SESSION(&mvx_log_session_if, MVX_LOG_WARNING, fw->session,
					      "Size is larger than capacity. capacity=%zd, size=%u.", capacity,
					      header.size);
			return;
		}

		rpos = (pos + DIV_ROUND_UP(header.size, sizeof(uint32_t))) % MVE_COMM_QUEUE_SIZE_IN_WORDS;
	}
}
...
int mvx_fw_send_idle_ack_v2(struct mvx_fw *fw)
{
	...
	ret = write_message(fw, fw->msg_host, fw->msg_mve, MVE_REQUEST_CODE_IDLE_ACK, NULL, 0,
			    MVX_LOG_FWIF_CHANNEL_MESSAGE, fw->buf_attr[MVX_FW_REGION_MSG_MVE],
			    fw->buf_attr[MVX_FW_REGION_MSG_HOST]);
...
int mvx_fw_construct_v2(struct mvx_fw *fw, struct mvx_fw_bin *fw_bin, struct mvx_mmu *mmu, struct mvx_session *session,
			struct mvx_client_ops *client_ops, struct mvx_client_session *csession, unsigned int ncores,
			unsigned char major, unsigned char minor)
{
	int ret;

	ret = mvx_fw_construct(fw, fw_bin, mmu, session, client_ops, csession, ncores);
	if (ret != 0)
		return ret;
