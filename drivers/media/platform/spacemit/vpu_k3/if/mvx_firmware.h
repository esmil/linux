...
struct mvx_fw {
	...
	struct {
		 ...
		 */
		int (*get_region)(enum mvx_fw_region region, uint32_t *begin, uint32_t *end);
		 ...
		 * Return: 0 on success, else error code.
		 */
		int (*print_stat)(struct mvx_fw *fw, int ind, struct seq_file *s);
	...
	struct {
		 ...
		 * Return: 0 on success, else error code.
		 */
		int (*to_mve_profile)(unsigned int mvx_profile, uint16_t *mve_profile);
		 ...
		 * Return: 0 on success, else error code.
		 */
		int (*to_mve_level)(unsigned int mvx_level, uint16_t *mve_level);
	} ops_priv;
};
 ...
 * Return: 0 on success, else error code.
 */
int mvx_fw_factory(struct mvx_fw *fw, struct mvx_fw_bin *fw_bin, struct mvx_mmu *mmu, struct mvx_session *session,
