...
{
	return (va >> (MVE_PAGE_SHIFT + (MVE_PTW_LEVELS - level - 1) * MVE_INDEX_SHIFT)) & MVE_INDEX_MASK;
}
 ...
 * +-------------------+-------------------+-----------------------+
 */
static unsigned int get_offset(const mvx_mmu_va va) { return va & MVE_PAGE_MASK; }
...
{
	return (((phys_addr_t)pte >> MVE_PTE_PHYSADDR_SHIFT) & MVE_PTE_PHYSADDR_MASK) << MVE_PAGE_SHIFT;
}
 ...
 */
static enum mvx_mmu_attr get_attr(const mvx_mmu_pte pte) { return (pte >> MVE_PTE_ATTR_SHIFT) & MVE_PTE_ATTR_MASK; }

/**
 * get_ap() - Return access permissions stored in PTE.
 */
static enum mvx_mmu_access get_ap(const mvx_mmu_pte pte) { return (pte >> MVE_PTE_AP_SHIFT) & MVE_PTE_AP_MASK; }
...
static mvx_mmu_pte *ptw(struct mvx_mmu *mmu, mvx_mmu_va va, bool alloc)
{
	...
	if (test_bit(index, mmu->l2_page_is_external) != 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "PTW virtual address to secure L2 page. va=0x%x.", va);
	...
	if (l2 == 0) {
		if (alloc == false) {
			MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Missing L2 page in PTW. va=0x%x.", va);
			return ERR_PTR(-EFAULT);
		}

		l2 = mvx_mmu_alloc_page(mmu->dev);
		if (l2 == 0) {
			MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Failed to allocate L2 page. va=0x%x.", va);
			return ERR_PTR(-ENOMEM);
		}

		pte[index] = mvx_mmu_set_pte(MVX_ATTR_PRIVATE, l2, MVX_ACCESS_READ_ONLY);
		dma_sync_single_for_device(mmu->dev, virt_to_phys(&pte[index]), sizeof(pte[index]), DMA_TO_DEVICE);
	}
 ...
 */
static int map_page(struct mvx_mmu *mmu, mvx_mmu_va va, phys_addr_t pa, enum mvx_mmu_attr attr,
		    enum mvx_mmu_access access)
{
	...
	if ((va | pa) & MVE_PAGE_MASK) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "VA and PA must be page aligned. va=0x%x, pa=0x%llx.", va,
			      pa);
		return -EFAULT;
	}
	...
	if (va & ~MVE_VA_MASK) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "VA out of valid range. va=0x%x.", va);
		return -EFAULT;
	}

	/* Check that PA is within valid address range. */
	if (pa & ~MVE_PA_MASK) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "PA out of valid range. pa=0x%llx.", pa);
		return -EFAULT;
	}
	...
	*pte = mvx_mmu_set_pte(attr, pa, access);
	dma_sync_single_for_device(mmu->dev, virt_to_phys(pte), sizeof(*pte), DMA_TO_DEVICE);
 ...
 * Return: 0 on success, else error code.
 */
static int remap_page(struct mvx_mmu *mmu, mvx_mmu_va va, phys_addr_t pa, enum mvx_mmu_attr attr,
		      enum mvx_mmu_access access)
{
	unmap_page(mmu, va);
...
static int remap_pages(struct mvx_mmu_pages *pages, size_t oldcount)
{
	...
	while (pages->count < oldcount) {
		...
		ret = remap_page(pages->mmu, pages->va + oldcount * MVE_PAGE_SIZE, MVE_PAGE_SIZE, MVX_ATTR_PRIVATE,
				 MVX_ACCESS_NO);
		if (ret != 0)
			return ret;
	}
	...
	while (pages->count > oldcount) {
		ret = remap_page(pages->mmu, pages->va + oldcount * MVE_PAGE_SIZE, pages->pages[oldcount], pages->attr,
				 pages->access);
		if (ret != 0)
			return ret;
...
static size_t get_sg_table_npages(struct sg_table *sgt)
{
	...
	for_each_sg_page(sgt->sgl, &piter, sgt->nents, 0) { count++; }
 ...
 * return: 0 on success, else error code.
 */
static int stat_open(struct inode *inode, struct file *file) { return single_open(file, stat_show, inode->i_private); }
...
static void *pages_seq_start(struct seq_file *s, loff_t *pos)
{
	...
	seq_puts(s, "#Page: [  va_start -     va_end] -> [          pa_start -             pa_end]\n");
	return pos;
}
...
phys_addr_t mvx_mmu_alloc_page(struct device *dev)
{
	...
	if (dma_mapping_error(dev, dma_handle) != 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Cannot map page to DMA address space. page=%p.", page);
		goto free_page;
	}
...
phys_addr_t mvx_mmu_dma_alloc_coherent(struct device *dev, void **data)
{
	...
	do {
		virt_addr =
			dma_alloc_coherent(dev, PAGE_SIZE, &dma_handle, GFP_KERNEL | __GFP_ZERO | __GFP_RETRY_MAYFAIL);
	} while (retry-- && virt_addr == NULL);

	if (virt_addr == NULL) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "dma alloc coherent buffer faild. retry=%d", retry);
		return 0;
	}
...
struct mvx_mmu_pages *mvx_mmu_alloc_pages(struct device *dev, size_t count, size_t capacity)
{
...
struct mvx_mmu_pages *mvx_mmu_alloc_pages_sg(struct device *dev, struct sg_table *sgt, size_t capacity)
{
	struct mvx_mmu_pages *pages;
	size_t count;
	int ret;
...
struct mvx_mmu_pages *mvx_mmu_alloc_pages_dma_buf(struct device *dev, struct dma_buf *dmabuf, size_t capacity)
{
	struct mvx_mmu_pages *pages;
	...
	if (IS_ERR(attach)) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Failed to attach DMA buffer.");
		return (struct mvx_mmu_pages *)attach;
	}

	sgt = dma_buf_map_attachment(attach, DMA_BIDIRECTIONAL);
	if (IS_ERR(sgt)) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Failed to get SG table from DMA buffer.");
...
int mvx_mmu_pages_append_dma_buf(struct mvx_mmu_pages *pages, struct dma_buf *dmabuf)
{
	struct dma_buf_attachment *attach;
	struct sg_table *sgt;
	struct mvx_mmu_dma_buf *mbuf;
	...
	if (pages->is_external == false) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Can't append DMA buffer to internal pages object.");
		return -EINVAL;
	}
...
int mvx_mmu_resize_pages(struct mvx_mmu_pages *pages, size_t npages)
{
	...
	if (pages->is_external != false) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "MMU with externally managed pages cannot be resized.");
		return -EINVAL;
	}
	...
	if (npages > pages->capacity) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING,
			      "New MMU pages size is larger than capacity. npages=%zu, capacity=%zu.", npages,
			      pages->capacity);
		return -ENOMEM;
	}
...
size_t mvx_mmu_size_pages(struct mvx_mmu_pages *pages) { return pages->count * MVE_PAGE_SIZE; }

int mvx_mmu_synch_pages(struct mvx_mmu_pages *pages, enum dma_data_direction dir, int page_off, int page_count)
{
	size_t i;
	if (page_off + page_count > pages->count) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Illegal mmu sync offset/size (%d/%d).", page_off,
			      page_count);
		page_off = 0;
	...
	if (dir == DMA_FROM_DEVICE) {
		for (i = 0; i < page_count; i += MVX_PAGES_PER_PAGE)
			dma_sync_single_for_cpu(pages->dev, pages->pages[page_off + i], PAGE_SIZE, DMA_FROM_DEVICE);
	} else if (dir == DMA_TO_DEVICE) {
		for (i = 0; i < page_count; i += MVX_PAGES_PER_PAGE)
			dma_sync_single_for_device(pages->dev, pages->pages[page_off + i], PAGE_SIZE, DMA_TO_DEVICE);
	} else {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Unsupported MMU flush direction. dir=%u.", dir);
		return -EINVAL;
	}
...
int mvx_mmu_map_pages(struct mvx_mmu *mmu, mvx_mmu_va va, struct mvx_mmu_pages *pages, enum mvx_mmu_attr attr,
		      enum mvx_mmu_access access)
{
	size_t i;
	int ret;
	...
	for (i = 0; i < pages->count; i++) {
		ret = map_page(mmu, va + i * MVE_PAGE_SIZE, pages->pages[i], attr, access);
		if (ret != 0)
			goto unmap_pages;
	}
	...
	for (; i < pages->capacity; i++) {
		ret = map_page(mmu, va + i * MVE_PAGE_SIZE, MVE_PAGE_SIZE, MVX_ATTR_PRIVATE, MVX_ACCESS_NO);
		if (ret != 0)
			goto unmap_pages;
	}
...
int mvx_mmu_map_pa(struct mvx_mmu *mmu, mvx_mmu_va va, phys_addr_t pa, size_t size, enum mvx_mmu_attr attr,
		   enum mvx_mmu_access access)
{
	int ret;
	size_t offset;
...
int mvx_mmu_map_l2(struct mvx_mmu *mmu, mvx_mmu_va va, phys_addr_t pa)
{
	...
	if (l2 != 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_ERROR, "Failed to map L2 page. Page already exists.");
		return -EINVAL;
	}

	set_bit(index, mmu->l2_page_is_external);

	pte[index] = mvx_mmu_set_pte(MVX_ATTR_PRIVATE, pa, MVX_ACCESS_READ_ONLY);
	dma_sync_single_for_device(mmu->dev, virt_to_phys(&pte[index]), sizeof(pte[index]), DMA_TO_DEVICE);
...
int mvx_mmu_write(struct mvx_mmu *mmu, mvx_mmu_va va, const void *data, size_t size)
{
	mvx_mmu_va end = va + size;
...
mvx_mmu_pte mvx_mmu_set_pte(enum mvx_mmu_attr attr, phys_addr_t pa, enum mvx_mmu_access access)
{
	return (attr << MVE_PTE_ATTR_SHIFT) | ((pa >> MVE_PAGE_SHIFT) << MVE_PTE_PHYSADDR_SHIFT) |
...
void mvx_mmu_print(struct mvx_mmu *mmu)
{
	...
	for (i = 0; i < MVE_INDEX_SIZE; i++) {
		...
		if (pa != 0) {
			...
			MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO, "%-4u: PA=0x%llx, ATTR=%u, ACC=%u", i, pa,
			...
			for (j = 0; j < MVE_INDEX_SIZE; j++) {
				...
				if (pa != 0) {
					...
					va = (i << (MVE_INDEX_SHIFT + MVE_PAGE_SHIFT)) | (j << MVE_PAGE_SHIFT);
					MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO,
						      "|------ %-4u: VA=0x%08x, PA=0x%llx, ATTR=%u, ACC=%u", j, va, pa,
						      get_attr(l2[j]), get_ap(l2[j]));
				}
			}
		}
	}
...
int mvx_mmu_pages_debugfs_init(struct mvx_mmu_pages *pages, char *name, struct dentry *parent)
{
