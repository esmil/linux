...
static struct kobj_type if_ktype = {.release = if_release, .sysfs_ops = &kobj_sysfs_ops};
...
struct mvx_if_ops *mvx_if_create(struct device *dev, struct mvx_client_ops *client_ops, void *priv)
{
	...
	ret = kobject_init_and_add(&ctx->kobj, &if_ktype, kernel_kobj, "amvx%u", dev->id);
	if (ret != 0) {
	...
	ret = mvx_fw_cache_construct(&ctx->firmware, dev, &ctx->secure, &ctx->kobj);
	if (ret != 0)
		goto destroy_secure;

	/* Create the external device interface. */
	ret = mvx_ext_if_construct(&ctx->ext, dev, &ctx->firmware, ctx->client_ops, ctx->dentry);
	if (ret != 0)
