...
int mvx_secure_construct(struct mvx_secure *secure, struct device *dev)
{
	...
	if (secure->kset == NULL) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Failed to create securevideo kset.");
		return -EINVAL;
	}
#endif
 ...
 * waiting process.
 */
static ssize_t firmware_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t size)
{
	struct mvx_secure_firmware_priv *securefw = container_of(kobj, struct mvx_secure_firmware_priv, kobj);
	const struct secure_firmware_desc *desc = (const void *)buf;

	MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO, "Loaded secure firmware. fd=%d, l2=0x%llx, major=%u, minor=%u.",
		      desc->fd, desc->l2pages, desc->protocol.major, desc->protocol.minor);
	...
	if (IS_ERR_OR_NULL(securefw->fw.dmabuf))
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Failed to get DMA buffer from fd. fd=%d.", desc->fd);
...
{
	struct mvx_secure_firmware_priv *securefw = container_of(work, struct mvx_secure_firmware_priv, work);
	int ret;

	ret = wait_event_interruptible_timeout(securefw->wait_queue, securefw->fw.dmabuf != NULL,
					       msecs_to_jiffies(10000));
	if (ret == 0)
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Firmware load timed out.");
 ...
 */
static struct mvx_secure_firmware_priv *secure_firmware_create(struct mvx_secure *secure, const char *name,
							       ...
							       mvx_secure_firmware_done done)
{
	...
	ret = kobject_init_and_add(&securefw->kobj, &secure_ktype, NULL, "%p", securefw);
	if (ret != 0)
		goto put_kobject;
	...
	if (ret != 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Failed to send secure firmware uevent. ret=%d.", ret);
		goto put_kobject;
	}
...
int mvx_secure_request_firmware_nowait(struct mvx_secure *secure, const char *name, unsigned int ncores, void *arg,
				       mvx_secure_firmware_done done)
{
	struct mvx_secure_firmware_priv *securefw;

	MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO, "Request secure firmware load nowait. firmware=%s.enc.", name);
...
{
	struct mvx_secure_firmware_priv *sfw = container_of(securefw, struct mvx_secure_firmware_priv, fw);
...
{
	struct mvx_secure_mem *smem = container_of(kobj, struct mvx_secure_mem, kobj);
 ...
 * waiting process.
 */
static ssize_t memory_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t size)
{
	struct mvx_secure_mem *smem = container_of(kobj, struct mvx_secure_mem, kobj);
	const int32_t *fd = (const int32_t *)buf;

	MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO, "Secure memory allocated. fd=%d.", *fd);

	smem->dmabuf = dma_buf_get(*fd);
	if (IS_ERR_OR_NULL(smem->dmabuf))
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Failed to get DMA buffer.");
...
struct dma_buf *mvx_secure_mem_alloc(struct mvx_secure *secure, size_t size)
{
	...
	/* Create kobject that the user space helper can interact with. */
	ret = kobject_init_and_add(&smem->kobj, &secure_mem_ktype, NULL, "%p", &smem->kobj);
	if (ret != 0)
		goto put_kobject;
	...
	if (ret != 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Failed to send secure memory uevent. ret=%d.", ret);
		goto put_kobject;
	}

	ret = wait_event_interruptible_timeout(smem->wait_queue, smem->dmabuf != NULL, msecs_to_jiffies(1000));
	if (ret == 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Secure memory allocation timed out.");
		goto put_kobject;
	}
