...
struct mvx_client_ops {
	 ...
	 */
	void (*get_hw_ver)(struct mvx_client_ops *client, struct mvx_hw_ver *version);
	 ...
	 */
	void (*get_formats)(struct mvx_client_ops *client, enum mvx_direction direction, uint64_t *formats);
	 ...
	 */
	struct mvx_client_session *(*register_session)(struct mvx_client_ops *client, struct mvx_if_session *session);
...
{
	return (format >= MVX_FORMAT_BITSTREAM_FIRST) && (format <= MVX_FORMAT_BITSTREAM_LAST);
}
...
{
	return (format >= MVX_FORMAT_FRAME_FIRST) && (format <= MVX_FORMAT_FRAME_LAST);
}
...
{
	return (format >= MVX_FORMAT_RGBA_8888) && (format <= MVX_FORMAT_ABGR_8888);
}
...
{
	return (format >= MVX_FORMAT_YUV420_AFBC_8) && (format <= MVX_FORMAT_YUV422_AFBC_10);
}
 ...
 */
struct mvx_if_ops *mvx_if_create(struct device *dev, struct mvx_client_ops *client_ops, void *priv);
