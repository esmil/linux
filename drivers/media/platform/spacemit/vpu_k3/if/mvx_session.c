...
static void watchdog_start(struct mvx_session *session, unsigned int timeout_ms)
{
	...
	MVX_SESSION_DEBUG(session, "Watchdog start. timeout_ms=%u.", timeout_ms);

	ret = mod_timer(&session->watchdog_timer, jiffies + msecs_to_jiffies(timeout_ms));
	if (ret != 0) {
		MVX_SESSION_WARN(session, "Failed to start watchdog. ret=%d", ret);
		return;
	}
...
static void watchdog_update(struct mvx_session *session, unsigned int timeout_ms)
{
	int ret;

#ifndef MODULE
	ret = mod_timer_pending(&session->watchdog_timer, jiffies + msecs_to_jiffies(timeout_ms));
#else
	...
	MVX_SESSION_DEBUG(session, "Watchdog update. ret=%d, timeout_ms=%u.", ret, timeout_ms);
}

static bool is_fw_loaded(struct mvx_session *session) { return (IS_ERR_OR_NULL(session->fw_bin) == false); }
...
{
	if (mvx_is_bitstream(session->port[MVX_DIR_INPUT].format) && mvx_is_frame(session->port[MVX_DIR_OUTPUT].format))
...
{
	return session->port[MVX_DIR_INPUT].stream_on && session->port[MVX_DIR_OUTPUT].stream_on;
}
...
static int wait_pending(struct mvx_session *session)
{
	...
	while (is_fw_loaded(session) != false && session->fw.msg_pending > 0 && session->error == 0) {
		...
		ret = wait_event_timeout(session->waitq,
					 is_fw_loaded(session) == false || session->fw.msg_pending == 0 ||
						 session->error != 0,
		...
		if (timeend - timestart >= 10000000l) {
			MVX_SESSION_WARN(session,
					 "wait_pending cost time is greater than 10 secs. cost_time=%llu us, ret=%d, "
					 "error=%d, msg_pending=%d, in_fmt=%d, out_fmt=%d",
					 timeend - timestart, ret, session->error, session->fw.msg_pending,
					 session->port[MVX_DIR_INPUT].format, session->port[MVX_DIR_OUTPUT].format);
		}
...
lock_mutex:
	...
	if (ret < 0) {
		MVX_SESSION_WARN(session, "Wait pending returned error. ret=%d, error=%d, msg_pending=%d.", ret,
				 session->error, session->fw.msg_pending);
...
static int wait_flush_done(struct mvx_session *session, enum mvx_direction dir)
{
	...
	while (is_fw_loaded(session) != false && port->buffer_count > 0 && session->error == 0) {
		mutex_unlock(session->isession.mutex);

		ret = wait_event_timeout(session->waitq,
					 is_fw_loaded(session) == false || port->buffer_count == 0 ||
						 session->error != 0,
...
static int wait_switch_out(struct mvx_session *session)
{
	...
	while (is_fw_loaded(session) != false && session->switched_in == true && session->error == 0) {
		mutex_unlock(session->isession.mutex);

		ret = wait_event_timeout(session->waitq,
					 is_fw_loaded(session) == false || session->switched_in == false ||
						 session->error != 0,
...
lock_mutex:
	...
	if (ret < 0) {
		MVX_SESSION_WARN(session,
				 "wait_switch_out returned error. ret=%d, error=%d, msg_pending=%d, switch_in=%d", ret,
				 session->error, session->fw.msg_pending, session->switched_in);
...
static int fw_send_msg_simple(struct mvx_session *session, enum mvx_fw_code code, const char *str)
{
...
static int fw_state_change(struct mvx_session *session, enum mvx_fw_state state)
{
	...
	if (state != session->fw_state) {
		MVX_SESSION_INFO(session, "Firmware req: State change. current=%d, new=%d.", session->fw_state, state);
		ret = fw_send_msg(session, &msg);
	}
...
static int fw_ping(struct mvx_session *session) { return fw_send_msg_simple(session, MVX_FW_CODE_PING, "Ping"); }

static int fw_dump(struct mvx_session *session) { return fw_send_msg_simple(session, MVX_FW_CODE_DUMP, "Dump"); }
...
static int fw_set_option(struct mvx_session *session, struct mvx_fw_set_option *option)
{
	struct mvx_fw_msg msg = {
	...
	MVX_SESSION_INFO(session, "Firmware req: Set option. code=%d.", option->code);

	return fw_send_msg(session, &msg);
}

static bool is_encoder(struct mvx_session *session) { return get_bitstream_port(session) == MVX_DIR_OUTPUT; }
...
static int fw_set_qp(struct mvx_session *session, int code, int qp)
{
	...
	if (ret != 0) {
		MVX_SESSION_WARN(session, "Failed to set QP. code=%d, ret=%d.", code, ret);
		return ret;
	}
...
static int fw_set_roi_regions(struct mvx_session *session, int code, struct mvx_roi_config *roi)
{
	struct mvx_fw_set_option option;
	int ret;
	...
	ret = fw_set_option(session, &option);
	if (ret != 0) {
		MVX_SESSION_WARN(session, "Failed to set ROI. code=%d, ret=%d.", code, ret);
		return ret;
	}
...
static int fw_common_setup(struct mvx_session *session)
{
	...
	if ((session->port[dir].format == MVX_FORMAT_VP8 || session->port[dir].format == MVX_FORMAT_VP9) &&
	...
	if (session->stream_escaping != MVX_TRI_UNSET) {
		...
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to set stream escaping.");
			return ret;
		}
	}
	...
	if (session->enable_profiling) {
		...
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to set set index profiling");
			return ret;
		}
	}
...
static int fw_encoder_setup(struct mvx_session *session)
{
	...
	if (session->profile[codec] != MVX_PROFILE_NONE) {
		...
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to set profile/level.");
			return ret;
		}
	}
	...
	if (codec != MVX_FORMAT_JPEG) {
		...
		if (session->rc_type) {
			...
			option.rate_control.target_bitrate = session->rc_type ? session->target_bitrate : 0;
			...
			ret = fw_set_option(session, &option);
			if (ret != 0) {
				MVX_SESSION_WARN(session, "Failed to put target bitrate.");
				return ret;
			}
		}
		...
		if (session->rc_enabled != false) {
			if (session->qp[codec].min < session->qp[codec].max) {
				...
				ret = fw_set_option(session, &option);
				if (ret != 0) {
					MVX_SESSION_WARN(session, "Failed to set qp range.");
					return ret;
				}
			}
		} else {
			ret = fw_set_qp(session, MVX_FW_SET_QP_I, session->qp[codec].i_frame);
			if (ret != 0)
				return ret;

			ret = fw_set_qp(session, MVX_FW_SET_QP_P, session->qp[codec].p_frame);
			if (ret != 0)
				return ret;

			ret = fw_set_qp(session, MVX_FW_SET_QP_B, session->qp[codec].b_frame);
			if (ret != 0)
				return ret;
		}
		...
		if (session->gop_type != MVX_GOP_TYPE_NONE) {
			...
			ret = fw_set_option(session, &option);
			if (ret != 0) {
				MVX_SESSION_WARN(session, "Failed to set GOP type.");
				return ret;
			}
		}
		...
		if (session->cyclic_intra_refresh_mb != 0) {
			...
			option.intra_mb_refresh = session->cyclic_intra_refresh_mb;
			ret = fw_set_option(session, &option);
			if (ret != 0) {
				MVX_SESSION_WARN(session, "Failed to set cyclic intra refresh Mb.");
				return ret;
			}
		}

		if (session->constr_ipred != MVX_TRI_UNSET && (codec == MVX_FORMAT_H264 || codec == MVX_FORMAT_HEVC)) {
			option.code = MVX_FW_SET_CONSTR_IPRED;
			option.constr_ipred = session->constr_ipred;
			ret = fw_set_option(session, &option);
			if (ret != 0) {
				MVX_SESSION_WARN(session, "Failed to set constr ipred.");
				return ret;
			}
		}
	}
	...
	if (codec == MVX_FORMAT_HEVC) {
		if (session->entropy_sync != MVX_TRI_UNSET) {
			...
			ret = fw_set_option(session, &option);
			if (ret != 0) {
				MVX_SESSION_WARN(session, "Failed to set entropy sync.");
				return ret;
			}
		}
		...
		if (session->temporal_mvp != MVX_TRI_UNSET) {
			...
			ret = fw_set_option(session, &option);
			if (ret != 0) {
				MVX_SESSION_WARN(session, "Failed to set temporal mvp.");
				return ret;
			}
		}
		...
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to set min luma cb size.");
			return ret;
		}
	}
	...
	if (session->entropy_mode != MVX_ENTROPY_MODE_NONE && codec == MVX_FORMAT_H264) {
		option.code = MVX_FW_SET_ENTROPY_MODE;
		option.entropy_mode = session->entropy_mode;
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to set entropy mode.");
			return ret;
		}
	}
	...
	if (codec == MVX_FORMAT_H264 || codec == MVX_FORMAT_HEVC) {
		...
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to set slice spacing.");
			return ret;
		}
		...
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to set CABAC init IDC.");
			return ret;
		}
		if (session->crop_left != 0) {
			...
			ret = fw_set_option(session, &option);
			if (ret != 0) {
				MVX_SESSION_WARN(session, "Failed to set crop left");
				return ret;
			}
		}
		if (session->crop_right != 0) {
			...
			ret = fw_set_option(session, &option);
			if (ret != 0) {
				MVX_SESSION_WARN(session, "Failed to set crop right");
				return ret;
			}
		}
		if (session->crop_top != 0) {
			...
			ret = fw_set_option(session, &option);
			if (ret != 0) {
				MVX_SESSION_WARN(session, "Failed to set crop top");
				return ret;
			}
		}
		if (session->crop_bottom != 0) {
			...
			ret = fw_set_option(session, &option);
			if (ret != 0) {
				MVX_SESSION_WARN(session, "Failed to set crop bottom");
				return ret;
			}
		}
		...
		if (session->color_desc.range != 0 || session->color_desc.matrix != 0 ||
			...
			ret = fw_set_option(session, &option);
			if (ret != 0) {
				MVX_SESSION_WARN(session, "Failed to set vui colour description");
				return ret;
			}
		}
		...
		if (session->sei_userdata.flags) {
			...
			ret = fw_set_option(session, &option);
			if (ret != 0) {
				MVX_SESSION_WARN(session, "Failed to set sei userdata");
				return ret;
			}
		}
		...
		if (session->mvx_ltr.mode != 0 || session->mvx_ltr.period != 0) {
			...
			ret = fw_set_option(session, &option);
			if (ret != 0) {
				MVX_SESSION_WARN(session, "Failed to set ltr mode/period");
				return ret;
			}
		}
	}
	...
	if (codec == MVX_FORMAT_VP9) {
		...
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to set VP9 prob update mode.");
			return ret;
		}
	}
	...
	if (session->mv_h_search_range != 0 && session->mv_v_search_range != 0) {
		...
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to set motion vector search range.");
			return ret;
		}
	}
	...
	if (session->force_chroma_format != 0) {
		...
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to set chroma format.");
			return ret;
		}
	}
	...
	if (mvx_is_rgb(session->port[MVX_DIR_INPUT].format) != false) {
		...
		if (session->color_desc.primaries == MVX_FW_PRIMARIES_BT709) {
			session->rgb_to_yuv = (session->color_desc.range == MVX_FW_RANGE_FULL)
		...
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to set RGB to YUV mode.");
			return ret;
		}
	}
	...
	if (session->band_limit != 0) {
		...
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to set bandwidth limit.");
			return ret;
		}
	}
	...
	if (codec == MVX_FORMAT_JPEG) {
		if (session->resync_interval >= 0) {
			...
			ret = fw_set_option(session, &option);
			if (ret != 0) {
				MVX_SESSION_WARN(session, "Failed to set resync interval.");
				return ret;
			}
		}
		...
		if (session->jpeg_quality != 0) {
			...
			generate_quant_tbl(session->jpeg_quality, qtbl_chroma_ref, qtbl_chroma);
			generate_quant_tbl(session->jpeg_quality, qtbl_luma_ref, qtbl_luma);
			option.code = MVX_FW_SET_QUANT_TABLE;
			option.quant_tbl.chroma = qtbl_chroma;
			option.quant_tbl.luma = qtbl_luma;
			ret = fw_set_option(session, &option);
			if (ret != 0) {
				MVX_SESSION_WARN(session, "Failed to set quantization table.");
				return ret;
			}
		}
	}
...
static int fw_decoder_setup(struct mvx_session *session)
{
	...
	if (codec == MVX_FORMAT_VC1 && session->profile[codec] != MVX_PROFILE_NONE) {
		option.code = MVX_FW_SET_PROFILE_LEVEL;
		option.profile_level.profile = session->profile[codec];
		option.profile_level.level = session->level[codec];
		ret = fw_set_option(session, &option);
	...
	if (session->ignore_stream_headers != MVX_TRI_UNSET) {
		...
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to set ignore stream headers.");
			return ret;
		}
	}
	...
	if (session->frame_reordering != MVX_TRI_UNSET) {
		...
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to set frame reordering.");
			return ret;
		}
	}
	...
	if (session->intbuf_size != 0) {
		...
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to set internal buffer size.");
			return ret;
		}
	}
	...
	if (session->dsl_frame.width != 0 && session->dsl_frame.height != 0) {
		...
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to set DSL frame width/height.");
			return ret;
		}
	}
...
static void ddr_qos_request_update(struct mvx_session *session)
{
	...
	mvx_dvfs_session_update_ddr_qos(session, session->estimated_ddr_read_throughput,
					session->estimated_ddr_write_throughput);
}
 ...
 * Return 0 on success, else error code.
 */
static int map_buffer(struct mvx_session *session, enum mvx_direction dir, struct mvx_buffer *buf)
{
	mvx_mmu_va begin;
	mvx_mmu_va end;
...
static int queue_roi_regions(struct mvx_session *session, struct mvx_roi_config *roi_cfg)
{
	int ret = 0;
...
static int queue_qp_epr(struct mvx_session *session, int *qp)
{
	int ret = 0;
	ret = fw_set_qp(session, MVX_FW_SET_QP_REGION, *qp);
 ...
 * Return: 0 on success, else error code.
 */
static int queue_buffer(struct mvx_session *session, enum mvx_direction dir, struct mvx_buffer *buf)
{
	struct mvx_session_port *port = &session->port[dir];
	struct mvx_fw_msg msg;
	int i;
	 ...
	 */
	enum dma_data_direction dma_dir = (dir == MVX_DIR_OUTPUT) ? DMA_FROM_DEVICE : DMA_TO_DEVICE;
	...
	if (mvx_is_afbc(port->format) != false) {
		...
		ret = mvx_buffer_afbc_set(buf, port->format, port->width, port->height, port->afbc_width, port->size[0],
					  port->interlaced);
		if (ret != 0)
			return ret;
	} else if (mvx_is_frame(port->format) != false) {
		ret = mvx_buffer_frame_set(buf, port->format, port->width, port->height, port->stride, port->size,
					   port->interlaced);
		if (ret != 0)
			return ret;
	}

	if (mapped && ((dir == MVX_DIR_OUTPUT) || (dir == MVX_DIR_INPUT && mvx_is_frame(port->format) &&
	...
	MVX_SESSION_INFO(session, "Firmware req: Buffer. dir=%u, len=[%u, %u, %u], flags=0x%08x, eos=%u, interlace=%u",
			 buf->dir, buf->planes[0].filled, buf->planes[1].filled, buf->planes[2].filled, buf->flags,
			 (buf->flags & MVX_BUFFER_EOS) != 0, (buf->flags & MVX_BUFFER_INTERLACE) != 0);
 ...
 * Return: 0 on success, else error code.
 */
static int queue_pending_buffers(struct mvx_session *session, enum mvx_direction dir)
{
...
static void fw_bin_ready(struct mvx_fw_bin *bin, void *arg, bool same_thread)
{
	...
	session->isession.ncores = session->client_ops->get_ncores(session->client_ops);
	session->isession.l0_pte =
		mvx_mmu_set_pte(MVX_ATTR_PRIVATE, virt_to_phys(session->mmu.page_table), MVX_ACCESS_READ_WRITE);
	...
	ret = mvx_fw_factory(&session->fw, bin, &session->mmu, session, session->client_ops, session->csession,
...
static int calc_afbc_size(struct mvx_session *session, enum mvx_format format, unsigned int width, unsigned int height,
			  bool tiled_headers, bool tiled_body, bool superblock, bool interlaced)
{
	...
	case MVX_FORMAT_YUV422_AFBC_10:
		...
		break;
	default:
		MVX_SESSION_WARN(session, "Unsupported AFBC format. format=%u.", format);
		return -EINVAL;
	}
...
static int try_format(struct mvx_session *session, enum mvx_direction dir, enum mvx_format format, unsigned int *width,
		      unsigned int *height, uint8_t *nplanes, unsigned int *stride, unsigned int *size,
		      bool *interlaced)
{
	int ret = 0;
	...
	*height = min_t(unsigned int, *height, 8192);
	...
	if (mvx_is_afbc(format) != false) {
		unsigned int afbc_alloc_bytes = session->port[dir].afbc_alloc_bytes;
		if (*nplanes <= 0)
			size[0] = 0;

		if (dir == MVX_DIR_INPUT) {
			/* it is basically a worst-case calcualtion based on a size rounded up to tile size*/
			int s1 = calc_afbc_size(session, format, *width, *height, false, false,
						false, //*height, false, false, false,
						*interlaced);
			int s2 = calc_afbc_size(session, format, *width, *height, false, false,
		...
		*nplanes = 1;
		...
		ret = mvx_buffer_frame_dim(format, *width, *height, nplanes, stride, size);
	} else {
		/*
...
{
	struct mvx_session *session = container_of(work, struct mvx_session, watchdog_work);
	int ret;
	...
	MVX_SESSION_WARN(session, "Watchdog timeout. count=%u. is_encoder=%d. in_fmt=%d, out_fmt=%d",
			 session->watchdog_count, is_encoder(session), session->port[MVX_DIR_INPUT].format,
...
{
	struct mvx_session *session = container_of(timer, struct mvx_session, watchdog_timer);

	queue_work(system_unbound_wq, &session->watchdog_work);
}

#if KERNEL_VERSION(4, 14, 0) > LINUX_VERSION_CODE
static void watchdog_timeout_legacy(unsigned long data) { watchdog_timeout((struct timer_list *)data); }
...
int mvx_session_construct(struct mvx_session *session, struct device *dev, struct mvx_client_ops *client_ops,
			  struct mvx_fw_cache *cache, struct mutex *mutex,
			  void (*destructor)(struct mvx_session *session),
			  void (*event)(struct mvx_session *session, enum mvx_session_event event, void *arg),
...
#else
	setup_timer(&session->watchdog_timer, watchdog_timeout_legacy, (uintptr_t)&session->watchdog_timer);
#endif
...
void mvx_session_destruct(struct mvx_session *session)
{
	...
	if (session->enable_profiling && session->frame_id > 0) {
		MVX_SESSION_WARN(session, "[Debug] Destroy session. is_encoder=%d, frame_count=%d", is_encoder(session),
				 session->frame_id);
		MVX_SESSION_WARN(session,
				 "[Debug] bus_write_bytes_total=%lu. bus_read_bytes_total=%lu. avg_write_bw=%lu, "
				 "avg_read_bw=%lu, 30fps_write_bw=%lu, 30fps_read_bw=%lu",
				 session->bus_write_bytes_total, session->bus_read_bytes_total,
				 session->bus_write_bytes_total / session->frame_id,
				 session->bus_read_bytes_total / session->frame_id,
				 session->bus_write_bytes_total / session->frame_id * 30,
				 session->bus_read_bytes_total / session->frame_id * 30);
	}
...
void mvx_session_get(struct mvx_session *session) { kref_get(&session->isession.kref); }
...
void mvx_session_get_formats(struct mvx_session *session, enum mvx_direction dir, uint64_t *formats)
{
	uint64_t fw_formats;
...
int mvx_session_try_format(struct mvx_session *session, enum mvx_direction dir, enum mvx_format format,
			   unsigned int *width, unsigned int *height, uint8_t *nplanes, unsigned int *stride,
			   unsigned int *size, bool *interlaced)
{
	return try_format(session, dir, format, width, height, nplanes, stride, size, interlaced);
}

int mvx_session_set_format(struct mvx_session *session, enum mvx_direction dir, enum mvx_format format,
			   unsigned int *width, unsigned int *height, uint8_t *nplanes, unsigned int *stride,
			   unsigned int *size, bool *interlaced)
{
	struct mvx_session_port *port = &session->port[dir];
	...
	ret = try_format(session, dir, format, width, height, nplanes, stride, size, interlaced);
	if (ret != 0)
		return ret;
	...
	if (mvx_is_bitstream(port->format) != false && format != port->format) {
		if (IS_ERR(session->fw_bin) != false) {
			MVX_SESSION_WARN(session, "Can't set format when firmware binary is pending. dir=%d.", dir);
			return -EINVAL;
		}
	...
	if (dir == MVX_DIR_INPUT) {
		...
		(void)try_format(session, MVX_DIR_OUTPUT, p->format, &p->width, &p->height, &p->nplanes, p->stride,
				 p->size, &p->interlaced);
	}
...
int mvx_session_qbuf(struct mvx_session *session, enum mvx_direction dir, struct mvx_buffer *buf)
{
	int ret;
	if (session->error != 0)
		return session->error;

	if (is_fw_loaded(session) == false || session->port[dir].is_flushing != false) {
...
int mvx_session_send_eos(struct mvx_session *session)
{
	...
	if (list_empty(&port->buffer_queue) != false) {
		MVX_SESSION_WARN(session, "Unable to signal EOS. Output buffer queue empty.");
		return 0;
	}
...
int mvx_session_streamon(struct mvx_session *session, enum mvx_direction dir)
{
	...
	if (bdir >= MVX_DIR_MAX) {
		MVX_SESSION_WARN(session,
				 "Session only support decoding and encoding, but not transcoding. input_format=%u, "
				 "output_format=%u.",
				 session->port[MVX_DIR_INPUT].format, session->port[MVX_DIR_OUTPUT].format);
		ret = -EINVAL;
		goto disable_port;
	...
	for (i = MVX_DIR_INPUT; i < MVX_DIR_MAX; i++) {
		...
		session->client_ops->get_formats(session->client_ops, MVX_DIR_INPUT, &formats);
	...
	ret = mvx_fw_cache_get(session->cache, session->port[bdir].format, bdir, &session->fw_event, &hw_ver,
...
int mvx_session_streamoff(struct mvx_session *session, enum mvx_direction dir)
{
	...
	MVX_SESSION_INFO(
		session,
		"Stream off. dir=%u, flushed=%d, is_flushing=%d, isreallocting=%d, isallocparam=%d, force_stop=%d", dir,
		port->flushed, port->is_flushing, port->isreallocting, port->isallocparam, force_stop);
	...
	if (is_fw_loaded(session) != false) {
		...
		if (session->fw_state == MVX_FW_STATE_STOPPED) {
			...
			if (session->switched_in) {
				MVX_SESSION_WARN(session, "warn: switch_in is %d when stream off done.",
						 session->switched_in);
			}
		}
	}
...
static void handle_fw_message(struct mvx_session *session, struct mvx_fw_msg *msg)
{
	switch (msg->code) {
	case MVX_FW_CODE_ALLOC_PARAM: {
		...
		try_format(session, MVX_DIR_INPUT, input->format, &input->width, &input->height, &input->nplanes,
			   input->stride, input->size, &input->interlaced);
		...
		try_format(session, MVX_DIR_OUTPUT, output->format, &output->width, &output->height, &output->nplanes,
			   output->stride, output->size, &output->interlaced);

		MVX_SESSION_INFO(session,
				 "Firmware rsp: Alloc param. width=%u, height=%u, nplanes=%u, size=[%u, %u, %u], "
				 "stride=[%u, %u, %u], interlaced=%d.",
				 msg->alloc_param.width, msg->alloc_param.height, output->nplanes, output->size[0],
				 output->size[1], output->size[2], output->stride[0], output->stride[1],
				 output->stride[2], output->interlaced);

		// update ddr qos for decoder output size changed.
		ddr_qos_request_update(session);

		if (output->size[0] > output->buffer_size[0] || output->size[1] > output->buffer_size[1] ||
	...
	case MVX_FW_CODE_BUFFER: {
		...
		MVX_SESSION_INFO(session, "Firmware rsp: Buffer. dir=%u, len=[%u, %u, %u], flags=0x%08x, eos=%u",
				 buf->dir, buf->planes[0].filled, buf->planes[1].filled, buf->planes[2].filled,
		...
		if (buf->dir == MVX_DIR_OUTPUT && !is_encoder(session)) {
			session->port[MVX_DIR_OUTPUT].buffer_on_hold_count =
				session->port[MVX_DIR_OUTPUT].pending_buffer_on_hold_count;
		}
	...
	case MVX_FW_CODE_ERROR: {
		MVX_SESSION_WARN(session, "Firmware rsp: Error. code=%u, message=%s.", msg->error.error_code,
				 msg->error.message);
		print_debug(session);
		fw_dump(session);
	...
	case MVX_FW_CODE_FLUSH: {
		MVX_SESSION_INFO(session, "Firmware rsp: Flushed. dir=%d.", msg->flush.dir);
	...
	case MVX_FW_CODE_SEQ_PARAM: {
		...
		MVX_SESSION_INFO(
			...
			msg->seq_param.planar.buffers_min, msg->seq_param.afbc.buffers_min, p->interlaced);
	...
	case MVX_FW_CODE_STATE_CHANGE: {
		MVX_SESSION_INFO(session, "Firmware rsp: State changed. old=%s, new=%s.",
				 state_to_string(session->fw_state), state_to_string(msg->state));
	...
	case MVX_FW_CODE_SWITCH_OUT: {
		...
		if (session->is_suspend == false &&
		    ((session->fw_state == MVX_FW_STATE_RUNNING && session->idle_count < 2) ||
	...
	case MVX_FW_CODE_MAX:
		break;
	default:
		MVX_SESSION_WARN(session, "Unknown fw msg code. code=%u.", msg->code);
	}
}
...
int mvx_session_set_frame_rate(struct mvx_session *session, int64_t frame_rate)
{
	...
	if (frame_rate < (1 << 16))
		frame_rate = (1 << 16); /*1 fps*/
	else if (frame_rate > (256 << 16))
		frame_rate = (256 << 16); /*256 fps*/
...
int mvx_session_set_rate_control(struct mvx_session *session, bool enabled)
{
	...
	if (is_fw_loaded(session) != false) {
		...
		option.target_bitrate = (session->rc_enabled != false) ? session->target_bitrate : 0;
		ret = fw_set_option(session, &option);
		if (ret != 0)
			return ret;
	}
...
int mvx_session_set_bitrate_control(struct mvx_session *session, struct mvx_buffer_param_rate_control *rc)
{
	int ret;
...
int mvx_session_set_nalu_format(struct mvx_session *session, enum mvx_nalu_format fmt)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_stream_escaping(struct mvx_session *session, enum mvx_tristate status)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_profile(struct mvx_session *session, enum mvx_format format, enum mvx_profile profile)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_level(struct mvx_session *session, enum mvx_format format, enum mvx_level level)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_ignore_stream_headers(struct mvx_session *session, enum mvx_tristate status)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_frame_reordering(struct mvx_session *session, enum mvx_tristate status)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_gop_type(struct mvx_session *session, enum mvx_gop_type gop_type)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_cyclic_intra_refresh_mb(struct mvx_session *session, int val)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_constr_ipred(struct mvx_session *session, enum mvx_tristate status)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_entropy_sync(struct mvx_session *session, enum mvx_tristate status)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_temporal_mvp(struct mvx_session *session, enum mvx_tristate status)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_entropy_mode(struct mvx_session *session, enum mvx_entropy_mode mode)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_multi_slice_mode(struct mvx_session *session, enum mvx_multi_slice_mode mode)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_vp9_prob_update(struct mvx_session *session, enum mvx_vp9_prob_update mode)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_rgb_to_yuv_mode(struct mvx_session *session, enum mvx_rgb_to_yuv_mode mode)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_i_frame_qp(struct mvx_session *session, enum mvx_format fmt, int qp)
{
	int ret;
...
int mvx_session_set_p_frame_qp(struct mvx_session *session, enum mvx_format fmt, int qp)
{
	int ret;
...
int mvx_session_set_b_frame_qp(struct mvx_session *session, enum mvx_format fmt, int qp)
{
	int ret;
...
int mvx_session_set_min_qp(struct mvx_session *session, enum mvx_format fmt, int qp)
{
	int ret;
...
int mvx_session_set_max_qp(struct mvx_session *session, enum mvx_format fmt, int qp)
{
	int ret;
...
int mvx_session_get_color_desc(struct mvx_session *session, struct mvx_fw_color_desc *color_desc)
{
	*color_desc = session->color_desc;
	return 0;
}

int mvx_session_set_color_desc(struct mvx_session *session, struct mvx_fw_color_desc *color_desc)
{
	int ret = 0;
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_roi_regions(struct mvx_session *session, struct mvx_roi_config *roi)
{
	int ret = 0;
	int roi_config_num = 0;
	if (is_fw_loaded(session) == false || session->port[MVX_DIR_INPUT].is_flushing != false) {
		...
		if (roi_config_num < MVX_ROI_QP_NUMS) {
			...
			MVX_SESSION_ERR(session, "fw is not ready for long time, too many roi pending:%d",
					roi_config_num);
		}
		return 0;
	}
...
int mvx_session_set_qp_epr(struct mvx_session *session, int *qp)
{
	int ret = 0;
	int qp_num = 0;
	if (is_fw_loaded(session) == false || session->port[MVX_DIR_INPUT].is_flushing != false) {
...
int mvx_session_set_dsl_frame(struct mvx_session *session, struct mvx_dsl_frame *dsl)
{
	int ret;
...
int mvx_session_set_dsl_ratio(struct mvx_session *session, struct mvx_dsl_ratio *dsl)
{
	if (session->error != 0)
		return session->error;
...
int mvx_session_set_long_term_ref(struct mvx_session *session, struct mvx_long_term_ref *ltr)
{
	int ret;
...
int mvx_session_set_force_idr(struct mvx_session *session)
{
	...
	if (is_fw_loaded(session) != false) {
		...
		ret = fw_set_option(session, &option);
		if (ret != 0) {
			MVX_SESSION_WARN(session, "Failed to  GOP reset.");
			return ret;
		}
	}
	return 0;
