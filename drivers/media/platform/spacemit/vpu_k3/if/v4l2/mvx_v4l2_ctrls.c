...
{
	int ret = 0;
	struct mvx_v4l2_session *vsession = container_of(ctrl->handler, struct mvx_v4l2_session, v4l2_ctrl);
	...
	case V4L2_CID_MPEG_VIDEO_H264_PROFILE:
		i32_val = *ctrl->p_new.p_s32;
		ret = find_idx(h264_profile_list, ARRAY_SIZE(h264_profile_list), i32_val);
		if (ret == -EINVAL)
			goto unlock_mutex;

		mvx_profile = mvx_h264_profile_list[ret];
		ret = mvx_session_set_profile(session, MVX_FORMAT_H264, mvx_profile);
		break;
	case V4L2_CID_MPEG_VIDEO_HEVC_PROFILE:
		i32_val = *ctrl->p_new.p_s32;
		ret = find_idx(hevc_profile_list, ARRAY_SIZE(hevc_profile_list), i32_val);
		if (ret == -EINVAL)
			goto unlock_mutex;

		mvx_profile = mvx_h265_profile_list[ret];
		ret = mvx_session_set_profile(session, MVX_FORMAT_HEVC, mvx_profile);
		break;
	...
	case V4L2_CID_MVE_VIDEO_VC1_PROFILE:
		...
		ret = mvx_session_set_profile(session, MVX_FORMAT_VC1, mvx_profile);
		break;
	case V4L2_CID_MPEG_VIDEO_H264_LEVEL:
		i32_val = *ctrl->p_new.p_s32;
		ret = find_idx(h264_level_list, ARRAY_SIZE(h264_level_list), i32_val);
		if (ret == -EINVAL)
			goto unlock_mutex;

		mvx_level = mvx_h264_level_list[ret];
		ret = mvx_session_set_level(session, MVX_FORMAT_H264, mvx_level);
		break;
	case V4L2_CID_MPEG_VIDEO_HEVC_LEVEL:
		i32_val = *ctrl->p_new.p_s32;
		ret = find_idx(hevc_level_list, ARRAY_SIZE(hevc_level_list), i32_val);
		if (ret == -EINVAL)
			goto unlock_mutex;

		mvx_level = mvx_h265_level_list[ret * 2];
		ret = mvx_session_set_level(session, MVX_FORMAT_HEVC, mvx_level);
		break;
	...
	case V4L2_CID_MPEG_VIDEO_H264_ENTROPY_MODE:
		i32_val = *ctrl->p_new.p_s32;
		ret = find_idx(h264_entropy_mode_list, ARRAY_SIZE(h264_entropy_mode_list), i32_val);
		if (ret == -EINVAL)
			goto unlock_mutex;
	...
	case V4L2_CID_MPEG_VIDEO_MULTI_SLICE_MODE:
		i32_val = *ctrl->p_new.p_s32;
		ret = find_idx(multi_slice_mode_list, ARRAY_SIZE(multi_slice_mode_list), i32_val);
		if (ret == -EINVAL)
			goto unlock_mutex;

		multi_slice_mode = mvx_multi_slice_mode_list[ret];
		ret = mvx_session_set_multi_slice_mode(session, multi_slice_mode);
		break;
	...
	case V4L2_CID_MPEG_VIDEO_H263_I_FRAME_QP:
		i32_val = *ctrl->p_new.p_s32;
		ret = mvx_session_set_i_frame_qp(session, MVX_FORMAT_H263, i32_val);
		break;
	case V4L2_CID_MPEG_VIDEO_H263_P_FRAME_QP:
		i32_val = *ctrl->p_new.p_s32;
		ret = mvx_session_set_p_frame_qp(session, MVX_FORMAT_H263, i32_val);
		break;
	case V4L2_CID_MPEG_VIDEO_H263_B_FRAME_QP:
		i32_val = *ctrl->p_new.p_s32;
		ret = mvx_session_set_b_frame_qp(session, MVX_FORMAT_H263, i32_val);
		break;
	...
	case V4L2_CID_MPEG_VIDEO_H264_I_FRAME_QP:
		i32_val = *ctrl->p_new.p_s32;
		ret = mvx_session_set_i_frame_qp(session, MVX_FORMAT_H264, i32_val);
		break;
	case V4L2_CID_MPEG_VIDEO_H264_P_FRAME_QP:
		i32_val = *ctrl->p_new.p_s32;
		ret = mvx_session_set_p_frame_qp(session, MVX_FORMAT_H264, i32_val);
		break;
	case V4L2_CID_MPEG_VIDEO_H264_B_FRAME_QP:
		i32_val = *ctrl->p_new.p_s32;
		ret = mvx_session_set_b_frame_qp(session, MVX_FORMAT_H264, i32_val);
		break;
	...
	case V4L2_CID_MPEG_VIDEO_HEVC_I_FRAME_QP:
		i32_val = *ctrl->p_new.p_s32;
		ret = mvx_session_set_i_frame_qp(session, MVX_FORMAT_HEVC, i32_val);
		break;
	case V4L2_CID_MPEG_VIDEO_HEVC_P_FRAME_QP:
		i32_val = *ctrl->p_new.p_s32;
		ret = mvx_session_set_p_frame_qp(session, MVX_FORMAT_HEVC, i32_val);
		break;
	case V4L2_CID_MPEG_VIDEO_HEVC_B_FRAME_QP:
		i32_val = *ctrl->p_new.p_s32;
		ret = mvx_session_set_b_frame_qp(session, MVX_FORMAT_HEVC, i32_val);
		break;
	...
	case V4L2_CID_MPEG_VIDEO_VPX_I_FRAME_QP:
		i32_val = *ctrl->p_new.p_s32;
		ret = mvx_session_set_i_frame_qp(session, MVX_FORMAT_VP9, i32_val);
		break;
	case V4L2_CID_MPEG_VIDEO_VPX_P_FRAME_QP:
		i32_val = *ctrl->p_new.p_s32;
		ret = mvx_session_set_p_frame_qp(session, MVX_FORMAT_VP9, i32_val);
		break;
	case V4L2_CID_MVE_VIDEO_VPX_B_FRAME_QP:
		i32_val = *ctrl->p_new.p_s32;
		ret = mvx_session_set_b_frame_qp(session, MVX_FORMAT_VP9, i32_val);
		break;
...
static int get_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_MIN_BUFFERS_FOR_CAPTURE:
		...
		break;
	default:
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Unsupported get control. id=%u.", ctrl->id);
		return -EINVAL;
	}
 ...
 */
static const struct v4l2_ctrl_ops ctrl_ops = {.g_volatile_ctrl = get_volatile_ctrl, .s_ctrl = set_ctrl};
 ...
 */
static struct v4l2_ctrl *mvx_v4l2_ctrl_new_custom_int(struct v4l2_ctrl_handler *hnd, int id, const char *name,
						      int64_t min, int64_t max, int64_t def, int32_t step)
{
 ...
 *         or NULL in case of failure.
 */
static struct v4l2_ctrl *mvx_v4l2_ctrl_new_custom_tristate(struct v4l2_ctrl_handler *hnd, int id, const char *name,
...
int mvx_v4l2_ctrls_init(struct v4l2_ctrl_handler *hnd)
{
	...
	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_SECURE_VIDEO, "secure video", 0, 1, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_FRAME_RATE, "frame rate", 0, 0x10000000, 30 << 16,
					    1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_CROP_LEFT, "bitrate control mode", 0, 10000000, 0,
					    1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_CROP_RIGHT, "bitrate control mode", 0, 10000000, 0,
					    1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_CROP_TOP, "bitrate control mode", 0, 10000000, 0,
					    1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_CROP_BOTTOM, "bitrate control mode", 0, 10000000, 0,
					    1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_HRD_BUFFER_SIZE, "HRD buffer size", 0, 1073741823,
					    0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_FRAME_RC_ENABLE, 0, 1, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std_menu(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_BITRATE_MODE, V4L2_MPEG_VIDEO_BITRATE_MODE_CQ,
				      ~7, V4L2_MPEG_VIDEO_BITRATE_MODE_VBR);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_BITRATE_PEAK, 1000, 1000000000, 1, 64000);
	if (ctrl == NULL)
		goto handler_free;
	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_BITRATE, 1000, 1000000000, 1, 64000);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl = mvx_v4l2_ctrl_new_custom_tristate(hnd, V4L2_CID_MVE_VIDEO_STREAM_ESCAPING, "stream escaping",
						 MVX_TRI_UNSET);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std_menu(
		hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_H264_PROFILE, V4L2_MPEG_VIDEO_H264_PROFILE_HIGH_10,
		get_skip_mask(h264_profile_list, ARRAY_SIZE(h264_profile_list)), V4L2_MPEG_VIDEO_H264_PROFILE_MAIN);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std_menu(
		hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_HEVC_PROFILE, V4L2_MPEG_VIDEO_HEVC_PROFILE_MAIN,
		get_skip_mask(hevc_profile_list, ARRAY_SIZE(hevc_profile_list)), V4L2_MPEG_VIDEO_HEVC_PROFILE_MAIN);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl = v4l2_ctrl_new_std_menu(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_H264_LEVEL, V4L2_MPEG_VIDEO_H264_LEVEL_5_1,
				      get_skip_mask(h264_level_list, ARRAY_SIZE(h264_level_list)),
				      V4L2_MPEG_VIDEO_H264_LEVEL_1_0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std_menu(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_HEVC_LEVEL, V4L2_MPEG_VIDEO_HEVC_LEVEL_6_2,
	...
	ctrl = mvx_v4l2_ctrl_new_custom_tristate(hnd, V4L2_CID_MVE_VIDEO_IGNORE_STREAM_HEADERS, "ignore stream headers",
						 MVX_TRI_UNSET);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_tristate(hnd, V4L2_CID_MVE_VIDEO_FRAME_REORDERING, "frame reordering",
						 MVX_TRI_UNSET);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_INTBUF_SIZE, "internal buffer size", 0, INT_MAX, 0,
					    1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_P_FRAMES, "video P frames", 0, INT_MAX, 30, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_B_FRAMES, 0, INT_MAX, 1, 0);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_CYCLIC_INTRA_REFRESH_MB, 0, INT_MAX, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_tristate(hnd, V4L2_CID_MVE_VIDEO_CONSTR_IPRED, "constrained intra prediction",
						 MVX_TRI_UNSET);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_tristate(hnd, V4L2_CID_MVE_VIDEO_ENTROPY_SYNC, "entropy sync", MVX_TRI_UNSET);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_tristate(hnd, V4L2_CID_MVE_VIDEO_TEMPORAL_MVP, "temporal mvp", MVX_TRI_UNSET);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_TILE_ROWS, "tile rows", 0, 65536, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_TILE_COLS, "tile columns", 0, 65536, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_MIN_LUMA_CB_SIZE, "min luma cb size", 8, 16, 8, 8);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl = v4l2_ctrl_new_std_menu(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_MULTI_SLICE_MODE,
				      V4L2_MPEG_VIDEO_MULTI_SLICE_MODE_MAX_MB,
				      get_skip_mask(multi_slice_mode_list, ARRAY_SIZE(multi_slice_mode_list)),
				      V4L2_MPEG_VIDEO_MULTI_SLICE_MODE_SINGLE);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_MULTI_SLICE_MAX_MB, 0, INT_MAX, 1, 0);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_MV_H_SEARCH_RANGE, 0, INT_MAX, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_MV_V_SEARCH_RANGE, 0, INT_MAX, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_BITDEPTH_CHROMA, "bitdepth chroma", 0, 0xff, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_BITDEPTH_LUMA, "bitdepth luma", 0, 0xff, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_FORCE_CHROMA_FORMAT, "force chroma format", -1,
					    INT_MAX, 0, 1);
	if (ctrl == NULL)
		goto handler_free;
	...
	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_BANDWIDTH_LIMIT, "bandwidth limit", 0, INT_MAX, 0,
					    1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_CABAC_INIT_IDC, "CABAC init IDC", 0, 4, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_WATCHDOG_TIMEOUT, "watchdog timeout", 5, 60, 30, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_PROFILING, "enable profiling", 0, 1, 0, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_H263_I_FRAME_QP, 0, 31, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_H263_P_FRAME_QP, 0, 31, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_H263_B_FRAME_QP, 0, 31, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_H263_MIN_QP, 1, 31, 1, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_H263_MAX_QP, 1, 31, 1, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_H264_I_FRAME_QP, 0, 51, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_H264_P_FRAME_QP, 0, 51, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_H264_B_FRAME_QP, 0, 51, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_H264_MIN_QP, 1, 51, 1, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_H264_MAX_QP, 1, 51, 1, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_HEVC_I_FRAME_QP, 0, 51, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_HEVC_P_FRAME_QP, 0, 51, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_HEVC_B_FRAME_QP, 0, 51, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_HEVC_MIN_QP, 1, 51, 1, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_HEVC_MAX_QP, 1, 51, 1, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_VPX_I_FRAME_QP, 0, 51, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_VPX_P_FRAME_QP, 0, 51, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = mvx_v4l2_ctrl_new_custom_int(hnd, V4L2_CID_MVE_VIDEO_VPX_B_FRAME_QP, "VPx B frame QP value", 0, 51, 0,
					    1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_VPX_MIN_QP, 1, 51, 1, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_VPX_MAX_QP, 1, 51, 1, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MIN_BUFFERS_FOR_OUTPUT, 1, 32, 1, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl->flags |= V4L2_CTRL_FLAG_READ_ONLY | V4L2_CTRL_FLAG_VOLATILE;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MIN_BUFFERS_FOR_CAPTURE, 1, 32, 1, 1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl->flags |= V4L2_CTRL_FLAG_READ_ONLY | V4L2_CTRL_FLAG_VOLATILE;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_JPEG_RESTART_INTERVAL, -1, 0xffff, 1, -1);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_JPEG_COMPRESSION_QUALITY, 0, 100, 1, 0);
	if (ctrl == NULL)
		goto handler_free;

	ctrl = v4l2_ctrl_new_std(hnd, &ctrl_ops, V4L2_CID_MPEG_VIDEO_FORCE_KEY_FRAME, 0, 1, 1, 0);
	if (ctrl == NULL)
		goto handler_free;
