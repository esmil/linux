...
int mvx_v4l2_open(struct file *file)
{
	...
	if (session == NULL) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Failed to allocate V4L2 session.");
		return -ENOMEM;
	}

	MVX_SESSION_INFO(&session->session, "v4l2: Open device. id=%u.", ctx->dev->id);
	...
	if (ret != 0) {
		MVX_SESSION_WARN(&session->session, "Failed to register V4L2 controls handler. ret=%x", ret);
		goto put_session;
	}
...
unsigned int mvx_v4l2_poll(struct file *file, struct poll_table_struct *wait)
{
	...
	if (vsession->port[MVX_DIR_INPUT].q_set)
		revents |= vb2_poll(&vsession->port[MVX_DIR_INPUT].vb2_queue, file, wait);
	if (vsession->port[MVX_DIR_OUTPUT].q_set)
		revents |= vb2_poll(&vsession->port[MVX_DIR_OUTPUT].vb2_queue, file, wait);
#ifndef MODULE
	MVX_SESSION_VERBOSE(&vsession->session, "v4l2: Poll. events=0x%lx, revents=0x%x, nevents=%d.", events, revents,
			    v4l2_event_pending(&vsession->fh));
#else
	MVX_SESSION_VERBOSE(&vsession->session, "v4l2: Poll. events=0x%lx, revents=0x%x", events, revents);
#endif
unlock_mutex:
	mutex_unlock(&vsession->mutex);
...
int mvx_v4l2_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	MVX_SESSION_INFO(&session->session,
			 "v4l2: Memory map. start=0x%08lx, end=0x%08lx, pgoff=0x%08lx, flags=0x%08lx.", vma->vm_start,
	...
	ret = vb2_mmap(q, vma);
	if (ret != 0) {
		MVX_SESSION_WARN(&session->session, "Failed to memory map buffer. q=%p, pgoff=0x%08lx, dir=%d, ret=%d",
