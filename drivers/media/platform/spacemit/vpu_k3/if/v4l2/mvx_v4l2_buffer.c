...
static void v4l2_buffer_show(struct mvx_v4l2_buffer *buffer, struct seq_file *s)
{
	...
	for (i = 0; i < vb2->num_planes; ++i) {
		...
		mvx_seq_printf(s, tag, ind,
			       "bytesused: %10u, length: %10u, m.offset: %10u, m.userptr: %10lu, m.fd: %10d, "
			       "data_offset: %10u\n",
			       plane->bytesused, plane->length, plane->m.offset, plane->m.userptr, plane->m.fd,
			       plane->data_offset);
	}
...
static int buffer_debugfs_init(struct dentry *parent, struct mvx_v4l2_buffer *vbuf)
{
	char name[20];
	...
	dentry = debugfs_create_file("stat", 0400, vbuf->dentry, vbuf, &buffer_stat_fops);
	...
	if (mvx_is_afbc(buf->format)) {
		...
		if (buf->dir == MVX_DIR_OUTPUT) {
			...
			if (flags & V4L2_BUF_FRAME_FLAG_ROTATION_MASK) {
				...
				} else if ((flags & V4L2_BUF_FRAME_FLAG_ROTATION_MASK) ==
			...
			if (flags & V4L2_BUF_FRAME_FLAG_SCALING_MASK) {
				...
				} else if ((flags & V4L2_BUF_FRAME_FLAG_SCALING_MASK) ==
			...
			if (flags & V4L2_BUF_FRAME_FLAG_MIRROR_MASK) {
				...
				} else if ((flags & V4L2_BUF_FRAME_FLAG_MIRROR_MASK) ==
		...
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "unrecognized buffer format!.");
	}
...
static int update_mvx_buffer(struct mvx_v4l2_buffer *vbuf)
{
...
#else
	{
		...
		mvx_buf->user_data = ((uint64_t)ts->tv_sec << 32) | (ts->tv_usec & 0xffffffff);
	}
#endif
	...
	for (i = 0; i < vb2->num_planes; i++) {
		...
		if (V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE == vb2->type && vb2->memory == V4L2_MEMORY_DMABUF && offset > 0) {
		...
		} else if (V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE == vb2->type && vb2->memory == V4L2_MEMORY_DMABUF &&
			   i > 0) {
		...
		ret = mvx_buffer_filled_set(mvx_buf, i, filled, offset);
		if (ret != 0)
			return ret;
	}
...
static int update_v4l2_bytesused(struct mvx_v4l2_buffer *vbuf)
{
	 ...
	 */
	for (i = 0; i < b->num_planes; i++) {
		b->planes[i].bytesused = buf->planes[i].filled + buf->planes[i].offset;
...
static int update_vb2_flags(struct mvx_v4l2_buffer *vbuf)
{
	...
	b->flags &= ~(V4L2_BUF_FLAG_ERROR | V4L2_BUF_FLAG_KEYFRAME | V4L2_BUF_FLAG_LAST);
...
int mvx_v4l2_buffer_construct(struct mvx_v4l2_buffer *vbuf, struct mvx_v4l2_session *vsession, enum mvx_direction dir,
			      unsigned int nplanes, struct sg_table **sgt)
{
	int ret;

	ret = mvx_buffer_construct(&vbuf->buf, vsession->ext->dev, &vsession->session.mmu, dir, nplanes, sgt);
	if (ret != 0)
		return ret;
	...
	if (IS_ENABLED(CONFIG_DEBUG_FS)) {
		...
		if (ret != 0) {
			MVX_SESSION_WARN(&vsession->session, "Failed to create buffer debugfs entry.");
...
enum vb2_buffer_state mvx_v4l2_buffer_update(struct mvx_v4l2_buffer *vbuf)
{
	...
	if (ret != 0 || (vbuf->vb2_v4l2_buffer.flags & V4L2_BUF_FLAG_ERROR) != 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO, "buf flag is V4L2_BUF_FLAG_ERROR user_data:%llx dir:%d",
			      mvx_buf->user_data, mvx_buf->dir);
		goto error;
	}
