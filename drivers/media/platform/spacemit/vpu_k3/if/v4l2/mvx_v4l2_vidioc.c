...
struct mvx_format_map mvx_fmts[] = {
	...
	{MVX_FORMAT_AVS2, V4L2_FMT_FLAG_COMPRESSED, V4L2_PIX_FMT_AVS2, "AVS2"},
	{MVX_FORMAT_H263, V4L2_FMT_FLAG_COMPRESSED, V4L2_PIX_FMT_H263, "H.263"},
	{MVX_FORMAT_H264, V4L2_FMT_FLAG_COMPRESSED, V4L2_PIX_FMT_H264, "H.264"},
	{MVX_FORMAT_H264, V4L2_FMT_FLAG_COMPRESSED, V4L2_PIX_FMT_H264_MVC, "H.264 MVC"},
	{MVX_FORMAT_H264, V4L2_FMT_FLAG_COMPRESSED, V4L2_PIX_FMT_H264_NO_SC, "H.264 (No Start Codes)"},
	{MVX_FORMAT_HEVC, V4L2_FMT_FLAG_COMPRESSED, V4L2_PIX_FMT_HEVC, "HEVC"},
	{MVX_FORMAT_MPEG2, V4L2_FMT_FLAG_COMPRESSED, V4L2_PIX_FMT_MPEG2, "MPEG-2 ES"},
	{MVX_FORMAT_MPEG4, V4L2_FMT_FLAG_COMPRESSED, V4L2_PIX_FMT_MPEG4, "MPEG-4 part 2 ES"},
	{MVX_FORMAT_RV, V4L2_FMT_FLAG_COMPRESSED, V4L2_PIX_FMT_RV, "Real Video"},
	{MVX_FORMAT_VC1, V4L2_FMT_FLAG_COMPRESSED, V4L2_PIX_FMT_VC1_ANNEX_G, "VC-1 (SMPTE 412M Annex G)"},
	{MVX_FORMAT_VC1, V4L2_FMT_FLAG_COMPRESSED, V4L2_PIX_FMT_VC1_ANNEX_L, "VC-1 (SMPTE 412M Annex L)"},
	{MVX_FORMAT_VP8, V4L2_FMT_FLAG_COMPRESSED, V4L2_PIX_FMT_VP8, "VP8"},
	{MVX_FORMAT_VP9, V4L2_FMT_FLAG_COMPRESSED, V4L2_PIX_FMT_VP9, "VP9"},
	{MVX_FORMAT_JPEG, V4L2_FMT_FLAG_COMPRESSED, V4L2_PIX_FMT_JPEG, "JPEG"},
	{MVX_FORMAT_YUV420_AFBC_8, 0, V4L2_PIX_FMT_YUV420_AFBC_8, "YUV420 AFBC 8 bit"},
	{MVX_FORMAT_YUV420_AFBC_10, 0, V4L2_PIX_FMT_YUV420_AFBC_10, "YUV420 AFBC 10 bit"},
	{MVX_FORMAT_YUV422_AFBC_8, 0, V4L2_PIX_FMT_YUV422_AFBC_8, "YUV422 AFBC 8 bit"},
	{MVX_FORMAT_YUV422_AFBC_10, 0, V4L2_PIX_FMT_YUV422_AFBC_10, "YUV422 AFBC 10 bit"},
	{MVX_FORMAT_YUV420_I420, 0, V4L2_PIX_FMT_YUV420M, "Planar YUV 4:2:0 (N-C)"},
	{MVX_FORMAT_YUV420_NV12, 0, V4L2_PIX_FMT_NV12, "Y/CbCr 4:2:0"},
	{MVX_FORMAT_YUV420_NV21, 0, V4L2_PIX_FMT_NV21, "Y/CrCb 4:2:0 (N-C)"},
	{MVX_FORMAT_YUV420_NV12, 0, V4L2_PIX_FMT_NV12M, "Y/CbCr 4:2:0"},
	{MVX_FORMAT_YUV420_P010, 0, V4L2_PIX_FMT_P010, "YUV 4:2:0 P010 (Microsoft format)"},
	{MVX_FORMAT_YUV420_Y0L2, 0, V4L2_PIX_FMT_Y0L2, "YUV 4:2:0 Y0L2 (ARM format)"},
	{MVX_FORMAT_YUV420_AQB1, 0, v4l2_fourcc('Y', '0', 'A', 'B'), "YUV 4:2:0 AQB1 (ARM format)"},
	{MVX_FORMAT_YUV422_YUY2, 0, V4L2_PIX_FMT_YUYV, "YYUV 4:2:2"},
	{MVX_FORMAT_YUV422_UYVY, 0, V4L2_PIX_FMT_UYVY, "UYVY 4:2:2"},
	{MVX_FORMAT_YUV422_Y210, 0, V4L2_PIX_FMT_Y210, "YUV 4:2:2 Y210 (Microsoft format)"},

	/* ARGB */
	{MVX_FORMAT_ARGB_8888, 0, DRM_FORMAT_BGRA8888, /* Equal to V4L2_PIX_FMT_ARGB32. */
	 "32-bit ARGB 8-8-8-8"},
	...
	{MVX_FORMAT_BGRA_8888, 0, DRM_FORMAT_ARGB8888, /* Equal to V4L2_PIX_FMT_ABGR32. */
	 "32-bit BGRA 8-8-8-8"},
...
static int to_v4l2_format(struct v4l2_format *f, enum v4l2_buf_type type, struct v4l2_pix_format_mplane *pix,
			  unsigned int *stride, unsigned int *size, bool interlaced)
{
	f->type = type;
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE: {
		...
		for (i = 0; i < pix->num_planes; i++) {
			...
			memset(p->plane_fmt[i].reserved, 0, sizeof(p->plane_fmt[i].reserved));
		}
...
static int from_v4l2_format(struct mvx_v4l2_session *vsession, struct v4l2_format *f,
			    struct v4l2_pix_format_mplane *pix, enum mvx_format *format, unsigned int *stride,
			    unsigned int *size, bool *interlaced)
{
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE: {
		...
		if (p->num_planes > MVX_BUFFER_NPLANES)
			MVX_SESSION_WARN(&vsession->session,
					 "Too many planes for format. format=0x%08x, num_planes=%u.", pix->pixelformat,
					 p->num_planes);

		memcpy(pix, p, sizeof(*pix));

		for (i = 0; i < min_t(unsigned int, MVX_BUFFER_NPLANES, p->num_planes); i++) {
	...
	if (IS_ERR(map)) {
		MVX_SESSION_WARN(&vsession->session, "Unsupported V4L2 pixel format. format=0x%08x.", pix->pixelformat);
		return PTR_ERR(map);
	}
 ...
 */
static void print_format(struct mvx_session *session, struct v4l2_format *f, const char *prefix)
{
	if (V4L2_TYPE_IS_MULTIPLANAR(f->type) != false) {
		struct v4l2_pix_format_mplane *p = &f->fmt.pix_mp;

		MVX_SESSION_INFO(session, "v4l2: %s. type=%u, pixelformat=0x%08x, width=%u, height=%u, num_planes=%u.",
				 prefix, f->type, p->pixelformat, p->width, p->height, p->num_planes);
	} else {
		struct v4l2_pix_format *p = &f->fmt.pix;

		MVX_SESSION_INFO(session, "v4l2: %s. type=%u, pixelformat=0x%08x, width=%u, height=%u.", prefix,
...
static int queue_setup(struct vb2_queue *q, unsigned int *buf_cnt, unsigned int *plane_cnt, unsigned int plane_size[],
		       struct device *alloc_devs[])
#else
static int queue_setup(struct vb2_queue *q, const void *unused, unsigned int *buf_cnt, unsigned int *plane_cnt,
		       unsigned int plane_size[], void *alloc_devs[])
#endif
{
	 ...
	 */
	if (vport->dir == MVX_DIR_OUTPUT && mvx_is_frame(port->format) != false &&
	...
	*plane_cnt = port->nplanes;
	...
	MVX_SESSION_VERBOSE(session,
			    "queue_setup. vsession=%p, vport=%p, vb2_queue=%p, dir=%d, format=0x%x, width=%u, "
			    "height=%u, nplanes=%u, plane_size=[%u, %u, %u]",
			    vsession, vport, q, vport->dir, port->format, port->width, port->height, port->nplanes,
...
static int buf_init(struct vb2_buffer *b)
{
	...
	MVX_SESSION_VERBOSE(session, "v4l2: Initialize buffer. vb=%p, type=%u, index=%u, num_planes=%u.", b, b->type,
			    b->index, b->num_planes);

	if (b->num_planes > MVX_BUFFER_NPLANES) {
		MVX_SESSION_WARN(session, "Failed to initialize buffer. Too many planes. vb=%p, num_planes=%u.", b,
				 b->num_planes);
		return -EINVAL;
	}
	...
	for (i = 0; i < b->num_planes; ++i) {
		...
		if (sgt[i] == NULL) {
			MVX_SESSION_WARN(session, "Cannot fetch SG descriptor. vb=%p, plane=%u.", b, i);
			return -ENOMEM;
		}
	}

	ret = mvx_v4l2_buffer_construct(vbuf, vsession, vport->dir, b->num_planes, sgt);
...
static void buf_cleanup(struct vb2_buffer *b)
{
	...
	if (session->port[vport->dir].stream_on) {
		MVX_SESSION_VERBOSE(session,
				    "v4l2: Cleanup buffer in the coding process. Will remap mve va and pa address. "
				    "dir=%d, type=%u, index=%u, vb=%p, vbuf=%p.",
				    vport->dir, b->type, b->index, b, vbuf);
	} else {
		MVX_SESSION_VERBOSE(session, "v4l2: Cleanup buffer. type=%u, index=%u, vb=%p, vbuf=%p.", b->type,
				    b->index, b, vbuf);
	}
...
static int start_streaming(struct vb2_queue *q, unsigned int cnt)
{
	...
	MVX_SESSION_VERBOSE(session, "v4l2: Start streaming. queue=%p, type=%u, cnt=%u.", q, q->type, cnt);
...
static void stop_streaming(struct vb2_queue *q)
{
	...
	MVX_SESSION_VERBOSE(session, "v4l2: Stop streaming. queue=%p, type=%u.", q, q->type);
...
static void buf_queue(struct vb2_buffer *b)
{
	...
	MVX_SESSION_VERBOSE(session, "v4l2: Queue buffer. b=%p, type=%u, index=%u.", b, b->type, b->index);
...
int mvx_v4l2_vidioc_querycap(struct file *file, void *fh, struct v4l2_capability *cap)
{
	...
	strncpy(cap->driver, "mvx", sizeof(cap->driver));
	strncpy(cap->card, "Linlon Video device", sizeof(cap->card));
	strncpy(cap->bus_info, "platform:mvx", sizeof(cap->bus_info));
#else
	...
	cap->capabilities = V4L2_CAP_DEVICE_CAPS | V4L2_CAP_VIDEO_M2M | V4L2_CAP_VIDEO_M2M_MPLANE |
 ...
 */
static int mvx_v4l2_vidioc_enum_fmt_vid(struct mvx_v4l2_session *session, struct v4l2_fmtdesc *f,
...
#if (KERNEL_VERSION(6, 7, 0) <= LINUX_VERSION_CODE)
				strncpy(f->description, mvx_fmts[i].description, sizeof(f->description));
#else
				strlcpy(f->description, mvx_fmts[i].description, sizeof(f->description));
#endif
				break;
			}
...
int mvx_v4l2_vidioc_enum_fmt_vid_cap(struct file *file, void *fh, struct v4l2_fmtdesc *f)
{
	struct mvx_v4l2_session *session = file_to_session(file);
	int ret;
...
int mvx_v4l2_vidioc_enum_fmt_vid_out(struct file *file, void *fh, struct v4l2_fmtdesc *f)
{
	struct mvx_v4l2_session *session = file_to_session(file);
	int ret;
...
int mvx_v4l2_vidioc_enum_framesizes(struct file *file, void *fh, struct v4l2_frmsizeenum *fsize)
{
...
static int mvx_v4l2_vidioc_g_fmt_vid(struct file *file, struct v4l2_format *f, enum mvx_direction dir)
{
	...
	to_v4l2_format(f, f->type, &vport->pix_mp, port->stride, port->size, port->interlaced);
...
int mvx_v4l2_vidioc_g_fmt_vid_cap(struct file *file, void *fh, struct v4l2_format *f)
{
	return mvx_v4l2_vidioc_g_fmt_vid(file, f, MVX_DIR_OUTPUT);
}

int mvx_v4l2_vidioc_g_fmt_vid_out(struct file *file, void *fh, struct v4l2_format *f)
{
	return mvx_v4l2_vidioc_g_fmt_vid(file, f, MVX_DIR_INPUT);
}

static int mvx_v4l2_vidioc_s_fmt_vid(struct file *file, struct v4l2_format *f, enum mvx_direction dir)
{
	struct mvx_v4l2_session *vsession = file_to_session(file);
	struct mvx_v4l2_port *vport = &vsession->port[dir];
	...
	ret = from_v4l2_format(vsession, f, &pix_mp, &format, stride, size, &interlaced);
	if (ret != 0)
		goto unlock_mutex;

	/* Validate and adjust settings. */
	ret = mvx_session_set_format(&vsession->session, dir, format, &pix_mp.width, &pix_mp.height, &pix_mp.num_planes,
...
int mvx_v4l2_vidioc_s_fmt_vid_cap(struct file *file, void *fh, struct v4l2_format *f)
{
	return mvx_v4l2_vidioc_s_fmt_vid(file, f, MVX_DIR_OUTPUT);
}

int mvx_v4l2_vidioc_s_fmt_vid_out(struct file *file, void *fh, struct v4l2_format *f)
{
	struct mvx_v4l2_session *vsession = file_to_session(file);
	struct v4l2_pix_format_mplane *in = &vsession->port[MVX_DIR_INPUT].pix_mp;
	struct v4l2_pix_format_mplane *out = &vsession->port[MVX_DIR_OUTPUT].pix_mp;
	int ret;
...
static int mvx_v4l2_vidioc_try_fmt_vid(struct file *file, struct v4l2_format *f, enum mvx_direction dir)
{
	struct mvx_v4l2_session *vsession = file_to_session(file);
	...
	ret = from_v4l2_format(vsession, f, &pix, &format, stride, size, &interlaced);
	if (ret != 0)
		goto unlock_mutex;

	ret = mvx_session_try_format(&vsession->session, dir, format, &pix.width, &pix.height, &pix.num_planes, stride,
				     size, &interlaced);
	if (ret != 0)
		goto unlock_mutex;
...
int mvx_v4l2_vidioc_try_fmt_vid_cap(struct file *file, void *fh, struct v4l2_format *f)
{
	return mvx_v4l2_vidioc_try_fmt_vid(file, f, MVX_DIR_OUTPUT);
}

int mvx_v4l2_vidioc_try_fmt_vid_out(struct file *file, void *fh, struct v4l2_format *f)
{
...
{
	struct mvx_v4l2_session *vsession = file_to_session(file);
	enum mvx_direction dir = V4L2_TYPE_IS_OUTPUT(a->type) ? MVX_DIR_INPUT : MVX_DIR_OUTPUT;
	struct mvx_v4l2_port *vport = &vsession->port[dir];
	struct mvx_session_port *port = &vsession->session.port[dir];
	...
	MVX_SESSION_INFO(&vsession->session, "v4l2: Get crop. dir=%u, crop={left=%u, top=%u, width=%u, height=%u.", dir,
			 a->c.left, a->c.top, a->c.width, a->c.height);

	return 0;
}
int mvx_v4l2_vidioc_g_selection(struct file *file, void *fh, struct v4l2_selection *s)
{
...
int mvx_v4l2_vidioc_s_selection(struct file *file, void *fh, struct v4l2_selection *s)
{
	struct mvx_v4l2_session *vsession = file_to_session(file);
	int ret = 0;
	ret = mutex_lock_interruptible(&vsession->mutex);
...
int mvx_v4l2_vidioc_s_parm(struct file *file, void *fh, struct v4l2_streamparm *a)
{
	struct mvx_v4l2_session *vsession = file_to_session(file);
	int ret = 0;
	ret = mutex_lock_interruptible(&vsession->mutex);
	...
	if (V4L2_TYPE_IS_OUTPUT(a->type)) {
		int64_t framerate = ((int64_t)a->parm.output.timeperframe.denominator << 16) /
...
int mvx_v4l2_vidioc_g_parm(struct file *file, void *fh, struct v4l2_streamparm *a)
{
	struct mvx_v4l2_session *vsession = file_to_session(file);
	int ret = 0;
	ret = mutex_lock_interruptible(&vsession->mutex);
...
int mvx_v4l2_vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type type)
{
	struct mvx_v4l2_session *vsession = file_to_session(file);
	enum mvx_direction dir = V4L2_TYPE_IS_OUTPUT(type) ? MVX_DIR_INPUT : MVX_DIR_OUTPUT;
	int ret;
	...
	if (ret != 0)
		MVX_SESSION_WARN(&vsession->session, "v4l2: Failed to stream on. dir=%u.", dir);
...
int mvx_v4l2_vidioc_streamoff(struct file *file, void *priv, enum v4l2_buf_type type)
{
	struct mvx_v4l2_session *vsession = file_to_session(file);
	enum mvx_direction dir = V4L2_TYPE_IS_OUTPUT(type) ? MVX_DIR_INPUT : MVX_DIR_OUTPUT;
	int ret;
	...
	if (ret != 0)
		MVX_SESSION_WARN(&vsession->session, "v4l2: Failed to stream off. dir=%u.", dir);

	MVX_SESSION_INFO(&vsession->session, "v4l2: Stream off exit. dir=%u, ret=%d.", dir, ret);
...
int mvx_v4l2_vidioc_encoder_cmd(struct file *file, void *priv, struct v4l2_encoder_cmd *cmd)
{
	struct mvx_v4l2_session *vsession = file_to_session(file);
	int ret;

	MVX_SESSION_INFO(&vsession->session, "v4l2: encoder cmd: %u.", cmd->cmd);
	...
	case V4L2_ENC_CMD_START:
		...
		break;
	default:
		MVX_SESSION_WARN(&vsession->session, "Unsupported command. cmd: %u.", cmd->cmd);
		ret = -EINVAL;
	}
...
int mvx_v4l2_vidioc_try_encoder_cmd(struct file *file, void *priv, struct v4l2_encoder_cmd *cmd)
{
	switch (cmd->cmd) {
	case V4L2_ENC_CMD_STOP:
	case V4L2_ENC_CMD_START:
...
int mvx_v4l2_vidioc_decoder_cmd(struct file *file, void *priv, struct v4l2_decoder_cmd *cmd)
{
	struct mvx_v4l2_session *vsession = file_to_session(file);
	int ret;

	MVX_SESSION_INFO(&vsession->session, "v4l2: decoder cmd: %u.", cmd->cmd);
...
int mvx_v4l2_vidioc_try_decoder_cmd(struct file *file, void *priv, struct v4l2_decoder_cmd *cmd)
{
	switch (cmd->cmd) {
	case V4L2_DEC_CMD_STOP:
	case V4L2_DEC_CMD_START:
...
int mvx_v4l2_vidioc_reqbufs(struct file *file, void *fh, struct v4l2_requestbuffers *b)
{
	struct mvx_v4l2_session *vsession = file_to_session(file);
	enum mvx_direction dir = V4L2_TYPE_IS_OUTPUT(b->type) ? MVX_DIR_INPUT : MVX_DIR_OUTPUT;
	struct mvx_v4l2_port *vport = &vsession->port[dir];
	int ret;

	MVX_SESSION_INFO(&vsession->session, "v4l2: Request buffers. dir=%d, type=%u, memory=%u, count=%u.", dir,
...
int mvx_v4l2_vidioc_create_bufs(struct file *file, void *fh, struct v4l2_create_buffers *b)
{
	struct mvx_v4l2_session *vsession = file_to_session(file);
	enum mvx_direction dir = V4L2_TYPE_IS_OUTPUT(b->format.type) ? MVX_DIR_INPUT : MVX_DIR_OUTPUT;
	struct mvx_v4l2_port *vport = &vsession->port[dir];
	int ret;

	MVX_SESSION_INFO(&vsession->session, "v4l2: Create buffers. dir=%d, type=%u, memory=%u, count=%u.", dir,
			 b->format.type, b->memory, b->count);
...
int mvx_v4l2_vidioc_querybuf(struct file *file, void *fh, struct v4l2_buffer *b)
{
	...
	MVX_SESSION_INFO(&vsession->session, "v4l2: Query buffer. dir=%d, type=%u, memory=%u, index=%u.", dir, b->type,
			 b->memory, b->index);
...
int mvx_v4l2_vidioc_qbuf(struct file *file, void *fh, struct v4l2_buffer *b)
{
	...
	MVX_SESSION_INFO(&vsession->session, "v4l2: Queue buffer. dir=%d, type=%u, index=%u, flags=0x%x.", dir, b->type,
			 b->index, b->flags);
	...
	if ((b->flags & V4L2_BUF_FLAG_MVX_BUFFER_EPR) == V4L2_BUF_FLAG_MVX_BUFFER_EPR) {
		...
		MVX_SESSION_INFO(&vsession->session,
				 "v4l2: Queue buffer. type:%d, config size:%d, buffer size:%d, cfg_type:0x%x, cols and "
				 "rows:%d, %d",
				 v4l2_general->type, v4l2_general->config_size, v4l2_general->buffer_size,
				 v4l2_general->config.blk_cfg_type,
				 v4l2_general->config.blk_cfgs.rows_uncomp.n_cols_minus1,
...
int mvx_v4l2_vidioc_dqbuf(struct file *file, void *fh, struct v4l2_buffer *b)
{
	...
	MVX_SESSION_INFO(&vsession->session, "v4l2: Dequeue buffer. dir=%d, type=%u.", dir, b->type);
	...
	if (vsession->port[MVX_DIR_INPUT].port->format <= MVX_FORMAT_BITSTREAM_LAST && dir == MVX_DIR_OUTPUT &&
...
#ifndef MODULE
	MVX_SESSION_INFO(&vsession->session,
			 "v4l2: Dequeued buffer. dir=%d, type=%u, index=%u, flags=0x%x, nevents=%u, fh=%p.", dir,
			 b->type, b->index, b->flags, v4l2_event_pending(&vsession->fh), fh);
#else
	MVX_SESSION_INFO(&vsession->session, "v4l2: Dequeued buffer. dir=%d, type=%u, index=%u, flags=0x%x, fh=%p.",
			 dir, b->type, b->index, b->flags, fh);
#endif

	return ret;
}

int mvx_v4l2_vidioc_subscribe_event(struct v4l2_fh *fh, const struct v4l2_event_subscription *sub)
{
	struct mvx_v4l2_session *session = v4l2_fh_to_session(fh);

	MVX_SESSION_INFO(&session->session, "v4l2: Subscribe event. fh=%p, type=%u.", fh, sub->type);
	...
	default:
		MVX_SESSION_WARN(&session->session, "Can't register for unsupported event. type=%u.", sub->type);
		return -EINVAL;
	}
...
long mvx_v4l2_vidioc_default(struct file *file, void *fh, bool valid_prio, unsigned int cmd, void *arg)
{
	struct mvx_v4l2_session *vsession = file_to_session(file);
	int ret;
	MVX_SESSION_INFO(&vsession->session, "Custom ioctl. cmd=0x%x, arg=0x%p.", cmd, arg);
	...
	default:
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Unsupported IOCTL. cmd=0x%x", cmd);
		return -ENOTTY;
	}
