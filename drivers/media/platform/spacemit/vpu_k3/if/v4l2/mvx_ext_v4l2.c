...
int mvx_ext_if_construct(struct mvx_ext_if *ext, struct device *dev, struct mvx_fw_cache *cache,
			 struct mvx_client_ops *client_ops, struct dentry *parent)
{
	int ret;
	const char name[] = "mvx";
	...
	if (ret != 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_ERROR, "Failed to register V4L2 device. ret=%d.", ret);
		goto delete_dentry;
	}
	...
	ext->vdev.device_caps =
		V4L2_CAP_VIDEO_M2M | V4L2_CAP_VIDEO_M2M_MPLANE | V4L2_CAP_EXT_PIX_FORMAT | V4L2_CAP_STREAMING;
	...
	if (ret != 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_ERROR, "Failed to register video device. ret=%d.", ret);
