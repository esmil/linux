...
static void set_format(struct v4l2_pix_format_mplane *pix_mp, unsigned int width, unsigned int height,
		       unsigned int num_planes, unsigned int *sizeimage, unsigned int *bytesperline)
{
	int i;
...
static const struct file_operations port_stat_fops = {
	.open = port_stat_open, .read = seq_read, .llseek = seq_lseek, .release = single_release};

static int port_debugfs_init(struct device *dev, unsigned int i, struct mvx_v4l2_port *vport,
			     struct mvx_session_port *sport, struct dentry *parent)
{
	char name[20];
	...
	dentry = debugfs_create_file("stat", 0400, vport->dentry, vport, &port_stat_fops);
...
static int session_debugfs_init(struct mvx_v4l2_session *session, struct dentry *parent)
{
	int ret;
	char name[20];
	int i;
	...
	for (i = 0; i < MVX_DIR_MAX; i++) {
		...
		ret = port_debugfs_init(session->ext->dev, i, vport, mport, session->dentry);
		if (ret != 0)
			goto remove_dentry;
	}
...
static struct mvx_v4l2_session *mvx_session_to_v4l2_session(struct mvx_session *session)
{
...
static struct v4l2_subscribed_event *v4l2_event_subscribed(struct v4l2_fh *fh, u32 type, u32 id)
{
	...
	list_for_each_entry(sev, &fh->subscribed, list) if (sev->type == type && sev->id == id) return sev;
...
static void __v4l2_event_queue_fh(struct v4l2_fh *fh, const struct v4l2_event *ev, u64 ts)
{
	...
	if (sev->in_use == sev->elems) {
		...
		if (sev->elems == 1) {
			...
			struct v4l2_kevent *second_oldest = sev->events + sev_pos(sev, 0);
...
#endif
static void handle_event(struct mvx_session *session, enum mvx_session_event event, void *arg)
{
	struct mvx_v4l2_session *vsession = mvx_session_to_v4l2_session(session);

	MVX_SESSION_INFO(&vsession->session, "Event. event=%d, arg=%p.", event, arg);
	...
	case MVX_SESSION_EVENT_BUFFER: {
		...
		if (vbuf->buf.dir == MVX_DIR_OUTPUT && vb->state != VB2_BUF_STATE_QUEUED &&
		    !waitqueue_active(&vb->vb2_queue->done_wq) && vsession->port[MVX_DIR_INPUT].q_set &&
		    waitqueue_active(&vsession->port[MVX_DIR_INPUT].vb2_queue.done_wq)) {
			/*wake up waiters of input queue, because output queue may reallocated and not registered in
			 * waiters list */
	...
	case MVX_SESSION_EVENT_PORT_CHANGED: {
		...
		set_format(&vport->pix_mp, port->width, port->height, port->nplanes, port->size, port->stride);
#ifndef MODULE
		v4l2_event_queue_fh(&vsession->fh, &event);
#else
		mvx_v4l2_event_queue_fh(&vsession->fh, &event);
#endif
		break;
	}
	...
	default:
		MVX_SESSION_WARN(&vsession->session, "Unsupported session event. event=%d", event);
	}
}

int mvx_v4l2_session_construct(struct mvx_v4l2_session *vsession, struct mvx_ext_if *ctx)
{
	int i;
	int ret;
	...
	ret = mvx_session_construct(&vsession->session, ctx->dev, ctx->client_ops, ctx->cache, &vsession->mutex,
				    free_session, handle_event, vsession->dentry);
	if (ret != 0)
		goto remove_dentry;
...
struct mvx_v4l2_session *file_to_session(struct file *file) { return v4l2_fh_to_session(file->private_data); }

int mvx_v4l2_session_get_color_desc(struct mvx_v4l2_session *vsession, struct v4l2_mvx_color_desc *c)
{
	int ret;
...
int mvx_v4l2_session_set_color_desc(struct mvx_v4l2_session *vsession, struct v4l2_mvx_color_desc *c)
{
	int ret;
	struct mvx_fw_color_desc cd;
...
int mvx_v4l2_session_set_roi_regions(struct mvx_v4l2_session *vsession, struct v4l2_mvx_roi_regions *roi)
{
	int ret;
...
int mvx_v4l2_session_set_qp_epr(struct mvx_v4l2_session *vsession, int *qp)
{
	int ret;
...
int mvx_v4l2_session_set_sei_userdata(struct mvx_v4l2_session *vsession, struct v4l2_sei_user_data *sei_userdata)
{
	int ret;
...
int mvx_v4l2_session_set_rate_control(struct mvx_v4l2_session *vsession, struct v4l2_rate_control *rc)
{
	int ret;
...
int mvx_v4l2_session_set_dsl_frame(struct mvx_v4l2_session *vsession, struct v4l2_mvx_dsl_frame *dsl)
{
	int ret;
...
int mvx_v4l2_session_set_dsl_ratio(struct mvx_v4l2_session *vsession, struct v4l2_mvx_dsl_ratio *dsl)
{
	int ret;
...
int mvx_v4l2_session_set_long_term_ref(struct mvx_v4l2_session *vsession, struct v4l2_mvx_long_term_ref *ltr)
{
	int ret;
