...
static unsigned int kref_read(const struct kref *kref) { return atomic_read(&kref->refcount); }
 ...
 */
static bool test_bit_32(int bit, uint32_t *addr) { return 0 != (addr[bit >> 5] & (1 << (bit & 0x1f))); }
 ...
 */
static int get_fw_name(char *filename, size_t size, enum mvx_format format, enum mvx_direction dir,
		       struct mvx_hw_ver *hw_ver)
{
	...
	n = snprintf(filename, size, "linlon-%s-%u-%u/%s%s.fwb", hw_id_to_string(hw_ver->id), hw_ver->revision,
		     hw_ver->patch, codec, enc_dec);
	if (n >= size)
		return -ENOENT;
...
static struct mvx_fw_bin *kobj_to_fw_bin(struct kobject *kobj) { return container_of(kobj, struct mvx_fw_bin, kobj); }
...
static void fw_bin_destroy(struct kobject *kobj)
{
	...
	MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO, "Releasing firmware binary. bin=0x%p.", fw_bin);

	if (fw_bin->securevideo == false && IS_ERR_OR_NULL(fw_bin->nonsecure.fw) == false)
		release_firmware(fw_bin->nonsecure.fw);

	if (fw_bin->securevideo != false && IS_ERR_OR_NULL(fw_bin->secure.securefw) == false)
...
static int fw_bin_validate(const struct firmware *fw, struct device *dev)
{
	...
	if (fw->size < sizeof(*header)) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING,
			      "Firmware binary size smaller than firmware header. size=%zu.", fw->size);
		return -EFAULT;
	}
 ...
 */
static void secure_request_firmware_done(struct mvx_secure_firmware *securefw, void *arg)
{
	struct mvx_fw_bin *fw_bin = arg;

	if (securefw == NULL) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Failed to load secure firmware binary. filename=%s.",
			      fw_bin->filename);
...
static void request_firmware_done(const struct firmware *fw, void *arg)
{
	...
	if (fw == NULL) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Failed to load firmware binary. filename=%s.",
			      fw_bin->filename);
		fw = ERR_PTR(-EINVAL);
	...
	for (i = 0; i < header->bss_bitmap_size; i++) {
		if (va >= header->master_rw_start_address &&
		    va < (header->master_rw_start_address + header->master_rw_size))
	...
	MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_INFO,
		      "Loaded firmware binary. bin=0x%p, major=%u, minor=%u, info=\"%s\", jump=0x%x, pages={text=%u, "
		      "bss=%u, shared=%u}, text_length=%u, bss=0x%x.",
		      fw_bin, header->protocol_major, header->protocol_minor, header->info_string, header->rasc_jmp,
		      fw_bin->nonsecure.text_cnt, fw_bin->nonsecure.bss_cnt, fw_bin->nonsecure.sbss_cnt,
...
static ssize_t hw_ver_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct mvx_fw_bin *fw_bin = kobj_to_fw_bin(kobj);
	struct mvx_hw_ver *hw_ver = &fw_bin->hw_ver;

	return scnprintf(buf, PAGE_SIZE, "%s-%u-%u\n", hw_id_to_string(hw_ver->id), hw_ver->revision, hw_ver->patch);
}
...
static ssize_t dirty_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct mvx_fw_bin *fw_bin = kobj_to_fw_bin(kobj);
	int dirty = 0;

	if (atomic_read(&fw_bin->flush_cnt) != atomic_read(&fw_bin->cache->flush_cnt))
		dirty = 1;
...
static struct attribute *fw_bin_attrs[] = {&path_attr.attr, &count_attr.attr, &hw_ver_attr.attr, &dirty_attr.attr,
					   NULL};

static const struct attribute_group fw_bin_group = {.name = "", .attrs = fw_bin_attrs};

static const struct attribute_group *fw_bin_groups[] = {&fw_bin_group, NULL};

static struct kobj_type fw_bin_ktype = {
	.release = fw_bin_destroy, .sysfs_ops = &kobj_sysfs_ops, .default_groups = fw_bin_groups};

/**
 * fw_bin_create() - Create a new firmware binary instance.
 */
static struct mvx_fw_bin *fw_bin_create(struct mvx_fw_cache *cache, enum mvx_format format, enum mvx_direction dir,
					struct mvx_hw_ver *hw_ver, bool securevideo)
{
	...
	ret = kobject_init_and_add(&fw_bin->kobj, &fw_bin_ktype, &cache->kobj, "%lx", (unsigned long)fw_bin);
	if (ret != 0)
		goto free_fw_bin;

	ret = get_fw_name(fw_bin->filename, sizeof(fw_bin->filename), format, dir, &fw_bin->hw_ver);
	if (ret != 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_ERROR, "No firmware available. format=%d, direction=%d.", format,
			      dir);
		goto free_fw_bin;
	}
	...
	if (securevideo != false)
		ret = mvx_secure_request_firmware_nowait(cache->secure, fw_bin->filename, MVX_SECURE_NUMCORES, fw_bin,
							 secure_request_firmware_done);
	else
		ret = request_firmware_nowait(THIS_MODULE, true, fw_bin->filename, fw_bin->dev, GFP_KERNEL, fw_bin,
					      request_firmware_done);

	if (ret != 0) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_ERROR, "Failed to request firmware. filename=%s, securevideo=%d.",
 ...
 * into memory.
 */
static struct mvx_fw_bin *fw_bin_get(struct mvx_fw_cache *cache, enum mvx_format format, enum mvx_direction dir,
				     struct mvx_hw_ver *hw_ver, bool securevideo)
{
	...
	{
		if (tmp->format == format && tmp->dir == dir && hwvercmp(&tmp->hw_ver, hw_ver) == 0 &&
		    tmp->securevideo == securevideo && atomic_read(&tmp->flush_cnt) == atomic_read(&cache->flush_cnt)) {
			fw_bin = tmp;
			break;
		}
	}
 ...
 */
static ssize_t cache_flush_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
 ...
 */
static ssize_t cache_flush_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t size)
{
 ...
 */
static struct kobj_attribute cache_flush = __ATTR(flush, 0600, cache_flush_show, cache_flush_store);
...
int mvx_fw_cache_construct(struct mvx_fw_cache *cache, struct device *dev, struct mvx_secure *secure,
			   struct kobject *kobj_parent)
{
	...
	ret = kobject_init_and_add(&cache->kobj, &cache_ktype, kobj_parent, "fw_cache");
	if (ret != 0)
		goto kobj_put;
...
void mvx_fw_cache_destruct(struct mvx_fw_cache *cache) { kobject_put(&cache->kobj); }

int mvx_fw_cache_get(struct mvx_fw_cache *cache, enum mvx_format format, enum mvx_direction dir,
		     struct mvx_fw_event *event, struct mvx_hw_ver *hw_ver, bool securevideo)
{
	int ret;
	struct mvx_fw_bin *fw_bin;
	...
	} else if (fw_bin->securevideo == false && IS_ERR_OR_NULL(fw_bin->nonsecure.fw) == false) {
		mutex_unlock(&fw_bin->mutex);
		event->fw_bin_ready(fw_bin, event->arg, true);
	} else {
...
void mvx_fw_cache_log(struct mvx_fw_bin *fw_bin, struct mvx_client_session *csession)
{
...
void mvx_fw_cache_get_formats(struct mvx_fw_cache *cache, enum mvx_direction direction, uint64_t *formats)
{
