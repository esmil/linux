 ...
 */
static bool test_bit_32(int bit, const uint32_t *addr) { return 0 != (addr[bit >> 5] & (1 << (bit & 0x1f))); }
...
static void fw_unmap(struct mvx_fw *fw)
{
	...
	if (fw->fw_bin->securevideo == false) {
		...
		for (i = 0; i < fw->ncores; i++) {
			ret = fw->ops.get_region(MVX_FW_REGION_CORE_0 + i, &begin, &end);
			if (ret == 0)
				mvx_mmu_unmap_va(fw->mmu, begin, 4 * 1024 * 1024);
		}
...
static int fw_map_core(struct mvx_fw *fw, unsigned int core)
{
	...
	/* Map text segment. */
	ret = mvx_mmu_map_pages(fw->mmu, fw_base + FW_TEXT_BASE_ADDR, fw->text, MVX_ATTR_PRIVATE,
	...
	for (i = 0; i < header->bss_bitmap_size; i++) {
		if (va >= header->master_rw_start_address &&
			...
			ret = mvx_mmu_map_pa(fw->mmu, fw_base + va, fw->bss_shared->pages[bss_scnt++], MVE_PAGE_SIZE,
					     MVX_ATTR_PRIVATE, MVX_ACCESS_READ_WRITE);
		else if (test_bit_32(i, header->bss_bitmap))
			ret = mvx_mmu_map_pa(fw->mmu, fw_base + va, fw->bss->pages[bss_cnt++], MVE_PAGE_SIZE,
					     MVX_ATTR_PRIVATE, MVX_ACCESS_READ_WRITE);
...
static int fw_map(struct mvx_fw *fw)
{
	...
	if (fw->fw_bin->securevideo != false) {
		...
		for (i = 0; i < fw->ncores; i++) {
			mvx_mmu_va fw_base;
			mvx_mmu_va end;
			phys_addr_t l2 = fw->fw_bin->secure.securefw->l2pages + i * MVE_PAGE_SIZE;

			ret = fw->ops.get_region(MVX_FW_REGION_CORE_0 + i, &fw_base, &end);
			if (ret != 0)
				return ret;
		...
		ret = mvx_mmu_write(fw->mmu, FW_TEXT_BASE_ADDR, fw_bin->nonsecure.fw->data, header->text_length);
		if (ret != 0)
			goto unmap_fw;
	}
...
static const struct file_operations fw_stat_fops = {
	.open = fw_stat_open, .read = seq_read, .llseek = seq_lseek, .release = single_release};
...
static void *rpcmem_seq_start(struct seq_file *s, loff_t *pos)
{
	...
	return mvx_seq_hash_start(fw->dev, fw->rpc_mem, HASH_SIZE(fw->rpc_mem), *pos);
}
...
static int rpcmem_seq_show(struct seq_file *s, void *v)
{
	...
	struct mvx_mmu_pages *pages = hlist_entry(it->node, struct mvx_mmu_pages, node);
...
static int fw_debugfs_init(struct mvx_fw *fw, struct dentry *parent)
{
	...
	dentry = debugfs_create_file("stat", 0400, fw->dentry, fw, &fw_stat_fops);
	...
	if (fw->fw_bin->securevideo == false) {
		...
		ret = mvx_mmu_pages_debugfs_init(fw->bss_shared, "bss_shared", fw->dentry);
		if (ret != 0)
			goto remove_dentry;

		dentry = debugfs_create_file("rpc_mem", 0400, fw->dentry, fw, &rpcmem_fops);
		if (IS_ERR_OR_NULL(dentry)) {
			ret = -ENOMEM;
			goto remove_dentry;
		}
		   ...
		   struct dentry *parent)
{
	...
	if ((fw_bin->securevideo == false && IS_ERR_OR_NULL(fw_bin->nonsecure.fw)) ||
	    (fw_bin->securevideo != false && IS_ERR_OR_NULL(fw_bin->secure.securefw))) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Firmware binary was loaded with error.");
		return -EINVAL;
	}

	if (fw_bin->securevideo != false && ncores > fw_bin->secure.securefw->ncores) {
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING, "Too few secure cores setup. max_ncores=%u, ncores=%u.",
	...
	switch (major) {
	case 2:
		ret = mvx_fw_construct_v2(fw, fw_bin, mmu, session, client_ops, csession, ncores, major, minor);
		if (ret != 0)
			return ret;

		break;
	case 3:
		ret = mvx_fw_construct_v3(fw, fw_bin, mmu, session, client_ops, csession, ncores, major, minor);
		if (ret != 0)
			return ret;
	...
	default:
		MVX_LOG_PRINT(&mvx_log_if, MVX_LOG_WARNING,
			      "Unsupported firmware interface revision. major=%u, minor=%u.", major, minor);
		return -EINVAL;
	}
