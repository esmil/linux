...
#define mvx_seq_printf(s, tag, ind, fmt, ...)                                                                          \
	seq_printf(s, "%-*s%-*s: " fmt, (3 * (ind)), "", 30 - (3 * (ind)), tag, ##__VA_ARGS__)
 ...
 */
struct mvx_seq_hash_it *mvx_seq_hash_start(struct device *dev, struct hlist_head *head, size_t size, loff_t pos);
 ...
 * is over.
 */
struct mvx_seq_hash_it *mvx_seq_hash_next(void *v, struct hlist_head *head, size_t size, loff_t *pos);
