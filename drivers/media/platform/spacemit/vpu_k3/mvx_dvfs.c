...
static bool allocate_session(const mvx_session_id session_id, bool is_encoder)
{
	...
	if (NULL == session) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING,
			      "DVFS is unable to allocate memory for a new session. session=%p", session_id);
		return false;
	}
...
static void warm_up(const int steps)
{
	...
	if (old_freq != new_freq) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_INFO, "warm_up. buffer count: %d, old_freq: %d, new_freq: %d",
			      steps, old_freq, new_freq);
...
static int get_restrict_buffer_count(mvx_session_id session_id)
{
	...
	if (!dvfs_session->is_encoder) {
		...
		// MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_INFO, "get_restrict_buffer_count. out buffer_count: %d,
		// buffer_on_hold_count: %d, in buffer_count: %d", session->port[MVX_DIR_OUTPUT].buffer_count,
		// session->port[MVX_DIR_OUTPUT].buffer_on_hold_count, session->port[MVX_DIR_INPUT].buffer_count);
	} else {
		buffers_cnt = session->port[MVX_DIR_INPUT].buffer_count;
		if (false != session->eos_queued && buffers_cnt < 2) {
			buffers_cnt = 2;
		}
		// MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_INFO, "get_restrict_buffer_count. buffer_count: %d, eos_queued:
		// %d", session->port[MVX_DIR_INPUT].buffer_count, session->eos_queued);
	}
...
static void update_sessions(void)
{
	...
	list_for_each_safe(entry, safe, &sessions)
	{
		...
		if (restrict_buffer_count < 0) {
			MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING,
				      "DVFS failed to retrieve status for the session. Session was removed? "
				      "session=%p, restrict_buffer_count=%d",
 ...
 */
static ssize_t sysfs_print_stats(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t num = 0;
	struct list_head *entry;
	struct session *session;
	...
	num += scnprintf(buf + num, PAGE_SIZE - num, "freq: %4u, max_freq: %4u, up_step_freq: %3u, down_step_freq: %3u",
	...
	num += scnprintf(buf + num, PAGE_SIZE - num,
			 ...
			 atomic_read(&dvfs_enabled), mvx_dvfs_ctx.poll_interval_ms, atomic_long_read(&changes_cnt),
			 atomic_long_read(&burst_cnt));
#endif
	num += scnprintf(buf + num, PAGE_SIZE - num, "\n");
	list_for_each(entry, &sessions)
	{
		session = list_entry(entry, struct session, list);
		num += scnprintf(buf + num, PAGE_SIZE - num, "%p: out_buf: %02u\n", session->session_id,
				 session->restrict_buffer_count);
	}
 ...
 * Used for debugging/verification purposes.
 */
static ssize_t sysfs_print_enabled(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t num = 0;
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_enabled(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int failed;
	int enabled;
 ...
 * Used for debugging/verification purposes.
 */
static ssize_t sysfs_print_freq(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t num = 0;
	uint32_t freq = get_clock_rate();
	num += scnprintf(buf, PAGE_SIZE, "%u\n", freq);
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_freq(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int failed;
	unsigned int freq;
 ...
 * Used for debugging/verification purposes.
 */
static ssize_t sysfs_print_min_freq(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t num = 0;
...
uint32_t clip_min_max_rate(uint32_t freq, bool is_min_freq)
{
	...
	if (is_min_freq) {
		...
		if (!clip)
			clip_freq = vpufclk_freqtable[FREQ_TABLE_SIZE - 1].freq;
	} else {
		...
		if (!clip)
			clip_freq = vpufclk_freqtable[0].freq;
	}
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_min_freq(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int failed;
	unsigned int freq;
	failed = kstrtouint(buf, 10, &freq);
 ...
 * Used for debugging/verification purposes.
 */
static ssize_t sysfs_print_max_frep(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t num = 0;
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_max_freq(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int failed;
	unsigned int freq;
	failed = kstrtouint(buf, 10, &freq);
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_poll_interval_ms(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int failed;
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_up_step_percent(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int failed;
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_down_step_percent(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int failed;
 ...
 * Used for debugging/verification purposes.
 */
static ssize_t sysfs_print_available_freq(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t num = 0;
	int32_t i;
	for (i = 0; i < FREQ_TABLE_SIZE; i++) {
...
#if (1 == VPU_DDR_QOS_ENABLE)
static ssize_t sysfs_print_ddr_qos_enable(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t num = 0;
...
static ssize_t sysfs_set_ddr_qos_enable(struct device *dev, struct device_attribute *attr, const char *buf,
					size_t count)
{
	int failed;
	int enable;
 ...
 * Used for debugging/verification purposes.
 */
static ssize_t sysfs_print_watchdog_timeout(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t num = 0;
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_watchdog_timeout(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int failed;
 ...
 * Used for debugging/verification purposes.
 */
static ssize_t sysfs_print_wait_pending_timeout(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t num = 0;
 ...
 * Used for debugging/verification purposes.
 */
ssize_t sysfs_set_wait_pending_timeout(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int failed;
...
static struct device_attribute sysfs_files[] = {
	...
	__ATTR(wait_pending_timeout, (S_IRUGO | S_IWUSR), sysfs_print_wait_pending_timeout,
	       sysfs_set_wait_pending_timeout),
};
...
static void sysfs_register_devices(struct device *dev)
{
	...
	while (i--) {
		...
		if (err < 0) {
			MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_ERROR, "DVFS is unable to create sysfs file. name=%s",
...
#ifdef CONFIG_THERMAL
static int vpu_get_max_state(struct thermal_cooling_device *cdev, unsigned long *state)
{
...
static int vpu_get_cur_state(struct thermal_cooling_device *cdev, unsigned long *state)
{
	struct mvx_dvfs_ctx_t *ctx = cdev->devdata;
...
static int vpu_set_cur_state(struct thermal_cooling_device *cdev, unsigned long state)
{
	struct mvx_dvfs_ctx_t *ctx = cdev->devdata;
...
void mvx_dvfs_init(struct device *dev)
{
	if (!initialized) {
		...
		if (of_find_property(dev->of_node, "#cooling-cells", NULL)) {
			mvx_dvfs_ctx.max_state = FREQ_TABLE_SIZE - 1;
			mvx_dvfs_ctx.cur_state = 0; // VPU_VMIN_LEVEL_6 for cur_state 0
			mvx_dvfs_ctx.cdev =
				thermal_cooling_device_register("mvx-vpu-clk", &mvx_dvfs_ctx, &vpu_cooling_ops);
			if (IS_ERR(mvx_dvfs_ctx.cdev)) {
				pr_err("%s: Failed to register vpu cooling device (%d)\n", __func__,
				       (int)PTR_ERR(mvx_dvfs_ctx.cdev));
...
bool mvx_dvfs_register_session(const mvx_session_id session_id, bool is_encoder)
{
	...
	if (!initialized) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING, "DVFS module was not initialized");
		return false;
	}

	sem_failed = down_interruptible(&dvfs_sem);
	if (sem_failed) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING, "DVFS semaphore was not obtained, sem_failed=%d",
			      sem_failed);
		return false;
	}
	...
	if (get_session(session_id) != NULL) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_INFO, "this session is already registered. session=%p", session_id);
		up(&dvfs_sem);
		return true;
	}

	MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_INFO, "mvx_dvfs_register_session. session=%p", session_id);
...
void mvx_dvfs_unregister_session(const mvx_session_id session_id)
{
	...
	sem_failed = down_interruptible(&dvfs_sem);
	if (sem_failed) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING, "DVFS semaphore was not obtained, %d", sem_failed);
		return;
	}
	...
	if (NULL != session) {
		...
		if ((session->ddr_qos_read + session->ddr_qos_write) &&
			...
			mvx_dvfs_ctx.ddr_qos_rsum -= session->ddr_qos_read;
			mvx_dvfs_ctx.ddr_qos_wsum -= session->ddr_qos_write;
			MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_INFO, "DVFS remove session[%p] ddr qos: [%d, %d]/[%d, %d]",
				      session_id, session->ddr_qos_read, session->ddr_qos_write,
				      mvx_dvfs_ctx.ddr_qos_rsum, mvx_dvfs_ctx.ddr_qos_wsum);
			session->ddr_qos_read = session->ddr_qos_write = 0;
		}
#endif
...
void mvx_dvfs_estimate_ddr_bandwidth(struct estimate_ddr_input *input, struct estimate_ddr_output *output)
{
	...
	MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_INFO,
		      "estimate_ddr_bandwidth. isEnc=%d, isAFBC=%d, size=(%d, %d), framerate=%d.", input->isEnc,
		      input->isAFBC, input->width, input->height, input->fps);

	estimated_read =
		((uint64_t)defined_bandwidth_tbl[input->isEnc][input->isAFBC][0] * (input->width * input->height) *
		 input->fps / (defined_width * defined_height * VPU_DDR_QOS_PREDEFINED_FPS));
	estimated_write =
		((uint64_t)defined_bandwidth_tbl[input->isEnc][input->isAFBC][1] * (input->width * input->height) *
		 input->fps / (defined_width * defined_height * VPU_DDR_QOS_PREDEFINED_FPS));
...
void mvx_dvfs_session_update_ddr_qos(const mvx_session_id session_id, uint32_t read_value, uint32_t write_value)
{
	...
	sem_failed = down_interruptible(&dvfs_sem);
	if (sem_failed) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_ERROR, "DVFS semaphore was not obtained, %d", sem_failed);
		return;
	}

	session = get_session(session_id);
	if (NULL != session && (session->ddr_qos_read != read_value || session->ddr_qos_write != write_value) &&
	    mvx_dvfs_ctx.ddr_qos_rsum >= session->ddr_qos_read && mvx_dvfs_ctx.ddr_qos_wsum >= session->ddr_qos_write) {
		mvx_dvfs_ctx.ddr_qos_rsum -= session->ddr_qos_read;
		...
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_INFO, "DVFS update session[%p] ddr qos: [%d, %d]/[%d, %d]",
			      session_id, read_value, write_value, mvx_dvfs_ctx.ddr_qos_rsum,
			      mvx_dvfs_ctx.ddr_qos_wsum);
	}
