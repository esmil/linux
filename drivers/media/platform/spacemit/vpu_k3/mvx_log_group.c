...
int mvx_log_group_init(const char *entry_name)
{
	...
	mvx_log_group_construct(&mvx_log_if, "MVX if", MVX_LOG_WARNING, drain_default);
	ret = mvx_log_group_add(&log, "generic", &mvx_log_if);
	if (ret != 0)
		goto delete_ftrace_drain;

	mvx_log_group_construct(&mvx_log_fwif_if, "MVX fwif", MVX_LOG_INFO, drain_ram);
	ret = mvx_log_group_add(&log, "firmware_interface", &mvx_log_fwif_if);
	if (ret != 0)
		goto delete_generic_group;

	mvx_log_group_construct(&mvx_log_session_if, "MVX session", MVX_LOG_WARNING, drain_default);
	ret = mvx_log_group_add(&log, "session", &mvx_log_session_if);
	if (ret != 0)
		goto delete_fwif_group;

	mvx_log_group_construct(&mvx_log_dev, "MVX dev", MVX_LOG_WARNING, drain_default);
