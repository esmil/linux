...
static int map_session(struct mvx_sched *sched, struct mvx_sched_session *session, struct mvx_lsid *lsid)
{
	int ret;

	MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_INFO, "%p Map LSID. lsid=%u, jobqueue=%08x, corelsid=%08x.",
...
static void unmap_session(struct mvx_sched *sched, struct mvx_sched_session *session)
{
	...
	MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_INFO, "%p Unmap LSID. lsid=%u, jobqueue=%08x, corelsid=%08x.",
		      mvx_if_session_to_session(session->isession), lsid->lsid,
		      mvx_hwreg_read(sched->hwreg, MVX_HWREG_JOBQUEUE),
		      mvx_hwreg_read(sched->hwreg, MVX_HWREG_CORELSID));
...
static struct list_head *list_find_node(struct list_head *list, struct list_head *node)
{
	struct list_head *i;
...
{
	struct mvx_sched *sched = container_of(ws, struct mvx_sched, sched_task);
	...
	list_for_each_entry_safe(pending, tmp, &sched->pending, pending) {
		...
		if (pending->lsid != NULL) {
			...
			ret = mvx_lsid_jobqueue_add(pending->lsid, pending->isession->ncores);
			if (ret != 0) {
				MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING,
					      "Cannot add pending session to job queue. csession=%p, mvx_session=%p",
					      pending, mvx_if_session_to_session(pending->isession));
				continue;
			}
		...
		if (lsid->session != NULL) {
			...
			if (ret != 0) {
				...
				MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_INFO, "Ref is zero. csession=%p", unmapped);
			}
		}
		...
		if (ret != 0) {
			MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING,
				      "Cannot map pending session. csession=%p, mvx_session=%p", pending,
		...
		if (ret != 0) {
			MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING,
				      "Cannot add pending session to job queue. csession=%p, mvx_session=%p", pending,
				      mvx_if_session_to_session(pending->isession));
			continue;
		}
...
static void sched_session_print(struct seq_file *s, struct mvx_sched_session *session, struct mvx_hwreg *hwreg, int ind)
{
	struct mvx_lsid *lsid;
...
int mvx_sched_construct(struct mvx_sched *sched, struct device *dev, struct mvx_if_ops *if_ops, struct mvx_hwreg *hwreg,
			struct dentry *parent)
{
	...
	if (!sched->sched_queue) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING, "Cannot create work queue");
		return -EINVAL;
	}
...
int mvx_sched_session_construct(struct mvx_sched_session *session, struct mvx_if_session *isession)
{
	uint32_t disallow;
	uint32_t maxcores;
	...
	session->pcb.ctrl = (disallow << MVE_CTRL_DISALLOW_SHIFT) | (maxcores << MVE_CTRL_MAXCORES_SHIFT);
...
int mvx_sched_switch_in(struct mvx_sched *sched, struct mvx_sched_session *session)
{
	int ret;

	MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_INFO, "%p Switch in session. jobqueue=%08x, coreslid=%08x.",
		      mvx_if_session_to_session(session->isession), mvx_hwreg_read(sched->hwreg, MVX_HWREG_JOBQUEUE),
		      mvx_hwreg_read(sched->hwreg, MVX_HWREG_CORELSID));
	...
	if (session->in_pending) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_VERBOSE, "Session is already in pending state.");
		goto unlock_mutex;
	}
...
int mvx_sched_send_irq(struct mvx_sched *sched, struct mvx_sched_session *session)
{
	mutex_lock(&sched->mutex);

	MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_DEBUG, "%p Send irq. lsid=%d, jobqueue=%08x, corelsid=%08x.",
		      mvx_if_session_to_session(session->isession), session->lsid == NULL ? -1 : session->lsid->lsid,
		      mvx_hwreg_read(sched->hwreg, MVX_HWREG_JOBQUEUE),
		      mvx_hwreg_read(sched->hwreg, MVX_HWREG_CORELSID));
...
int mvx_sched_flush_mmu(struct mvx_sched *sched, struct mvx_sched_session *session)
{
	mutex_lock(&sched->mutex);
...
static void print_session(struct mvx_sched *sched, struct mvx_sched_session *session, struct mvx_session *s)
{
	int lsid = -1;
	uint32_t irqve = 0;
	uint32_t irqhost = 0;
	...
	MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING, "%p    session=%p, lsid=%d, irqve=%08x, irqhost=%08x", s,
		      mvx_if_session_to_session(session->isession), lsid, irqve, irqhost);
}
...
void mvx_sched_print_debug(struct mvx_sched *sched, struct mvx_sched_session *session)
{
	struct mvx_hwreg *hwreg = sched->hwreg;
	struct mvx_sched_session *pending;
	struct mvx_sched_session *tmp;
	...
	list_for_each_entry_safe(pending, tmp, &sched->pending, pending) { print_session(sched, pending, s); }

	MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING, "%p Print register:", s);

	MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING,
		      "%p     jobqueue=%08x, corelsid=%08x, "
		      "irqve=%08x,HARDWARE_ID=0x%08x,ENABLE=0x%08x,NCORES=0x%08x,NLSID=0x%08x,CLKFORCE=0x%08x,PROTCTRL="
		      "0x%08x,RESET=0x%08x,FUSE=0x%08x",
		      s, mvx_hwreg_read(hwreg, MVX_HWREG_JOBQUEUE), mvx_hwreg_read(hwreg, MVX_HWREG_CORELSID),
		      mvx_hwreg_read(hwreg, MVX_HWREG_IRQVE), mvx_hwreg_read(hwreg, MVX_HWREG_HARDWARE_ID),
		      mvx_hwreg_read(hwreg, MVX_HWREG_ENABLE), mvx_hwreg_read(hwreg, MVX_HWREG_NCORES),
		      mvx_hwreg_read(hwreg, MVX_HWREG_NLSID), mvx_hwreg_read(hwreg, MVX_HWREG_CLKFORCE),
		      mvx_hwreg_read(hwreg, MVX_HWREG_PROTCTRL), mvx_hwreg_read(hwreg, MVX_HWREG_RESET),
	...
	for (i = 0; i < sched->nlsid; i++) {
		...
		MVX_LOG_PRINT(
			...
			"%p     lsid=%u, session=%p, irqve=%08x, "
			"irqhost=%08x,CTRL=0x%08x,MMU_CTRL=0x%08x,NPROT=0x%08x,ALLOC=0x%08x,FLUSH_ALL=0x%08x,SCHED=0x%"
			"08x,TERMINATE=0x%08x,INTSIG=0x%08x,STREAMID=0x%08x",
			s, i, ls, mvx_hwreg_read_lsid(hwreg, i, MVX_HWREG_LIRQVE),
			mvx_hwreg_read_lsid(hwreg, i, MVX_HWREG_IRQHOST), mvx_hwreg_read_lsid(hwreg, i, MVX_HWREG_CTRL),
			mvx_hwreg_read_lsid(hwreg, i, MVX_HWREG_MMU_CTRL),
			mvx_hwreg_read_lsid(hwreg, i, MVX_HWREG_NPROT), mvx_hwreg_read_lsid(hwreg, i, MVX_HWREG_ALLOC),
...
void mvx_sched_terminate(struct mvx_sched *sched, struct mvx_sched_session *session)
{
...
static void suspend_session(struct mvx_sched *sched, struct mvx_sched_session *session)
{
	struct mvx_lsid *lsid = session->lsid;
	...
	MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING, "%p suspend_session LSID. lsid=%u, jobqueue=%08x, corelsid=%08x.",
		      mvx_if_session_to_session(session->isession), lsid->lsid,
		      mvx_hwreg_read(sched->hwreg, MVX_HWREG_JOBQUEUE),
		      mvx_hwreg_read(sched->hwreg, MVX_HWREG_CORELSID));
...
static int resume_session(struct mvx_sched *sched, struct mvx_sched_session *session, struct mvx_lsid *lsid)
{
	int ret;

	MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING, "%p resume_session LSID. lsid=%u, jobqueue=%08x, corelsid=%08x.",
		      mvx_if_session_to_session(session->isession), lsid->lsid,
		      mvx_hwreg_read(sched->hwreg, MVX_HWREG_JOBQUEUE),
		      mvx_hwreg_read(sched->hwreg, MVX_HWREG_CORELSID));
...
static void switch_out_session(struct mvx_sched *sched, unsigned int lsid_id)
{
	...
	for (i = 0; i < wait_count; i++) {
		if (sched->lsid[lsid_id].session != NULL && session != NULL && session->switched_in == false) {
			MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING, "%p finish switch_out session LSID. lsid=%u.",
				      session, sched_session->lsid->lsid);
			break;
		}
		mutex_unlock(&sched->mutex);
		msleep(10);
		...
		if (sched->lsid[lsid_id].session != NULL && session != NULL) {
			MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING,
				      "%p wait switch_out session LSID. lsid=%u. loop=%d, switch_in=%d", session,
				      sched_session->lsid->lsid, i, session->switched_in);
		}
	}
}
