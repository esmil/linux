...
static void get_formats(struct mvx_client_ops *client, enum mvx_direction direction, uint64_t *formats)
{
	struct mvx_dev_ctx *ctx = client_ops_to_ctx(client);
	uint32_t fuses;
...
static struct mvx_client_session *register_session(struct mvx_client_ops *client, struct mvx_if_session *isession)
{
	struct mvx_dev_ctx *ctx = client_ops_to_ctx(client);
...
{
	MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_INFO, "%p Unregister client session. csession=0x%p.",
		      mvx_if_session_to_session(csession->session.isession), csession);
...
static irqreturn_t irq_top(int irq, void *dev_id)
{
	...
	while (nlsid-- > 0)
		if ((irqve >> nlsid) & 0x1) {
			mvx_hwreg_write_lsid(&ctx->hwreg, nlsid, MVX_HWREG_LIRQVE, 0);
...
static int mvx_dev_probe(struct device *dev, struct resource *iores, struct resource *irqres)
{
	struct mvx_dev_ctx *ctx;
	int ret;
	...
	ret = mvx_sched_construct(&ctx->scheduler, dev, ctx->if_ops, &ctx->hwreg, ctx->dentry);
	if (ret != 0)
		goto destruct_hwreg;
	...
	if (ctx->work_queue == NULL) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_ERROR, "Failed to create work queue.");
		ret = -EINVAL;
		goto destruct_sched;
	}
	...
	ret = request_irq(ctx->irq, irq_top, IRQF_SHARED | (irqres->flags & IRQF_TRIGGER_MASK), dev_name(dev), ctx);
	if (ret != 0) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_ERROR, "Failed to request IRQ. irq=%u, ret=%d.", ctx->irq, ret);
		goto workqueue_destroy;
	}

	MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING,
		      "Linlon v%x identified. cores=%u, nlsid=%u, id=%u, fuse=%08x, hw_id=%08x, hw_rev=%08x",
		      mvx_hwreg_get_hw_id(&ctx->hwreg, NULL, NULL), mvx_hwreg_read(&ctx->hwreg, MVX_HWREG_NCORES),
		      mvx_hwreg_read(&ctx->hwreg, MVX_HWREG_NLSID), dev->id, ctx->fuses, ctx->hw_id, ctx->hw_revision);
...
static int mvx_pdev_probe(struct platform_device *pdev)
{
	...
	if (ret != 0) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_ERROR, "Failed to get address of resource. ret=%d.", ret);
		return ret;
	}
