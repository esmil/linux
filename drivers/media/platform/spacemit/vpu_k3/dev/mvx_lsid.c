...
{
	return (reg >> (MVE_CORELSID_LSID_BITS * core)) & MVX_CORELSID_LSID_MASK;
}
...
{
	return (reg >> (MVE_JOBQUEUE_JOB_BITS * nr + MVE_JOBQUEUE_LSID_SHIFT)) & MVE_JOBQUEUE_LSID_MASK;
}

static uint32_t set_lsid_ncores(uint32_t reg, unsigned int nr, unsigned int lsid, unsigned int ncores)
{
	reg &= ~(MVE_JOBQUEUE_JOB_MASK << (nr * MVE_JOBQUEUE_JOB_BITS));
	reg |= ((lsid << MVE_JOBQUEUE_LSID_SHIFT) | ((ncores - 1) << MVE_JOBQUEUE_NCORES_SHIFT))
...
int mvx_lsid_construct(struct mvx_lsid *lsid, struct device *dev, struct mvx_hwreg *hwreg, unsigned int id)
{
	lsid->dev = dev;
	lsid->hwreg = hwreg;
	lsid->session = NULL;
...
int mvx_lsid_map(struct mvx_lsid *lsid, struct mvx_lsid_pcb *pcb)
{
	...
	if (is_alloc(lsid)) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING,
			      "Failed to map session to LSID. LSID already allocated. lsid=%u.", lsid->lsid);
		return -EFAULT;
	}
	...
	do {
		ret = mvx_hwreg_read_lsid(hwreg, lsid->lsid, MVX_HWREG_TERMINATE);
	} while (ret != 0);
	...
	mvx_hwreg_write_lsid(hwreg, lsid->lsid, MVX_HWREG_MMU_CTRL, pcb->mmu_ctrl);
	mvx_hwreg_write_lsid(hwreg, lsid->lsid, MVX_HWREG_FLUSH_ALL, 0);

	if (of_property_read_u32_array(lsid->dev->of_node, "busattr", busattr, ARRAY_SIZE(busattr))) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_VERBOSE, "busattr in of_node is not available.");
		...
		for (i = 0; i < ARRAY_SIZE(busattr); i++)
			MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_VERBOSE, "busattr[%d] = 0x%x.", i, busattr[i]);
	}

	mvx_hwreg_write_lsid(hwreg, lsid->lsid, MVX_HWREG_BUSATTR_0, busattr[0]);
	mvx_hwreg_write_lsid(hwreg, lsid->lsid, MVX_HWREG_BUSATTR_1, busattr[1]);
	mvx_hwreg_write_lsid(hwreg, lsid->lsid, MVX_HWREG_BUSATTR_2, busattr[2]);
	mvx_hwreg_write_lsid(hwreg, lsid->lsid, MVX_HWREG_BUSATTR_3, busattr[3]);

	/* Restore interrupt registers. */
	mvx_hwreg_write_lsid(hwreg, lsid->lsid, MVX_HWREG_LIRQVE, 0);
	mvx_hwreg_write_lsid(hwreg, lsid->lsid, MVX_HWREG_IRQHOST, pcb->irqhost);
...
void mvx_lsid_unmap(struct mvx_lsid *lsid, struct mvx_lsid_pcb *pcb)
{
	...
	if (!is_alloc(lsid)) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING, "LSID was not allocated. lsid=%u.", lsid->lsid);
		return;
	}
	...
	pcb->mmu_ctrl = mvx_hwreg_read_lsid(hwreg, lsid->lsid, MVX_HWREG_MMU_CTRL);
	pcb->irqhost = mvx_hwreg_read_lsid(hwreg, lsid->lsid, MVX_HWREG_IRQHOST);
	pcb->nprot = mvx_hwreg_read_lsid(hwreg, lsid->lsid, MVX_HWREG_NPROT);

	/* Deallocate LSID. */
	mvx_hwreg_write_lsid(hwreg, lsid->lsid, MVX_HWREG_ALLOC, MVE_ALLOC_FREE);
}
...
int mvx_lsid_jobqueue_add(struct mvx_lsid *lsid, unsigned int ncores)
{
	...
	for (i = 0; i < MVE_JOBQUEUE_NJOBS; i++)
		if (get_jobqueue_lsid(jobqueue, i) == MVE_JOBQUEUE_JOB_INVALID) {
			jobqueue = set_lsid_ncores(jobqueue, i, lsid->lsid, ncores);
...
void mvx_lsid_jobqueue_remove(struct mvx_lsid *lsid)
{
	...
	for (i = 0, j = 0; i < MVE_JOBQUEUE_NJOBS; i++)
		if (get_jobqueue_lsid(jobqueue, i) != lsid->lsid)
			jobqueue = set_jobqueue_job(jobqueue, j++, get_jobqueue_job(jobqueue, i));

	/* Blank out remaining job entries. */
	for (; j < MVE_JOBQUEUE_NJOBS; j++)
		jobqueue = set_lsid_ncores(jobqueue, j, MVE_JOBQUEUE_JOB_INVALID, ncores);
