...
static enum mvx_hw_id get_hw_id(void *registers, uint32_t *revision, uint32_t *patch)
{
	uint32_t value;
	...
	case 0x5663:
		return MVE_v52_v76;
	default:
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_ERROR, "Unknown hardware version. version=0x%08x.", value);
		break;
	}
...
static int regs_open(struct inode *inode, struct file *file) { return single_open(file, regs_show, inode->i_private); }

static const struct file_operations regs_fops = {
	.open = regs_open, .read = seq_read, .llseek = seq_lseek, .release = single_release};
...
static int lsid_regs_debugfs_init(struct mvx_lsid_hwreg *lsid_hwreg, struct dentry *parent)
{
	struct dentry *dentry;
	char name[20];
	...
	dentry = debugfs_create_file(name, 0400, parent, lsid_hwreg, &lsid_regs_fops);
	if (IS_ERR_OR_NULL(dentry))
		return -ENOMEM;
...
int debugfs_init(struct mvx_hwreg *hwreg, struct dentry *parent)
{
	...
	if (IS_ENABLED(CONFIG_DEBUG_FS)) {
		...
		for (lsid = 0; lsid < MVX_LSID_MAX; ++lsid) {
			ret = lsid_regs_debugfs_init(&hwreg->lsid_hwreg[lsid], parent);
			if (ret != 0)
				return ret;
		}
	}
...
int mvx_hwreg_construct(struct mvx_hwreg *hwreg, struct device *dev, struct resource *res, struct dentry *parent)
{
	...
	if (hwreg->res == NULL) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_ERROR, "Failed to request mem region. start=0x%llx, size=0x%llx.",
	...
	if (hwreg->registers == NULL) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_ERROR, "Failed to iomap region. start=0x%llx, size=0x%llx.",
			      res->start, resource_size(res));
		ret = -ENOMEM;
		goto release_mem;
	}
...
void mvx_hwreg_write(struct mvx_hwreg *hwreg, enum mvx_hwreg_what what, uint32_t value)
{
	unsigned int offset = get_offset(what);
...
uint32_t mvx_hwreg_read_lsid(struct mvx_hwreg *hwreg, unsigned int lsid, enum mvx_hwreg_lsid what)
{
...
void mvx_hwreg_write_lsid(struct mvx_hwreg *hwreg, unsigned int lsid, enum mvx_hwreg_lsid what, uint32_t value)
{
	unsigned int offset = get_lsid_offset(lsid, what);
...
enum mvx_hw_id mvx_hwreg_get_hw_id(struct mvx_hwreg *hwreg, uint32_t *revision, uint32_t *patch)
{
