...
int mvx_pm_runtime_get_sync(struct device *dev)
{
	...
	if (ret < 0)
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING, "PM runtime get sync failed! ret=%d", ret);
...
int mvx_pm_runtime_put_sync(struct device *dev)
{
	...
	if (ret < 0) {
		MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING, "PM runtime put sync failed! ret=%d", ret);
		while (ret == -EAGAIN && retry_count > 0) {
			...
			MVX_LOG_PRINT(
				...
				"PM runtime put sync return EAGAIN. try to put sync again. ret=%d, retry_count=%d", ret,
				retry_count);
			retry_count--;
		}
	}
