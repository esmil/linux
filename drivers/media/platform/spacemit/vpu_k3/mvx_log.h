...
#define MVX_LOG_PRINT_SESSION(_lg, _severity, _session, _fmt, ...)                                                     \
	do {                                                                                                           \
		if ((_severity) <= (_lg)->severity) {                                                                  \
			__MVX_LOG_PRINT(_lg, _severity, "%px " _fmt, _session, ##__VA_ARGS__);                         \
		}                                                                                                      \
	} while (0)
...
#define __MVX_LOG_PRINT(_lg, _severity, _fmt, ...)                                                                     \
	((_lg)->drain->print((_lg)->drain, _severity, (_lg)->tag, _fmt " (%s:%d)", __MVX_LOG_N_ARGS(__VA_ARGS__),      \
			     ##__VA_ARGS__, mvx_log_strrchr(__FILE__), __LINE__))
#else
#define __MVX_LOG_PRINT(_lg, _severity, _fmt, ...)                                                                     \
	((_lg)->drain->print((_lg)->drain, _severity, (_lg)->tag, _fmt, __MVX_LOG_N_ARGS(__VA_ARGS__), ##__VA_ARGS__))
#endif /* MVX_LOG_PRINT_FILE_ENABLE */

#define __MVX_LOG_N_ARGS(...) __MVX_LOG_COUNT(dummy, ##__VA_ARGS__, 8, 7, 6, 5, 4, 3, 2, 1, 0)
 ...
 */
typedef void (*mvx_print_fptr)(struct mvx_log_drain *drain, enum mvx_log_severity severity, const char *tag,
 ...
 */
typedef void (*mvx_data_fptr)(struct mvx_log_drain *drain, enum mvx_log_severity severity, struct iovec *vec,
			      size_t count);
