...
static const struct dev_pm_ops spacemit_mmp_pdma_pm_qos = {
	NOIRQ_SYSTEM_SLEEP_PM_OPS(mmp_pdma_suspend_noirq, mmp_pdma_resume_noirq)
};
#endif

static struct platform_driver spacemit_mmp_pdma_driver = {
	.driver		= {
		.name	= "mmp-pdma",
#ifdef CONFIG_PM_SLEEP
		.pm	= &spacemit_mmp_pdma_pm_qos,
#endif
...
{
	return platform_driver_register(&spacemit_mmp_pdma_driver);
}

static void __exit mmp_pdma_exit(void)
{
	platform_driver_unregister(&spacemit_mmp_pdma_driver);
}
