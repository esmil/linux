...
static int dma_malloc(dma_map_info_t *dma_info, struct vm_area_struct *vma)
{
	...
	if (ret < 0) {
		dev_err(dma_dev->dev,
			"Unable to remap address %p to userspace address 0x%lx, size %zu\n",
			dma_info->kern_addr, dma_info->user_addr, dma_info->size);
		return -1;
	}
...
static int kernel_malloc(dma_map_info_t *dma_info, struct vm_area_struct *vma)
{
	...
	dma_info->dma_addr = dma_map_single(dma_dev->dev, dma_info->kern_addr,
...
static int va2pa(void *va, size_t size, va2pa_t **va2pa)
{
	...
	p = kmalloc_array(num, sizeof(*p), GFP_KERNEL);
	*va2pa = p;
